...
Once freeze mode has been implemented ([Freeze Mode](/versioned_docs/privilegesecurefordiscovery_2.21/privilegesecure/discovery/freeze_mode.md)), persistent access needs to be reduced. This should be done   
in a risk-based approach, by targeting the groups that convey most access. This activity can be split by   
workstations/laptops and servers, if that suits the organization. The steps described in this article  
explain the steps that should be undertaken group by group to remove persistent access to eliminate   
lateral movement.

### Project Management Steps

The Active Directory group or groups to be targeted for removal of persistence should be decided. This  
could be done by targeting a specific team and hence the group(s) that they use for their access.   
Once the group(s) have been decided, the people in those AD groups should be informed that change is   
...
change to be implemented. A checklist should be created to ensure that the following steps are carried   
out:
...
- Service accounts have been applied directly to the necessary machines (via group or individual) and set as persistent
...
The group(s) should now be reviewed for any service accounts that are exist within the group(s). These   
do not need to be removed from the group, but should be applied directly to the same machines that   
the group exists on. This will ensure that the software or process that utilizes that service account will   
continue to run.

### Apply Service Accounts Directly to Machine

Applying these service accounts directly to the machines that they exist on as part of the interactive   
group can easily be done by Quickstart. The method for carrying this out is detailed in the “Applying a   
...
Once the services have been mitigated, a Quickstart file should be obtained for the target machines.   
This should either cover all of the environment, or could be split between servers and workstations.   
...
![Screenshot__3_.png](/img/versioned_docs/privilegesecurefordiscovery_2.21/privilegesecure/discovery/admin/configuration/5099864805015_screenshot__3__543x307.png)

This produces an Excel file with two tabs, Computer Data and Admin List.

### Update Quickstart File

Updating the excel file to remove persistence is easy. Using the Admin List tab, filter for the group that   
needs persistence to be reverted, in this case Domain Admins.

![Screenshot_2022-03-29_094603.png](/img/versioned_docs/privilegesecurefordiscovery_2.21/privilegesecure/discovery/admin/configuration/5099864805015_screenshot_2022-03-29_094603.png)
...
It is recommended to include the date and what the purpose of the excel file is expected to do within   
the file name. Use the --dry-run flag to check that file will make the intended changes. This will remove the persistence and ensure that users who gain administrative access to machines using this group must now use Privilege Secure.
...
- Removing a group's persistence across ALL machines makes the change management much easier. i.e. As of this date all users of a given group must start using Privilege Secure for access. Breaking this into multiple sets of machines can cause confusion and also make the technical steps more challenging to track.
