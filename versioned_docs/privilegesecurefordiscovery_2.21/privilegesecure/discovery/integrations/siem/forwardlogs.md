...
This topic covers forwarding Privilege Secure application logs to a syslog server or Security Information and Event Management (SIEM) solutions, such as:
...
Edit the match docker.\*\* section.  Example below.  Mind the indents, alter IP, port, and tag as necessary.  File example contents:
...
![360063180233_mceclip1](/img/versioned_docs/privilegesecurefordiscovery_2.21/privilegesecure/discovery/integrations/siem/360063180233_mceclip1.png)
  ...
  - sudo tcpdump -vv -i any port 514
    ...
    - Alternative tcpdump example, ‘any’ can be change to a specific adapter:
      ...
      - sudo tcpdump -n -e -q -i any dst 10.100.11.13
        ...
        - -n=no dns lookup; -e=link-level header; -q=quiet (less protocol info); -i=interface; dst=destination
