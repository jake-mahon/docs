...
__NOTE:__ All PPE Tool operations can be executed from the Command Prompt, if run with administrator rights.

### PPE Tool Operations

___RECOMMENDED:___ PPE Tool operations should only be executed one at a time. For example, you should not execute the /e (Export) and /i (Import) operations simultaneously; you should not run /e (Export) and /r (Report) operations simultaneously.
