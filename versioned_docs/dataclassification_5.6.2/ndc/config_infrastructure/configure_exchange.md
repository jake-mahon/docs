...
2. Go to Roles > __Role Assignments__ > __Exchange__.
3. Select __Add new role__.
4. In the Set up basics step, enter the Name and Description '_NetwrixCrawlerImpersonation_' . Select __Next__.
5. On the __Add Permission__  step, select ApplicationImpersonation and Mailbox Search permissions. Select __Next__.

   ![exchange_assign_roles](/img/versioned_docs/dataclassification_5.6.2/ndc/config_infrastructure/exchange_assign_roles.png)
6. Select the users to assign to this role group. They will have permissions to manage the roles that you assigned in the previous step.
7. Finish adding the permissions by selecting ____Add role group____.
...
__NOTE:__ You need to use an admin user name with Modern Authentication.
...
2. Configure [Exchange Server](/versioned_docs/dataclassification_5.6.2/ndc/sources/exchange_mailbox/exchange_server.md) source settings.
