...
![addtogroup](/img/versioned_docs/dataclassification_5.6.2/ndc/sources/source_groups/addtogroup.png)

A group can either be "mixed", which allows it to contain all source types, or source specific. In the example above a group would be created entitled "Demo Content", which only supports the addition of SharePoint sources. If a supporting source group already exists, this can be selected from the drop-down list provided. [See Dynamic Source Groups for more information.](#dynamic-source-groups)

Selecting the cog icon on the main sources grid screen for a source group allows you to amend the group settings:

![editgroup](/img/versioned_docs/enterpriseauditor_11.6/config/dellpowerscale/editgroup.png)

Here you can:

- Amend the group name
- Delete the group
- Enable / Disable Indexing—When disabled content will not be processed into the core search index (content will be excluded from Browse / Search / Suggestions, classification will occur as normal)
...
- Enable / Disable Content Collection — Disabled files within the source group will not be processed. Containers (folders, lists etc.) will be processed as normal with their children inserted at a pending state until child collection is re-enabled. This allows a source to be fully enumerated before any files are processed
...
- [Dynamic Source Groups — Exchange](/versioned_docs/dataclassification_5.6.2/ndc/sources/source_groups/dynamic_source_groups_-_exchange.md)
- [Dynamic Source Groups — File Servers](/versioned_docs/dataclassification_5.6.2/ndc/sources/source_groups/dynamic_source_groups_-_file.md)
- [Dynamic Source Groups — Google Drive Organization](/versioned_docs/dataclassification_5.6.2/ndc/sources/source_groups/dynamic_source_groups_-_google_drive.md)
- [Dynamic Source Groups — SharePoint Online](/versioned_docs/dataclassification_5.6.2/ndc/sources/source_groups/dynamic_source_groups_-_sharepoint.md)
