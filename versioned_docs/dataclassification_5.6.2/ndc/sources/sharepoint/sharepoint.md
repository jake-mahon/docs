...
![addsharepoint](/img/versioned_docs/dataclassification_5.6.2/ndc/sources/sharepoint/addsharepoint.png)
...
| Re-Index Period | Specifies how often the source should be checked for changes. The number specifies the period in days.  __NOTE:__ Netwrix Data Classification monitors site collections to detect when a document is added/modified. These will then be queued for reprocessing. The source will still be checked for changes based on the re-index period in case any updates are not received. [See Manage Sources and Control Data Processing for more information.](/versioned_docs/dataclassification_5.6.2/ndc/sources/sources_manage.md) |
