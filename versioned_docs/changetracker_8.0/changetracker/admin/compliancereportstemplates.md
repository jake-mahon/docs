...
To import a new template or update an existing one, just upload the template: use the ‘__Overwrite__’ option if updating.

![ComplianceReportsTemplates](/img/versioned_docs/changetracker_8.0/changetracker/admin/compliancereportstemplates.png)

## Reports Layout Templates Administration

Formats used for the various reports produced by Netwrix Change Tracker Gen 7 are controlled via a series of underlying templates. New templates, or updated versions of existing templates, can be added to the system here. The __Version__ and __Change Date__ details for each report are displayed and when a new version is available, either after a Gen 7 version upgrade or by manually uploading a new template through the UI, this can be updated, or the current format preserved if necessary.

For report template modifications or new formats, please contact [Netwrix Support](https://www.netwrix.com/support.html).

![ComplianceReportsLayout](/img/versioned_docs/changetracker_8.0/changetracker/admin/compliancereportslayout.png)
