...
- Rescan Resources — Scans resources for services and scheduled tasks dependent on the selected service account
- Ready to Scan — Opens the Select Resources to Scan window. See the topic for additional information.
...
Follow the steps to update the service account password and then update all Windows services and Scheduled tasks that run as the at account with the new password.

__Step 1 –__ Navigate to the __Dashboards__ > __Credentials__ page.

__Step 2 –__ In the Credentials table, select the desired account. You can apply the Type filter of Service to find all service accounts with dependencies.

__Step 3 –__ Select the checkbox for the service account for the rotation.

__Step 4 –__ Click Rotate Service Account to open the Account Dependencies window.

__Step 5 –__ Click __Change Now__ to initiate credential rotation.
