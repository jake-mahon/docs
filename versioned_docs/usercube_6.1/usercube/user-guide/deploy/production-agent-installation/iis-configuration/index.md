...
5. Make sure that IIS contains an SSL certificate, by accessing the home page of IIS server and double-clicking on __Server Certificates__.
   ...
   If the certificate is not there yet, import it by clicking on __Import�__ in the right-side menu, and specify the certificate's path and password.
...
6. Add the certificate's URL to the site by right-clicking on the site, selecting __Edit Bindings�__ and clicking on __Add__, then choosing ```https``` as type, ```443``` as port, specifying the server's URL (without the ```https``` part) as host name, and finally selecting the server certificate.
...
To continue, [set the right permissions on the working directory](/versioned_docs/usercube_6.1/usercube/user-guide/deploy/production-agent-installation/directory-permissions/index.md).
