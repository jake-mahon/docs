...
In the following migration examples ```--version``` always refers to the _starting_ version. Make sure to reference either 5.2.1, 5.2.2 or 5.2.3 as is appropriate.
...
1. Install the new .NET 6.0 Framework hosting bundle from [https://dotnet.microsoft.com/en-us/download/dotnet/6.0](https://dotnet.microsoft.com/en-us/download/dotnet/6.0).
2. Stop server
3. Backup and Install
   ...
   5. In the appsettings.json file, at the end of the connection string add ```;Encrypt=false``` or ```;Encrypt=true``` depending on the database configuration.
      > For example:
      >
      > ```
      >
...
7. Optional: Migrate the configuration (This allows new features to be taken into account.)
   ...
   1. Make sure the XML configuration has already been imported to the database. If not, import the configuration with the old Runtime into the database.
   ...
   5. Deploy the configuration, correcting any warnings or new errors (that appear because the configuration import tool is smarter).
   6. Restart the server
