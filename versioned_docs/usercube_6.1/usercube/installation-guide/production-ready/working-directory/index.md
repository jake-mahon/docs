...
The working directory is a simple Windows directory where Usercube's Server and/or Agent executable(s) and dependencies are stored on the workstation. This section shows how to set up the directory for the rest of the installation and Usercube's lifespan.
...
The recommended naming convention is ```C:/Usercube<Organization>```, where ```<Organization>``` is the name of the organization targeted by this installation.
...
- ```Usercube-Agent.exe```: the separate Usercube Agent executable, that will be used only if you choose to install a separate agent.
- ```appsettings.*.json```: [technical configuration files](/versioned_docs/usercube_6.1/usercube/integration-guide/network-configuration/index.md).

## What's Next?

Next section shows how to [install the Usercube Database](/versioned_docs/usercube_6.1/usercube/installation-guide/production-ready/database/index.md).
