...
Usercube server or agent can be configured to go through a proxy server to access internal or external web resources.

## Introduction

A Usercube agent often needs to access internal or external systems using the HTTP protocol. It may easily be configured to use a proxy server through which all or part of the HTTP traffic will be routed.
...
The ```HTTPS_PROXY``` environment variable may be the hostname or IP address, optionally followed by a colon and port number, or it may be an http URL, optionally [including a username and password](#including-a-username-and-password) for proxy authentication.
...
For example, while running the Usercube agent all tasks started by the agent will inherit the proxy environment variables.
...
To test that a Usercube agent using a proxy server can reach the Usercube server:
