...
- [```Usercube-Protect-X509JsonValue.exe```](/versioned_docs/usercube_6.1/usercube/integration-guide/executables/references/protect-x509jsonvalue/index.md) to encrypt given values;
- [```Usercube-Protect-X509JsonFile.exe```](/versioned_docs/usercube_6.1/usercube/integration-guide/executables/references/protect-x509jsonfile/index.md) to encrypt a whole file.

  The file encryption tool should be used only on files that contain only plain text values, not already encrypted ones.

Once encrypted, sensitive values can be added to the ```appsettings.encrypted.json``` and ```appsettings.encrypted.agent.json``` files. Usercube will read first the values from the encrypted appsettings files, before reading those from the usual non-encrypted appsettings files.
...
These files follow the exact same structure as the [agent's configuration files](/versioned_docs/usercube_6.1/usercube/integration-guide/network-configuration/agent-configuration/index.md).

### Read the Encrypted Files

Usercube can use an RSA decoding algorithm fed by a [public-key certificate](https://en.wikipedia.org/wiki/X.509) in order to read the encrypted application settings.
