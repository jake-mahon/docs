...
| Name | Details |
| --- | --- |
| ConnectionString   required | __Type__    String   __Description__ Identification token used to retrieve the connection information for the server to access Usercube's database in SQL Server.   __Note:__ must be compliant with [SQL Server connection string syntax](/versioned_docs/usercube_6.1/usercube/installation-guide/production-ready/server/index.md#sql-server-connection-string-syntax). [See more information in the Installation Guide](/versioned_docs/usercube_6.1/usercube/installation-guide/production-ready/server/index.md#see-more-information-in-the-installation-guide). __Example__``` {     �     "ConnectionString": "data source=.;Database=Usercube;Integrated Security=SSPI;Min Pool Size=10;encrypt=false;"   } ``` |
| ConnectionStringGovernor   required | __Type__    String   __Description__ Identification token used to retrieve the connection information to SQL Server Resource Governor which is a feature used to manage SQL Server's workload and system resource consumption.   __Info:__ Resource Governor enables specifying limits on the amount of CPU, physical I/O, and memory that incoming application requests can use.    __Note:__ must be compliant with [SQL Server connection string syntax](/versioned_docs/usercube_6.1/usercube/installation-guide/production-ready/server/index.md#sql-server-connection-string-syntax).   __Note:__ all tasks and jobs use this connection string, when specified. __Example__``` {     �     "ConnectionStringGovernor": "data source=.;Database=Usercube;Integrated Security=SSPI;Min Pool Size=10;encrypt=false;"   } ``` |
