...
Once encrypted, sensitive values can be added to the ```appsettings.encrypted.json``` file. Usercube will read first the values from the encrypted appsettings file, before reading those from the usual non-encrypted appsettings file.
...
This file follows the exact same structure as the [server's configuration files](/versioned_docs/usercube_6.1/usercube/integration-guide/network-configuration/server-configuration/index.md).
