...
```Usercube-Fulfill-ToEasyVistaTicket``` creates tickets in an EasyVista instance: ```Usercube-EasyVistaTicket-UpdateFulfillmentState``` sets the fulfillment state of the corresponding assigned resource types in Usercube for tickets that are closed (```Executed```) or canceled (```Error```).
