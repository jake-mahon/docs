...
| POST | ```https://{host:port}/netwrix/api/v1/activity_records/enum{?format=json}{&count=Number}``` | [Continuation Mark](/versioned_docs/auditor_10.6/auditor/api/postdata/continuationmark.md) |
...
__Step 1 –__ Send a GET request. For example:
...
__Step 3 –__ Continue retrieving Activity Records. Send a POST request containing this Continuation mark to the same endpoint. See the [Continuation Mark](/versioned_docs/auditor_10.6/auditor/api/postdata/continuationmark.md) topic for more information. For example:
...
__Step 5 –__ Continue retrieving Activity Records. Send POST requests containing new Continuation marks until you receive a 200 OK response with no Activity Records inside the ```ActivityRecordList```. It means you reached the end of the Audit Database.
