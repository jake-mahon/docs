...
See the [Configure Integration API Settings](/versioned_docs/auditor_10.6/auditor/api/prerequisites.md#Configure-Integration-API-Settings)[Audit Database](/versioned_docs/auditor_10.6/auditor/admin/settings/auditdatabase.md)topic for additional information.
...
__Step 1 –__ Login to Splunk Web using Splunk Administrator account.

__Step 2 –__ Open the Splunk Apps settings in any of the following ways:

- On the main Explore Splunk Enterprise screen, click the gear icon at the top of the left __Apps__ panel:

![config](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/config.png)

- When on any other screen, you can expand the drop-down list at the top panel and choose Manage Apps:

  ![searchreporting](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/searchreporting.png)

__Step 3 –__ On the __Apps__ screen, click Install app from file:

![installapp](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/installapp.png)
...
![uploadapp](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/uploadapp.png)
...
![searchreportingapp](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/searchreportingapp.png)
...
![configuration](/img/versioned_docs/privilegesecure_4.1/privilegesecure/accessmanagement/enduser/configuration.png)

__Step 2 –__ Configure the account:

1. On the Configuration page, open the Account section.

   ![addaccount](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/addaccount.png)
2. Click __Add__ and populate the fields:
   ...
   - In the Username field insert the user name of the account that will be used to access Netwrix Auditor Integration API. If a domain account is used, make sure to use the _DOMAIN\User_ format.
   - In the Password field insert the account password
3. Click the Add button. The added account should appear in the list:

   ![configurationaccount](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/configurationaccount.png)
...
1. On the Configuration page open the Add-on Settings section:
   ...
   ![configurationaddonsettings](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/configurationaddonsettings.png)
...
3. In the Netwrix Auditor API port field provide the TCP port used by Netwrix Auditor Integration API; by default it is 9699.
   ...
   __NOTE:__ Make sure that your Netwrix Auditor Integration API is configured to use HTTPS protocol.
...
__Step 1 –__ Create a new index to store data from Netwrix Auditor:

1. In Splunk expand the Settings drop-down menu and click on the Indexes option under the DATA section.
2. Press the __New Index__ button to create an index.
3. Provide the new index parameters:

   - Index name — this parameter will be used in the search.
   - App — points where the index configuration is stored; Choosing __Search & Reporting__ is recommended.
   ...
   - By default, Splunk deletes old events when the size of the index exceeds its max value. If you want Splunk to archive them instead specify the Frozen Path.
     ...
     Please refer to the [Managing Indexers and Clusters of Indexers](https://docs.splunk.com/Documentation/Splunk/8.1.0/Indexer/Aboutmanagingindexes) manual for additional details on indexes.

__Step 2 –__ Create a data input:

1. Open Netwrix Auditor add-on for Splunk and go to the __Inputs__ section.

   ![inputs](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/inputs.png)
2. Click Create New Input.

   ![addapi](/img/versioned_docs/auditor_10.6/auditor/addon/splunk/addapi.png)
3. Provide the new data input parameters:
   ...
   - If not following the upgrade procedure, keep the default value for Continuation Mark.
     ...
     __CAUTION:__ This field should never be empty.
