...
Before running or scheduling the add-on, you must define connection details: Auditor Server host, user credentials, etc. Most parameters are optional, the script uses the default values unless parameters are explicitly defined. You can skip or define parameters depending on your execution scenario and security policies. See the[Choose Appropriate Execution Scenario](/versioned_docs/auditor_10.6/auditor/addon/arcsight/deployment.md) topic for additional information.
...
| NetwrixAuditorUserName | Current user credentials | Unless specified, the add-on runs with the current user credentials.  If you want the add-on to use another account to connect to Auditor Server, specify the account name in the _DOMAIN\username_ format.  __NOTE:__ The account must be assigned the __Global reviewer__ role in Netwrix Auditor or be a member of the __Netwrix Auditor Client Users__ group on the computer hosting Auditor Server. |
