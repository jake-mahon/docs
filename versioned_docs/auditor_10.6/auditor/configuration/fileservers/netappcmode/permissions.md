...
- The account requires the following __NTFS__ permissions:
...
__NOTE:__ This article applies to NetApp 8.3.2 and later. You must be a __cluster administrator__ to run the commands below.
...
__NOTE:__ This option is required for auto audit configuration.
...
__NOTE:__ The commands in the first two lines above can be used on NetApp versions 9.11+. In earlier versions, use the following commands:
...
__Step 2 –__ The capabilities must be assigned one by one. To review currently applied capabilities, you can use the following command:
...
_Remember,_ that to be able to add event policy for NetApp, the role you set up for working with ONTAPI must have the following attributes:
