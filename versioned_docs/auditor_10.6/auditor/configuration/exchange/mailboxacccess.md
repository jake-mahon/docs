...
It is recommended to select __Adjust audit settings automatically__ option when setting up Exchange monitoring in Netwrix Auditor. See the [Create a New Monitoring Plan](/versioned_docs/auditor_10.6/auditor/admin/monitoringplans/create.md) topic for additional information.
...
__Step 2 –__ Execute the following command:

```Set-EventLogLevel "MSExchangeIS\9000 Private\Logons" –Level Low```

__Step 3 –__  Navigate to __Start → Run__ and type _"services.msc"_. In the Services snap-in, locate the Microsoft Exchange Information Store service and restart it.
