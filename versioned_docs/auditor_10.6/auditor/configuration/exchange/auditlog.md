...
__To configure Exchange Administrator Audit Logging settings__
...
__Step 3 –__ To reduce server load, you can exclude the cmdlets listed in the table above from Exchange logging. For that:

- On the computer where Netwrix Auditor is installed, browse to the _%Netwrix Auditor Server installation folder%/Active Directory Auditing_ folder, locate the __SetAALExcludedCmdlets.ps1__ PowerShell script file and copy it to Exchange server.

- In __Exchange Management Shell__, run this script using the command line:
