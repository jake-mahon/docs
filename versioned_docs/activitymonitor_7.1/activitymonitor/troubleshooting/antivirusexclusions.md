...
- ```<INSTALL_DIR>``` — Agent installation directory. Default path is ```%ProgramFiles%\Netwrix\Activity Monitor\Agent```. The agent stores binaries and install files in this location.
- ```<DATA_DIR>``` — Agent configuration directory. Default path is ```%ProgramData%\Netwrix\Activity Monitor\Agent```. The agent stores configuration, and debug log files in this location.
- ```<DATA_DIR>\ActivityLogs``` — Default location for collected activity files. If files are stored in a separate location, specify the user-designated directory instead of the default location.
