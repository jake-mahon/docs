...
You can expire, renew or delete a group using GroupID portal. You can change expiry policy of a group and attest an expiring group, if required.
...
1. On the left navigation bar, click __Groups__ and then select the __My Groups__, __My Memberships__, or __My Expiring Groups__ tab.
   ...
   You can also [Directory Search](/versioned_docs/groupid_11.0/groupid/portal/search/search.md) the group you want to expire.
...
- When you manually expire a group with an expiry policy other than ‘Never Expire’, the group expires and is moved it to the [My Expired Groups](/versioned_docs/groupid_11.0/groupid/portal/group/myexpiredgroups.md) page.
- A group without an expiry policy will not expire.
  ...
  In GroupID, you cannot distinguish groups with an expiry policy from group without an expiry policy because groups without an expiry policy are displayed with "Never Expire' set as policy.

If expired groups are not renewed within a specific period (set by the GroupID administrator in the Group Lifecycle policy for the identity store), they are logically deleted when the period ends. See Group life cycle job.

Logically deleted groups are moved to the [My Deleted Groups](/versioned_docs/groupid_11.0/groupid/portal/group/mydeletedgroups.md) page with the “Deleted\_” prefix added to their names.
...
The Group Life Cycle job expires groups according to their respective expiry policies and moves them to the [My Expired Groups](/versioned_docs/groupid_11.0/groupid/portal/group/myexpiredgroups.md) page.

1. [Directory Search](/versioned_docs/groupid_11.0/groupid/portal/search/search.md) the group you want to change the expiry policy for.
2. Select this group on the __Search Results__ page and click __Properties__ on the toolbar.

   The group's [Group Properties](/versioned_docs/groupid_11.0/groupid/portal/group/properties/overview.md) page is displayed with the __General__ tab in view.
...
NOTE: If the GroupID administrator has specified this action for review, your changes will not take effect until verified by an approver. See [Requests](/versioned_docs/groupid_11.0/groupid/portal/request/overview.md).

## Attest an expiring group

See [Attest an Expiring Group](/versioned_docs/groupid_11.0/groupid/portal/group/manage/attestation.md).
