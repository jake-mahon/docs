...
4. On the Data Source page, select the data source that contains the objects to import to the group membership from the directory. Select either of the two options:
   ...
   - __External Data Source__
     ...
     1. On the __Providers__ page, select [Query Designer](/versioned_docs/groupid_11.0/groupid/portal/group/querydesigner/overview.md) to create a query. The query will fetch all those members that match the set criteria.
...
2. Click the __Search Groups__ button; the [Find Dialog Box](/versioned_docs/groupid_11.0/groupid/portal/search/find.md) is displayed, where you can search and select the group(s) whose members you want to import into the membership of this
   group.
3. Click __Next__.
