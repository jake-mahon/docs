...
NOTE: When Smart Groups and Dynasties reside in a target OU, GroupID does not process
them.
...
You do not need to reapply a policy when a new group is created in the policy’s target OU through GroupID. In this case, the policy is automatically applied.
