...
In Admin Center, you can view the history data that the GroupID history-tracking function collects for changes made to the configurations, workflows, and security roles in an identity store.
...
3. Click __History__ under __Settings__ in the left pane.
   ...
   Click a history item to view its details. See the [History Item Details](/versioned_docs/groupid_11.0/groupid/admincenter/identitystore/history/details.md) topic.

## Filter History Data

GroupID provides filters to limit the history results displayed on the __Identity Store History__ page. You can define a criterion to filter history data by action, machine name, new value, old value, and more.
...
GroupID enables a user to add notes to history actions that he or she performed. A note may explain the reason for an action, such as why he or she changed the security type for a group. Only the user who added a note can update it. Other users can
