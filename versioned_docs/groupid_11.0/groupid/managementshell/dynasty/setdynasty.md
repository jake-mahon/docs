...
The __Set-Dynasty__ commandlet lets you to modify a Dynasty or its children in Directory.

GroupID maintains a history for this commandlet, which you can view against the __History__ node in the left panel.
...
The following command modifies the __Departmental__ Dynasty by changing the __Group-by__ attributes list using the credentials of current user logged-on to the identity store.
...
The following command modifies the search criteria for the Managerial Dynasty to retrieve all mail-enabled objects who are the member of the __Training__ group.
...
The following command adds three group-by levels to an __Organizational Dynasty__.
...
The following command modifies additional owners, __Includes__ and __Excludes__ lists and replaces Search Scope of a Managerial Dynasty.
...
The following command clears the groups specified in the __Includes__ list of a Managerial Dynasty.
...
- [Dynasty Commands](/versioned_docs/groupid_11.0/groupid/managementshell/dynasty/overview.md)
