...
The commandlet __Set-IdentityStore__ modifies the identity store settings and configurations.

NOTE: Many parameters of this cmdlet require the user to specify schema attribute names. You can use __Get-SchemaAttributes__ commandlet to retrieve a list of attributes available for an identity store.
...
‘PasswordExceptions’ parameter accepts 2-Length arrays having at first index the operator and at second index the string. Allowed operators contain: ‘equals’; ‘startswith’; ‘endswith’; ‘contains’; and ‘regexp’.
