...
__CAUTION:__ Netwrix Threat Manager cannot be installed on the same server as Netwrix Threat Manager Reporting Module.
...
The following permissions are required for the credentials used by Netwrix Threat Manager Reporting Module for Active Directory Sync:
...
Netwrix Threat Manager Reporting Module is a web service that can be accessed locally or remotely if the server’s firewall permits. The supported browsers are:
