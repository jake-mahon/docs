...
![Policy window - FSMO Role Monitoring Event Type](/img/versioned_docs/threatprevention_7.4/threatprevention/admin/policies/eventtype/fsmorolemonitoring.png)
...
![Policy window - FSMO Operations filter](/img/versioned_docs/threatprevention_7.4/threatprevention/admin/policies/eventtype/fsmooperations.png)

Select the __All__ checkbox to generate events when a domain controller acquires or relinquishes a FSMO role.

On selecting it, both the Acquire and Relinquish checkboxes are automatically checked.

- Check the __Acquire__ checkbox to generate events when a domain controller acquires a FSMO role.
- Check the __Relinquish__ checkbox to generate events when a domain controller relinquishes a FSMO role.

## FSMO Roles Filter

On the FSMO Roles filter, specific FSMO role assignments for a domain controller are selected to be monitored.

![Policy window - FSMO Roles filter](/img/versioned_docs/threatprevention_7.4/threatprevention/admin/policies/eventtype/fsmoroles.png)

Check the __All__ checkbox to include all FSMO roles for monitoring or select specific roles:
...
See the Netwrix [Understanding the Five FSMO Roles and Their Impact on Active Directory](https://blog.netwrix.com/2021/11/30/what-are-fsmo-roles-active-directory/) article for additional information.
