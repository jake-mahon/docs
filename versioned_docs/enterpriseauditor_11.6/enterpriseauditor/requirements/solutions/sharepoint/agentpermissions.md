...
In addition to having the Sensitive Data Discovery Add-on be installed on the Enterprise Auditor Console server, The following is required to run Sensitive Data Discovery scans:

- Sensitive Data Discovery Add-On, 64-bit version, installed on the Enterprise Auditor SharePoint Agent server
...
- Membership in the local Administrator group on the on server where the Enterprise Auditor SharePoint Agent is installed
...
- ```'GRANT EXECUTE'``` permissions on the following stored procedures in the SharePoint Configuration database:
  ...
  - ```proc_ReturnWebFeatures```
    ...
    __NOTE:__ The above four stored procedures would already have the correct permissions if Web Application scoping is desired.
