...
The AD\_GroupPolicy Job has been configured to inherit its host from the __4.Group Policy__ > __Settings__ > __Host List Assignment__ node. It is set to target the __Default domain controller__ host list, which is the domain in which the Enterprise Auditor Console server resides.
