...
The script then takes the value of ```REMAINDER```, which is in seconds, and converts it to days, hours, minutes, and seconds. These values are then recorded in the Query object so that Enterprise Auditor can store this data.

__NOTE:__ In this task, the hours, minutes, and seconds properties were specified manually using the task dialog. See the [Script Properties](/versioned_docs/enterpriseauditor_11.6/enterpriseauditor/admin/datacollector/script/properties.md) topic for additional information.
