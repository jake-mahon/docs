...
- Manual – The FSAA Data Collector will expect all certificates to be valid and in their respective certificate stores prior to running a scan
  ...
  - To create and store certificates, the ```FSAACertificateManager.exe``` tool can be used. This application was created to simplify the process of creating certificates and will store the certificates in the location that the FSAA Data Collector and Applet server expect them to be stored. See the [FSAA Manual Certificate Configuration](/versioned_docs/enterpriseauditor_11.6/enterpriseauditor/admin/datacollector/fsaa/manualcertificate.md) topic for additional information.
