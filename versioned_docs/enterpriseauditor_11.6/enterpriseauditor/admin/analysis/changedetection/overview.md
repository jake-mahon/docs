...
This module compares values collected for two different query instances. Therefore, as change detection depends on the existence of a __JobRunTimeKey__, history must be enabled and data collected at least twice to produce the desired results. Configure History settings under the job’s __Settings__ > __History__ node. See the [History](/versioned_docs/enterpriseauditor_11.6/enterpriseauditor/admin/settings/history.md) topic for additional information.
