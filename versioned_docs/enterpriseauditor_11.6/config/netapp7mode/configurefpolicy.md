...
- The Activity Monitor must register with the NetApp device as an FPolicy server. By default, it looks for a policy named ```StealthAUDIT```. See the [Customize FPolicy Policy Name](/versioned_docs/enterpriseauditor_11.6/config/netapp7mode/customizefpolicy.md) section for information on using a different policy name.
