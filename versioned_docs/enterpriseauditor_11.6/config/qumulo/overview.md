...
Netwrix Enterprise Auditor can execute Access Auditing (FSAA) and/or Sensitive Data Discovery Auditing scans on Qumulo devices. The Netwrix Activity Monitor can be configured to monitor activity on Qumulo devices and make the event data available for Enterprise Auditor Activity Auditing (FSAC) scans.
...
Netwrix Activity Monitor requires an account with the Observers role to monitor a Qumulo cluster. See the [Qumulo Activity Auditing Configuration](/versioned_docs/enterpriseauditor_11.6/config/qumulo/activity.md) topic for instructions.
