...
During a Resource Review, the Access Information Center can provide the owner of a resource with recommendations based on user activity (providing Netwrix Enterprise Auditor is collecting activity data). By default, these recommendations are based on a 90-day sample of activity. However, this configuration can be changed by editing the ```AccessInformationCenter.Service.exe.config``` file in the Access Information Center installation directory:
...
__Step 2 –__ Locate the ```ActivityDays``` parameter. By default, the value will be set to 90 days. Change the value to the desired number of Activity Days for the sample. For example, the parameter would look like this if set to 120 days:
