...
__CAUTION:__ If the password for logging in to the Server Manager on the database has been saved, it is essential that it is noted down or temporarily saved elsewhere before an update!
...
Further information on the installation wizard can be found in the section [Installation Server Manager](/versioned_docs/passwordsecure_9.1/passwordsecure/installation/installation_server_manager/installation_server_manager.md).
...
The application server must firstly be updated. A new Web Application ([Installation Web Application](/versioned_docs/passwordsecure_9.1/passwordsecure/installation/installation_web_application/installation_web_application.md) is then created according to the instructions for the web server being used. The document directory on the web server should now be completely emptied. The Web Application is then unzipped and copied to the document directory on the corresponding web server.
...
NOTE: If the Web Application is used, the module: ```proxy_wstunnel``` must be installed when using Apache. With IIS the ```WebSocket Protocol``` becomes necessary. Further information can be found in the chapter [Webserver](/versioned_docs/passwordsecure_9.1/passwordsecure/installation/requirements/webserver.md). This applies to version 8.5.0.14896 or newer.
