...
function TopicItem({ topic, showCategory = false, showVotes = false, showSolved = true }) {
  ...
  return (
    <Link to={`https://community.netwrix.com/t/${topic.slug}/${topic.id}`} className={styles.topicItemCard}>
      <div className={styles.topicMeta}>
        {showSolved && topic.has_accepted_answer && <span className={styles.solvedBadge}>✅ Solved</span>}
  ...
  useEffect(() => {
    async function fetchCommunityData() {
      try {
        ...
        const announcementResponse = await fetch('https://community.netwrix.com/tag/announcement.json')
        const announcementData = await announcementResponse.json()
        
        // Fetch solved topics using search API
        const solvedResponse = await fetch('https://community.netwrix.com/search.json?q=status%3Asolved%20order%3Aviews')
        const solvedData = await solvedResponse.json()
        
        // Fetch top voted ideas using quarterly top ideas endpoint
        const topResponse = await fetch('https://community.netwrix.com/tag/ideas/l/top.json?period=quarterly')
      ...
      <div className="container">
        <div className={styles.sectionHeader}>
          ...
          <p className={styles.sectionSubtitle}>
            Connect with other Netwrix users, get help, share ideas, and stay updated with the latest announcements
          </p>
        </div>
        ...
        <div className={styles.communityGrid}>
          <CommunityCard title="Recently Solved" icon="✅">
            ...
            <Link to="https://community.netwrix.com/search?q=status:solved" className={styles.viewAllLink}>
          ...
          <CommunityCard title="Top Voted Ideas" icon="💡">
            ...
            <Link to="https://community.netwrix.com/tag/ideas/l/top?period=quarterly" className={styles.viewAllLink}>
              View popular ideas →
            </Link>
          </CommunityCard>
          ...
          <CommunityCard title="Product Updates" icon="📢">
            {announcements.length > 0 ? (
              announcements.map((topic, idx) => (
                <TopicItem key={idx} topic={topic} />
              ))
            ) : (
              <p className={styles.emptyState}>No recent announcements</p>
            )}
            <Link to="https://community.netwrix.com/tag/announcement" className={styles.viewAllLink}>
              View all updates →
            </Link>
          </CommunityCard>
        </div>
