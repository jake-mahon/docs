...
Check the [**`docusaurus.config.js` API reference**](/reference-docs/docusaurus/api/docusaurus.config.js.mdx) for an exhaustive list of options.
...
It is recommended to check the [deployment docs](/reference-docs/docusaurus/deployment.mdx) for more information.

### Theme, plugin, and preset configurations {#theme-plugin-and-preset-configurations}

List the [themes](./using-plugins.mdx#using-themes))), [plugins](/reference-docs/docusaurus/using-plugins.mdx), and [presets](./using-plugins.mdx#using-presets))) for your site in the `themes`, `plugins`, and `presets` fields, respectively. These are typically npm packages:
...
For further help configuring themes, plugins, and presets, see [Using Plugins](/reference-docs/docusaurus/using-plugins.mdx).
