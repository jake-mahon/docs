...
![My image alternative text](/reference-docs/docusaurus/assets/docusaurus-asset-example-banner.png)

</BrowserWindow>

:::note

If you are using [@docusaurus/plugin-ideal-image](/reference-docs/docusaurus/api/plugins/plugin-ideal-image.mdx), you need to use the dedicated image component, as documented.
...
[Download this docx using Markdown](/reference-docs/docusaurus/assets/docusaurus-asset-example.docx)
...
import DocusaurusSvg from '@site/img/docusaurus.svg';
...
If a Markdown link or image has an absolute path, the path will be seen as a file path and will be resolved from the static directories. For example, if you have configured [static directories](/reference-docs/docusaurus/static-assets.mdx) to be `['public', 'static']`, then for the following image:
