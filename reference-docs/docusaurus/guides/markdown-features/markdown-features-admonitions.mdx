...
You can customize how each individual admonition type is rendered through [swizzling](/reference-docs/docusaurus/swizzling.mdx). You can often achieve your goal through a simple wrapper. For example, in the follow example, we swap out the icon for `info` admonitions only.

```jsx title="src/theme/Admonition.js"
import React from 'react';
import Admonition from '@theme-original/Admonition';
import MyCustomNoteIcon from '@site/img/info.svg';
...
Admonitions are implemented with a [Remark plugin](/reference-docs/docusaurus/guides/markdown-features/markdown-features-plugins.mdx). The plugin is designed to be configurable. To customize the Remark plugin for a specific content plugin (docs, blog, pages), pass the options through the `admonitions` key.
