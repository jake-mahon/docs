...
When [localizing your site](/reference-docs/docusaurus/i18n/i18n-introduction.mdx), the `<Translate/>` component will allow providing **translation support to React components**, such as your homepage. The `<Translate>` component supports [interpolation](#interpolate).
...
React hook to access Docusaurus Context. The context contains the `siteConfig` object from [docusaurus.config.js](/reference-docs/docusaurus/api/docusaurus.config.js.mdx) and some additional site metadata.
...
Prefer a `require()` call for [assets](/reference-docs/docusaurus/guides/markdown-features/markdown-features-assets.mdx):

```jsx
<img src={require('@site/img/myImage.png').default} />
```
