...
Refer to the Getting Started [**Configuration**](/reference-docs/docusaurus/configuration.mdx) for examples.
...
With a [TypeScript](/reference-docs/docusaurus/typescript-support.mdx) Docusaurus codebase your config file may be called `docusaurus.config.ts`. The syntax is broadly identical to the `js` config file with the addition of types. You can see an example on the [Docusaurus Website](https://github.com/facebook/docusaurus/blob/main/website/docusaurus.config.ts) itself.
...
Refer to the [deployment guide](/reference-docs/docusaurus/deployment.mdx) and [slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide) to choose the appropriate setting.
...
The i18n configuration object to [localize your site](/reference-docs/docusaurus/i18n/i18n-introduction.mdx).
...
- `v4`: Permits to opt-in for upcoming Docusaurus v4 breaking changes and features, to prepare your site in advance for this new version. Use `true` as a shorthand to enable all the flags.
  ...
  - [`useCssCascadeLayers`](https://github.com/facebook/docusaurus/pull/11142): This enables the [Docusaurus CSS Cascade Layers plugin](/reference-docs/docusaurus/api/plugins/plugin-css-cascade-layers.mdx) with pre-configured layers that we plan to apply by default for Docusaurus v4.
...
The [theme configuration](/reference-docs/docusaurus/api/themes/theme-configuration.mdx) object to customize your site UI like navbar and footer.
...
See [plugin method references](/reference-docs/docusaurus/api/plugin-methods/README.mdx) for the shape of a `PluginModule`.
