...
Docusaurus adds some metadata out-of-the-box. For example, if you have configured [i18n](/reference-docs/docusaurus/i18n/i18n-introduction.mdx), you will get a [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions) alternate link.
...
Similar to [global metadata](#global-metadata), Docusaurus also allows for the addition of meta-information to individual pages. Follow [this guide](/reference-docs/docusaurus/guides/markdown-features/markdown-features-head-metadata.mdx) for configuring the `<head>` tag. In short:
...
```md
![Docusaurus banner](/reference-docs/docusaurus/assets/docusaurus-asset-example-banner.png) 'Image title')
```
...
Docusaurus blogs support [rich search results](https://search.google.com/test/rich-results) out-of-the-box to get maximum search engine experience. The information is created depending on your meta information in blog/global configuration. In order to get the benefits of the rich search information, fill in the information about the post's publish date, authors, and image, etc. Read more about the meta-information [here](/reference-docs/docusaurus/blog.mdx).

## Robots file {#robots-file}

A `robots.txt` file regulates search engines' behavior about which should be displayed and which shouldn't. You can provide it as [static asset](/reference-docs/docusaurus/static-assets.mdx). The following would allow access to all sub-pages from all requests:
...
To prevent your whole Docusaurus site from being indexed, use the [`noIndex`](./api/docusaurus.config.js.mdx#noIndex))) site config. Some [hosting providers](/reference-docs/docusaurus/deployment.mdx) may also let you configure a `X-Robots-Tag: noindex` HTTP header (GitHub Pages does not support this).
