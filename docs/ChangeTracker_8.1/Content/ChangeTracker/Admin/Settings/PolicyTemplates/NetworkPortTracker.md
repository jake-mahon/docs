...
* **Self-Scan** - Local Agent scans the local host using the local netstat. The same agent can run a full template including all other Tracker types.
* **Remote Scan** – A remote Agent scans the host/device. In this mode, the remote device is configured as a Proxied device for the Agent being used, and therefore for other Trackers to be run against the Proxied Device, credentials will be required e.g. Cisco Firewall running config can also be tracked if valid credentials are assigned.
* **Network Scan** – A Agent can be used to scan a range of devices. In this instance the ‘device’ is defined as ‘The Network’ or ‘New York Office LAN’, with the address range defined as ‘192.168.1.1-255, or 192.168.1.18 192.168.1.19 192.168.1.20-100’, i.e. individually specified addresses space separated, ranges defined using a -

The **NMAP** package must be installed to a default program file location (Windows: Program Files(x86)\Nmap), Linux: /usr/bin/)

![PolicyTemplatesNetworkPortTrackerDiagram](../../../../../../../static/images/ChangeTracker_8.1/Content/Resources/Images/ChangeTracker/PolicyTemplatesNetworkPortTrackerDiagram.png "PolicyTemplatesNetworkPortTrackerDiagram")
...
![PolicyTemplatesNetworkPortTrackerTab](../../../../../../../static/images/ChangeTracker_8.1/Content/Resources/Images/ChangeTracker/PolicyTemplatesNetworkPortTrackerTab.png "PolicyTemplatesNetworkPortTrackerTab")