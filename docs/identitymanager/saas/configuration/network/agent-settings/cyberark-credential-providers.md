...
Identity Manager often needs to connect to [Connectors](/docs/identitymanager/saas/connectors/index.md) with
credentials that need protection. See the [Connectors](/docs/identitymanager/saas/connectors/index.md) topic for
...
Any [Connectors](/docs/identitymanager/saas/connectors/index.md) using one of these attributes as key can retrieve the
associated value from CyberArk.

> For example,
> [Active Directory](/docs/identitymanager/saas/connectors/connector-reference/active-directory.md) can
...
Netwrix Identity Manager (formerly Usercube) recommends customizing the account's name because it will be used in [
              ...
              Connection
              ](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) to retrieve this account from the vault.
...
| Password optional | **Type** String **Description** [PKCS #12](https://en.wikipedia.org/wiki/PKCS_12) archive password. **Info:** storing a `.pfx` file's password in plain text in a production environment is strongly discouraged. It should always be encrypted using the [ Usercube-Protect-CertificatePassword ](/docs/identitymanager/saas/automation/executables/protect-certificate-password.md) tool. |
