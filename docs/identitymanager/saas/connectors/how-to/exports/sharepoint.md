...
This guide shows how to set up a [SharePoint](/docs/identitymanager/saas/connectors/connector-reference/sharepoint.md)
...
This guide focuses on the [ Architecture ](/docs/identitymanager/saas/architecture/index.md) method. Remember that
...
[ Upward Data Synchronization ](/docs/identitymanager/saas/synchronization/upward-data-sync.md) task.
...
[Toolkit for XML Configuration](/docs/identitymanager/saas/toolkit/index.md) topic for additional information.
...
the[ Create a Working Directory ](/docs/identitymanager/saas/installation/production-setup/working-directory.md)
...
- In the `SharePoint` directory, create a `SharePoint Connector.xml` file.
  ...
  [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md).

- Use the [ Connector ](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md)element to
...
[Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md). See
...
The [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) should match as closely as possible the structure
of the SharePoint data relevant for Identity Manager. It is designed by analyzing the SharePoint
data structure, and describing said data with [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) and an
...
[Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) that best serves the
[ Assignment Policy ](/docs/identitymanager/saas/role-management/role-model-rules.md) needs. It will be refined
...
The [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) for the SharePoint connector is written in the
applicative configuration. See the [Toolkit for XML Configuration](/docs/identitymanager/saas/toolkit/index.md) topic
...
Declaring an [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) is achieved with the `<EntityType>` tag
...
- **DisplayName_Li, i ? [1..16]** are used in the UI to identify this
  [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) for the end-user. **DisplayName_L1** is the name of
...
The SharePoint object attributes are modeled by [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md), with the ```<Property>``` tags declared as children of the ```<EntityType>```.

Remember that there are several kinds of properties: scalar and navigation. Scalar properties can be defined to represent scalar attributes such as ```city```, ```country``` or ```companyName```. represent associations such as group memberships. See the [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) topic for additional information.
...
- __TargetColumnIndex__ defines in which column of the resource table the property is stored. See more details about Target Column Index. See the [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) topic for additional information.
...
[Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md) types are associated through their navigation
properties with
[ Entity Association ](/docs/identitymanager/saas/identitymanager/integration-guide/toolkit/xml-configuration/metadata/entityassociation/index.md)
elements.
...
Entity Association Mapping
](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md), and Entity Type Mapping elements serve this purpose.

#### Entity type mapping

The [
Entity Type Mapping
](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) element maps the scalar properties from the CSV source file to an entity type.
...
The [
Entity Association Mapping
](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) element maps the navigation properties used in [
Entity Association
...
An [
Entity Association Mapping
](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) element refers to an [
Entity Association
...
Entity Type Mapping
](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md), it maps column values from a CSV source file to an EntityType property.
...
A [ Menu Item ](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) can be added to
...
The
[Display Entity Type](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md)
...
The scalar properties require no configuration: they are automatically displayed. The only information that the [Display Entity Type](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) adds here, is that the property ```BasicCollection``` is a navigation property. An eye icon will be displayed to take you directly to the matching page.

#### Display table

[Display Table](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) elements describe how a list of resources should be displayed.

The [Display Table](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) contains a list of display table column elements that identify which properties should be included in the list display.
...
An `InternalDisplayName` can also be declared as an [Entity Model](/docs/identitymanager/saas/identity-management/entity-model.md).
...
The [Access Control Rule](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md) and [Access Control Rule](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md) elements define [
AccessControlPermission
](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md) for end-user profiles to read and write the connector's data (such as resources of a given entity type). It is used by the UI when displaying data such as resources and available roles.
...
Notice the __Agent__ attribute that contains the name of the Agent which executes the Job. This attribute is mandatory for a Job containing Tasks executed agent-side, even if a unique local Agent exists. See the [appsettings.agent](/docs/identitymanager/saas/configuration/network/agent-settings/appsettings.md) topic for additional information.

### Components

The[
Upward Data Synchronization
](/docs/identitymanager/saas/synchronization/upward-data-sync.md)job includes three steps:

- Export
- Prepare-Synchro
- Synchro

These three steps are all contained in a which allows the generation of the Incremental Synchronization configuration. See the [Create Connector Synchro Incremental](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) topic for additional information.
...
the[ Usercube-Invoke-Job ](/docs/identitymanager/saas/automation/executables/invoke-job.md) uses:

- a [ Profile ](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md) associated with
...
Profile
](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md) to be used during the Synchronization jobs which will be different from the one used during the Provisioning job. This contributes to separating access rights.
...
Usercube-Invoke-Job
](/docs/identitymanager/saas/automation/executables/invoke-job.md) tool, the profile linked to these tasks and used by the tool should be authorized to execute said tasks.
...
](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md) and the [
...
the[ Job Execution Access Control Rules ](/docs/identitymanager/saas/configuration/xml-configuration/scaffoldings.md)
scaffolding.
...
An Agent's a[
Profile
](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md)is associated with a ```ClientId/Secret``` pair used by the Agent to authenticate to the Server.

Usable ```ClientId/Secret``` pairs are written to the database from the xml configuration using the [
OpenIdClient
](/docs/identitymanager/saas/configuration/xml-configuration/access-control.md) xml element.
...
](/docs/identitymanager/saas/automation/executables/new-openid-secret.md) tool.
...
The ```ClientId/Secret``` pairs that the Agent may use are written to the Agent's [appsettings.agent](/docs/identitymanager/saas/configuration/network/agent-settings/appsettings.md) technical configuration set.
...
Usercube-Invoke-Job
](/docs/identitymanager/saas/automation/executables/invoke-job.md) tool.
