...
[Usercube's resource repository](/docs/identitymanager/6.1/identitymanager/core-concepts/entity-model.md)
...
[set of Tasks](/docs/identitymanager/6.1/identitymanager/automation/jobs-tasks/index.md).

A _Sync Up_ is planned for every managed system that interact with Usercube.

A _Sync Up_ is associated with a
[**Connector**](/docs/identitymanager/6.1/identitymanager/connectors-integration/index.md).
...
are written to the
[export directory](/docs/identitymanager/6.1/identitymanager/installation-setup/agent-configuration.md)
...
Usercube's
[**Connectors**](/docs/identitymanager/6.1/identitymanager/connectors-integration/index.md)
...
[Connectors](/docs/identitymanager/6.1/identitymanager/connectors-integration/index.md) are
...
[PowerShell script](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md),
...
[PowerShell script](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md)
or a
[SQL command](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md).
...
[connectors section](/docs/identitymanager/6.1/identitymanager/connectors-integration/index.md).
...
[Usercube-Export-ActiveDirectory task](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md)
within a Job.
...
![Active Directory Export Example](/img/versioned_docs/identitymanager_6.1/identitymanager/integration-guide/synchronization/upward-data-sync/ad_export_example.webp)
...
The aim of the _Sync Up_ is to load managed systems' data into the resource repository. As such, it requires Usercube to translate data from the managed system format (or, more accurately, the _export task_'s output format) into the resource repository format, that is, the [Entity Model](/docs/identitymanager/6.1/identitymanager/core-concepts/entity-model.md).

The translation rules are described in the [applicative configuration](/docs/identitymanager/6.1/identitymanager/tools-utilities/toolkit/index.md) by [```<EntityTypeMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) and [```<EntityAssociationMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) elements.

[```<EntityTypeMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) elements map the resources _CSV source files_ columns to [Entity Types](/docs/identitymanager/6.1/identitymanager/core-concepts/entity-model.md#entity-types) properties. Each mapping also identifies one column as the _primary key_ for this Entity Type. The _primary key_ is used to uniquely identify a resource in the _Sync Up_ process. It's mandatory to be able to perform _incremental__Sync Up_, as it allows to identify a resource on which an _update_ or a _delete_ has to be performed.

[```<EntityAssociationMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) elements translate the _CSV source files_ into [Entity Associations](/docs/identitymanager/6.1/identitymanager/core-concepts/entity-model.md). They describe rules identifying associations between resources loaded thanks to the [```<EntityTypeMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md).
...
1. Removing columns that are not used in [```<EntityTypeMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) or [```<EntityAssociationMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md)
2. Removing entries that have a null primary key
3. Removing duplicates
4. Sorting entries according to the primary key

The result of the _Prepare-Synchronization_ is stored in the [_export directory_](/docs/identitymanager/6.1/identitymanager/installation-setup/agent-configuration.md) as three files:

For every entity type of the relevant _Connector_ involved in an [```<EntityTypeMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) or an [```<EntityAssociationMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) , a ```.sorted.csv``` file is generated, containing the final, cleaned, sorted result.
...
The ```.sorted``` file (the original cleaned export file, not the changes) is stored in the ```previous``` folder inside the [_export directory_](/docs/identitymanager/6.1/identitymanager/installation-setup/agent-configuration.md). It will be used as a reference for the next _incremental__prepare-synchronization_ to compute the changes if needed.
...
- [PrepareSynchronizationTask](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md) is the standard _prepare-synchronization_ task.
- [PrepareSynchronization Change Task](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md) is used to process data source files containing changes.
- [PrepareSynchronizationActiveDirectoryTask](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md) is specialized for Active Directory. This task handles Active Directory _incremental_ prepare-synchronization by using Active Directory _cookies_.
...
Before writing to the Usercube's database, the _Server_ uses [```<EntityTypeMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) and [```<EntityAssociationMapping>```](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) to translate _CSV source files_ into _Entity Model compliant_ resources and resolve association links.
...
- Mono-valued associations ( [target column index](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/index.md) 128 to 137 included ) are stored in UR_Resources as well,
- Multi-valued associations ( [target column index](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/index.md) null or -1 or 0 to 127 included ) are stored in the UR_ResourceLinks table.
...
_Complete synchronization_ replaces entirely the database resources. That means that all resource, for that [Connector](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md), that are in the database but not in the _CSV source files_ will be deleted. That means no change made to the database from outside of the connectors or the UI are persistent.
...
- [SynchronizeTask](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md) is the standard _synchronization_ task.
- [SynchronizeChangesTask](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md) is used to handle changes together with [PrepareSynchronization Change Task](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md).
- [SynchronizeActiveDirectoryTask](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md) is specialized for Active Directory. To be used with [PrepareSynchronizationActiveDirectoryTask](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/jobs-config.md).

### Example

This example illustrates the _complete_ loading of Active Directory ```.sorted``` files into Usercube database.

![Active Directory Synchronization Example](/img/versioned_docs/identitymanager_6.1/identitymanager/integration-guide/synchronization/upward-data-sync/ad_synchro_example.webp)
...
_Incremental_ mode also offers another optimization that will be described in the [evaluate policy section](/docs/identitymanager/6.1/identitymanager/access-control-security/role-assignment/assignment-policies.md). Trade-offs of that optimization can also be counterbalanced by running a daily _complete_ synchronization.
...
Thresholds can be configured by the user in the applicative configuration and be specific to a [Connector](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md), an [EntityTypeMapping](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md) and/or an [EntityAssociationMapping](/docs/identitymanager/6.1/identitymanager/configuration-reference/xml-configuration/connector-config.md). They are expressed as number of lines (ex: ```MaximumInsertedLines```) or as a rate (ex: ```MaxPercentageDeletedLines```).
