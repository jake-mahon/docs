...
__Step 2 –__ Select __Exchange__ source type and in the properties window specify the necessary settings.

__Step 3 –__ Remember to click the "wrench" icon next to __Settings__ in the bottom-left corner to display all settings.
...
| Option | Description |
| --- | --- |
| Email Address / Password | Administrator account that has been assigned the right of Impersonation as well as the Discovery Management role.  See [Configure Microsoft Exchange for Crawling and Classification](/docs/product_docs/dataclassification/ndc/configuration/configinfrastructure/exchange.md) for details on the rights assignment. |
