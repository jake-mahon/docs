...
Approvers are populated from the Change/Approval Policy for the Change Request. Approval notifications are sent when the Change Request owner advances the status to __Pending Approval__. Approvers must be [licensed](../installing_strongpoint/license_manager.md) Platform Governance for NetSuite users and have the correct [role permissions](../installing_strongpoint/setting_permissions.md) if they are using a custom (non-Strongpoint) role.

![change_request_approving_change_request](../../../static/img/product_docs/strongpointfornetsuite/change_management/change_request_approving_change_request.webp)
...
3. Change Request owner [Completes and Validates the Change Request](completing_validating_change_request.md).
