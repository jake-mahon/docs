...
This walkthrough is one example based on our test account. You must [install and configure](zendesk_integration.md) the Zendesk integration, prior to using this walkthrough.
...
If you do not see the app, make sure the Apps are toggled on using the Apps icon ![Zendesk Apps visibility icon](../../../static/img/product_docs/strongpointfornetsuite/integrations/zendesk_apps_icon.webp) and verify you are a member of a group or role [authorized to access the app](zendesk_integration.md).
...
Once you have made a change, the __Request Approval__ button is available if you are a member of a group with the [Set up Approvals](zendesk_integration.md#set-up-approvals) permission.
...
There is an options menu available for each added customization. Options include __Remove__ and __ERD__. Selecting __ERD__ launches the [Entity Relationship Diagram](../customizations/using_erd.md) for the customization.
...
5. Enter the Script ID __custentity_fmt_cust_credit_on_hold__ in __Add Customizations__ and click __+__.
