...
How to create a new [Connector](../../../integration-guide/toolkit/xml-configuration/connectors/connector/index) using the provided SaaS agent. See the [Architecture](../../../introduction-guide/architecture/index "Architecture") topic for additional information.
...
* a [Connector](../../../integration-guide/toolkit/xml-configuration/connectors/connector/index) is linked to an agent which acts as the go-between for Identity Manager's server and the managed system;

  > Our `AD` connector uses the provided SaaS agent.
* a [Connection](../../../integration-guide/toolkit/xml-configuration/connectors/connection/index) describes the technology used that enables data to flow back and forth between Identity Manager and the managed system;
...
You can find standard connections dedicated to one application (AD, Microsoft Entra ID, etc.), and generic connections to communicate with any application (CSV, Powershell, RobotFramework, SQL, etc.).
  :::
* the shape of the extracted managed system's data is modeled by [Entity Type](../../../integration-guide/toolkit/xml-configuration/metadata/entitytype/index) (we will use the term resource to refer to an entity type that has been instantiated);
...
* the intent of resources within the managed system is made clear by categorizing resources into [Create a Resource Type](../categorization/resource-type-creation/index). See the [Categorize Resources](../categorization/index) topic for additional information.
...
See the [Install the Development Environment](../development-environment-installation/index) [Create the Workforce Repository](../initial-identities-loading/index), and [Configure a User Profile](../user-profile-configuration/index "Configure a User Profile") topics for additional information.

## Create a Target Connector

For one managed system, create a connector by proceeding as follows:

1. Outside Identity Manager, [Model the Data](connector-modeling/index).
2. [Create the Connector](connector-declaration/index) for said managed system.
3. Enable the technical transfer of data by creating and configuring [Create a Connection](connection-creation/index).
4. Set up [Create an Entity Type](entity-type-creation/index) to represent the data model decided upon in step 1.
...
![Jobs Results Dashboard](../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/Synchro_Dashboard_V522.png)

## Next Steps

Once the connector has been created, you can start to[Synchronize Data](../synchronization/index).