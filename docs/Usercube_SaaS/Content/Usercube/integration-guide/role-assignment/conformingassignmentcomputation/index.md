...
The [Compute Role Model Task](../../toolkit/xml-configuration/jobs/tasks/server/computerolemodeltask/index "ComputeRoleModelTask") is able to compute, for a given identity, the appropriate assignments.
...
> **FAQ**: During onboarding, moving, offboarding, how can we make sure that an identity has the appropriate assignments?   
...
> **FAQ**: Are employees working on tasks that need this assignment? Are they senior enough to have that responsibility?
...
2. Find the resource in the target application that should be updated to satisfy the assignment requirement. It is often an account. That's the **correlation**.
3. Compute the value of the data that should be updated in the target resource. That's **provisioning rules**.
