...
1. The [Application Settings](appsettings/index) set is written either to the Agent's working directory appsettings.json file or as environment variables. See the [Architecture](../../architecture/index) topic for additional information.
2. The [appsettings.agent](appsettings-agent/index) set is written as environment variables or to the appsettings.agent.json files from the Agent's working directory.
3. There are two additional files involved in the *Agent*'s configuration to protect sensitive data: appsettings.encrypted. agent. json and appsettings.cyberark.agent.json. See the [RSA Encryption](rsa-encryption/index) and [CyberArk's AAM Credential Providers](cyberark-application-access-manager-credential-providers/index)topics for additional information.
...
* An [Azure Key Vault](azure-key-vault/index) safe.
