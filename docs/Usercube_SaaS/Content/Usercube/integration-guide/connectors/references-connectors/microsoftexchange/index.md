...
This page is about [Microsoft Exchange](../../references-packages/microsoft-exchange/index "Microsoft Exchange").

![Package: Server/Microsoft Exchange](../../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/Packages_exchange_V603.png)
...
| Name | Details |
| --- | --- |
| Server required | **Type**  String  **Description** Address of the Exchange Server used by the remote PowerShell: `http:///PowerShell/` where `` is the fully qualified domain name of the Exchange server, like `mailbox01.contoso.com`. |
...
* `_mailboxes.csv` with the following columns:
...
* `_databases.csv` with the following columns:
...
The CSV files are stored in the [Application Settings](../../../network-configuration/agent-configuration/appsettings/index "Application Settings")Export Output, and the cookie file in the Export Cookies folder.
...
As it works via a PowerShell script. See the [PowerShellProv](../powershellprov/index "PowerShellProv") topic for additional information.

Identity Manager's PowerShell script can be found in the SDK in `Usercube.Demo/Scripts/Fulfill-Exchange.ps1`.

See the [PowerShellProv](../powershellprov/index "PowerShellProv") topic for additional information.
...
* A [Connection](../../../toolkit/xml-configuration/connectors/connection/index "Connection")able to store Microsoft Exchange's `Server`.
