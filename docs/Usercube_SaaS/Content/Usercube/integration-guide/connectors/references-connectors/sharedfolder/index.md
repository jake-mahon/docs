...
This page is about [Shared Folders](../../references-packages/shared-folders/index).

![Package: Storage/Shared Folders](../../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/Packages_sharedfolders_V603.png)
...
* **Log on as a batch job** in the local group policy, when the connector's authentication mode is batch.

  ![SharedFolder - Permission for Batch Authentication](../../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/SharedFolder_permission.png)
...
* `_ACE.csv`, with the following columns:
  ...
  * **AllowOrDeny**: `0` (or `false`) if the entitlement is allowed, `1` (or `true`) if it is denied;
  * **OwnerSID**: SID of the entitlement's owner.
* `_PathInformations.csv`, with the following columns:
  * **Path**;
...
* `_SID.csv`, with only one column **SID**.
...
| Interactive | `Connections----Interactive` |
| LevelOfScan | `Connections----MembersFile` |
| ListOfSIDToAvoid | `Connections----ListOfSIDToAvoid` |
| Login | `Connections----Login` |
| OnlyDirectoryScan | `Connections----OnlyDirectoryScan` |
| Password | `Connections----Password` |
| InputDirectories | `Connections----InputDirectories` |

* A [Connection](../../../toolkit/xml-configuration/connectors/connection/index "Connection") able to store Active Directory's `Login` and `Password`.
