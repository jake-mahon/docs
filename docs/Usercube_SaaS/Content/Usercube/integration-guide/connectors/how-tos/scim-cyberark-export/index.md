...
* Use the [Connector](../../../toolkit/xml-configuration/connectors/connector/index "Connector") element to declare the connector with the following attributes:
  ...
  * **Identifier** identifies this connector in the applicative configuration. We recommend using a meaningful name such as `CyberArk`. If several connections to several CyberArk targets are possible, only one CyberArk Connector per Agent is used. See the [Create a Working Directory](../../../../installation-guide/production-ready/working-directory/index "Create a Working Directory") topic for additional information.
...
The [Entity Model](../../../entity-model/index "Entity Model") should match as closely as possible the structure of the CyberArk data relevant for Identity Manager. It is designed by analyzing the CyberArk data structure, and describing said data with the Entity Types and [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "Entity Association").
...
A good starting point for the Entity Model is to mirror the shape of the exported CyberArk SCIM objects. This guide provides a few examples that can serve this purpose. Thus, CyberArk SCIM objects such as **Users** and **Groups** can be described by Entity Types, and group membership by [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation"). See the [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") topic for additional information.

The [Entity Model](../../../entity-model/index "Entity Model") for the CyberArk connector is written in the applicative configuration. It is strongly recommended to write the entity model to the newly created `Conf/SCIMCyberArk/CyberArk Connector.xml` file. See the [Toolkit for XML Configuration](../../../toolkit/index "Toolkit for XML Configuration") topic for additional information.

#### Write entity types

Declaring an Entity Type is achieved with the `` tag and the following attributes:
...
* **DisplayName\_Li, i ? [1..16]** are used in the UI to identify this Entity Type for the end-user. **DisplayName\_L1** is the name of the entity type in *language number one*. If this language is *English*, a good example value would be `CyberArk - User`. See the [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") topic for additional information.
...
```
The CyberArk SCIM objects attributes are modeled by Entity properties, with the `` tags declared as children of the ``.
...
* **TargetColumnIndex** defines in which column of the resource table the property is stored. See the [Entity Type](../../../toolkit/xml-configuration/metadata/entitytype/index "EntityType")topic for additional information.
...
![SCIM CyberArk Menu Items](../../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/scim_cyberark_export_menu_item_5.1.6.png)
...
![SCIM CyberArk Display Entity Type](../../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/scim_cyberark_export_display_entity_type_5.1.6.png)
...
The [Display Table](../../../toolkit/xml-configuration/user-interface/displaytable/index "Display Table") contains a list of [Display Table](../../../toolkit/xml-configuration/user-interface/displaytable/index "Display Table") column elements that identify which properties should be included in the list display.
...
![SCIM CyberArk Display Table](../../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/scim_cyberark_export_display_table_5.1.6.png)
...
The [Access Control Rule](../../../toolkit/xml-configuration/access-control/accesscontrolrule/index "AccessControlRule") and [Access Control Rule](../../../toolkit/xml-configuration/access-control/accesscontrolrule/index "AccessControlRule") elements define the [AccessControlPermission](../../../toolkit/xml-configuration/access-control/accesscontrolpermission/index "AccessControlPermission") for end-user profiles to read and write the connector's data (such as resources of a given entity type). It used by the UI when displaying data such as resources and available roles.
...
To complete a Job, the Agent, via the [Usercube-Invoke-Job](../../../executables/references/invoke-job/index "Usercube-Invoke-Job") uses:
...
For an Agent to launch server-side Tasks from the Job via the [Usercube-Invoke-Job](../../../executables/references/invoke-job/index "Usercube-Invoke-Job"), the profile linked to these tasks and used by the tool should be authorized to execute said tasks.
...
Granting access can be done via the [SynchronizationAccessControlRules](../../../toolkit/xml-configuration/configuration/scaffoldings/accesscontrolrules/jobs/synchronizationaccesscontrolrules/index "Synchronization Access Control Rules") scaffolding and the[Job View Access Control Rules](../../../toolkit/xml-configuration/configuration/scaffoldings/templates/jobviewaccesscontrolrules/index "JobViewAccessControlRules") scaffolding.
...
The following example creates a `ClientId/Secret` pair to be used by the Agent to authenticate to the Server and complete Jobs. The secret is hashed with the [Usercube-New-OpenIDSecret](../../../executables/references/new-openidsecret/index "Usercube-New-OpenIDSecret") tool.
...
Use the [Job](../../../toolkit/xml-configuration/jobs/job/index "Job") CronTab Expression attribute.
