...
The Introduction Guide introduced the role model and how it influences assigning entitlements to identities. Let's sum up the key principles here. See the [Entitlement Management](../../../introduction-guide/overview/entitlement-management/index) topic for additional information.
...
4. The role model is first a catalog of available roles ([Single Role](../../toolkit/xml-configuration/provisioning/singlerole/index "SingleRole") and [Composite Role](../../toolkit/xml-configuration/provisioning/compositerole/index "CompositeRole")), identified by meaningful names aimed at non-technical end-users. These roles represent status of trust and privileges, to be assigned to identities, manually or automatically.
5. The role model is also a set of rules aiming at assign automatically roles to identities, based on relevant criteria, namely [Dimension](../../toolkit/xml-configuration/metadata/dimension/index "Dimension").
6. The role model classifies resources by security concerns thanks to [resource types](../toolkit/xml-configuration/provisioning/resourcetype/index).
...
Category
. They are used in the UI to organize the Roles catalog display. Categories are organized in a hierarchical tree structure.
:::

### Policy

A [Policy](../../toolkit/xml-configuration/provisioning/policy/index "Policy") is a set of assignment rules. At least one policy must be declared.
...
The integration team defines [Context Rule](../../toolkit/xml-configuration/provisioning/contextrule/index "ContextRule") and [Record Section](../../toolkit/xml-configuration/provisioning/recordsection/index)in the applicative configuration that assigns, for every identity, a context as a set of dimension-value pair.

The details of how contexts are generated can be found in [Generate Contexts](../../role-assignment/generate-contexts/index "Generate Contexts").
...
This mechanism allows the integration team to define rules to take care of the most basics and repetitive assignments. For example, a [Single Role Rule](../../toolkit/xml-configuration/provisioning/singlerolerule/index "SingleRoleRule") assigning a specific single role to the resources that match a specific context.
...
The role model is built iteratively, together with the [Entity Model](../../entity-model/index), as they closely influence one another. See the [Entity Model](../../entity-model/index) topic for additional information.
...
The first iteration of building of the organization reference model starts to reveal the archetypal responsibilities and positions of the members of the organization. A [Single Role](../../toolkit/xml-configuration/provisioning/singlerole/index "SingleRole") is defined for every fine-grained organization-level responsibility or position.
...
For every [Single Role](../../toolkit/xml-configuration/provisioning/singlerole/index "SingleRole") assigned to an identity, fine-grained entitlements need to be granted. Those are the resource values in a managed system.
...
* Provisioning rules, such as [Resource Type](../../toolkit/xml-configuration/provisioning/resourcetype/index) rules that decide what resources should be found in the managed systems; and navigation rules or scalar rules, that identify actual values to be fulfilled from the identity to which the single role is assigned;
* [Resource Correlation Rule](../../toolkit/xml-configuration/provisioning/resourcecorrelationrule/index "ResourceCorrelationRule") that identify for an identity, the target resources to fulfill;
* [Resource type](../toolkit/xml-configuration/provisioning/resourcetype/index) that organize resources and describe a source/target (or owner/resource) relationship.
  ...
  Sets of scalar rules and navigation rules relevant to a specific resource type are gathered into a [resource type](../toolkit/xml-configuration/provisioning/resourcetype/index).
...
To modelize that need within the role model, every identity with `Internet Access` single role is associated with an Active Directory account. We can find the Active Directory for an identity by comparing the identity email with the Active Directory entry e-mail. That's an example of [Resource Correlation Rule](../../toolkit/xml-configuration/provisioning/resourcecorrelationrule/index "ResourceCorrelationRule") that define the ownership of an Active Directory entry resource by an identity resource.

### 3. Write assignment rules

[Single Role Rule](../../toolkit/xml-configuration/provisioning/singlerolerule/index "SingleRoleRule") describe criteria for which a [Single Role](../../toolkit/xml-configuration/provisioning/singlerole/index "SingleRole") is assigned to a resource. The main criterion is a dimension value. For a given resource, the single role is assigned if the resource's context matches the given dimension value. The second criterion is the assignment of a specific [Composite Role](../../toolkit/xml-configuration/provisioning/compositerole/index "CompositeRole") (see further).

A [navigation rule](../toolkit/xml-configuration/provisioning/resourcetype/index) describes a fine-grained entitlement in the form of resource association such as a group membership. Its enforcement is also conditioned by a single role assignment to the relevant source resource, which in turn materializes the link between a single role and a [resource type](../toolkit/xml-configuration/provisioning/resourcetype/index).
...
[Single Role](../../toolkit/xml-configuration/provisioning/singlerole/index "SingleRole") can be packaged into [Composite Role](../../toolkit/xml-configuration/provisioning/compositerole/index "CompositeRole"). Assigning a composite role to an identity immediately assigns the packaged single role to that identity. Single roles assigned this way are said to be inferred.

The [Composite Role Rule](../../toolkit/xml-configuration/provisioning/compositerolerule/index "CompositeRoleRule") (see composite role rules describe criteria for which a composite role is assigned to an identity. Then, the composite role can be used as a condition in a [Single Role Rule](../../toolkit/xml-configuration/provisioning/singlerolerule/index "SingleRoleRule"). This is how packages are built.
...
3. Identity Manager's engine identifies a resource type `Rt` with the type rule `Tr` whose condition matches `SRa` and/or `Ci`.
4. Using `Rt`'s definition, Identity Manager's engine identifies by correlation a target resource `Tr` from the resource repository that must be created or updated to materialize `SRa`.
...
This series of steps is actually a very simplified version of the [Evaluate Policy](../../role-assignment/evaluate-policy/index "Evaluate Policy") algorithm.

![Cascading From Dimensions To Roles To Provisioning Orders](../../../../../../../static/images/Usercube_SaaS/Content/Resources/Images/enforce-assignment-policy-summary.png)

---

## Evaluate Policy

This chapter gives the basis of the assignments vocabulary. The next chapter enlightens the reader about the inner details of the Evaluate Policy algorithm. See the [Evaluate Policy](../../role-assignment/evaluate-policy/index "Evaluate Policy") topic for additional information.