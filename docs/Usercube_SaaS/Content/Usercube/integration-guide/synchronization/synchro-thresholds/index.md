...
Once the changes have been reviewed, the blocked job can be resumed (or not). See the [Synchronize Data](../../../user-guide/set-up/synchronization/index) topic for additional information.
...
* [Entity Type Mapping](../../toolkit/xml-configuration/connectors/entitytypemapping/index "EntityTypeMapping") to count the number of resources impacted by synchronization inside a given entity type. They are configured with:
...
* [Entity Association Mapping](../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping") to count the number of navigation properties impacted by synchronization inside a given entity type. They are configured with:

  | Absolute Threshold | Relative Threshold |
  | --- | --- |
  | `MaximumLinkDeletedLines` | `MaxLinkPercentageDeletedLines` |
  | `MaximumLinkInsertedLines` | `MaxLinkPercentageInsertedLines` |
* [Connector](../../toolkit/xml-configuration/connectors/connector/index "Connector") to count the number of resources and/or navigation properties impacted by synchronization inside all entity types of a given connector. They are configured with:
...
Distinct [Thresholds](../../provisioning/prov-thresholds/index "Thresholds") are configurable for provisioning.