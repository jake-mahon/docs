...
Netwrix Password Policy Enforcer can enforce up to 256 different password policies. You can assign policies to users directly, or indirectly through Active Directory security groups and containers (Organizational Units). See the [Assign Policies to Users & Groups](usersgroups.md) topic for additional information.
...
Launches the Test policy tool in a separate window. You can test __By user__ and by __Password bulk test__. See the [Test Policy](testpolicy.md) topic for additional information.
...
This flowchart shows how Password Policy Enforcer determines a policy for each user. Use the [Test Policy](testpolicy.md) tool to quickly determine which policy Password Policy Enforcer is enforced for a particular user.
