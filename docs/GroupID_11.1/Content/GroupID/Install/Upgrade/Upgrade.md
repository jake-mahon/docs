...
Step 1 – To launch the Upgrade wizard, click **Next** on the GroupID Successfully Configured page of the Configuration Tool.

OR

Click **Start** > **Imanami** > **GroupID Upgrade Tool 11.0**.

![Welcome page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/1-Welcome.jpg "Welcome page")

Step 2 – Read the welcome message and click **Next**.

![](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/2-Select Source Version.jpg)

Step 3 – From the Select the previous version to upgrade list, select the Directory Manager version to upgrade from.

NOTE: The following steps discuss the upgrade process with Directory Manager 10 as the source version. The process may vary for different source versions.

Step 4 – Click **Next**.

![Select modules to upgrade](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/3-Select modules.jpg "Select modules to upgrade")
...
* Custom – choose what data you want to upgrade. On selecting it, the following options are listed, from where you can choose the data to upgrade.
  ...
  ![](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/3-Select modules-custom.jpg)

  NOTE: If later on, you wish to upgrade specific groups and their history via the Upgrade-Group commandlet, then you must upgrade the Configuration and History in the first upgrade run. This will upgrade the history in the database as per Directory Manager 11.1 format and replicates it to Elasticsearch. Later on, when you upgrade specific groups and their history using the Upgrade-Group commandlet, that will be done successfully. See the [Upgrade-Group](../../ManagementShell/SmartGroup/UpgradeGroup "Upgrade-Group") commandlet for additional information.
...
![Microsoft Entra ID Store Upgrade page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/EntraIDStore.png "Microsoft Entra ID Store Upgrade page")

Provide the following information:

* Registered Client Secret on EntraID: provide the client secret value generated against the certificate uploaded Microsoft Entra Admin Centerwhile registering the Directory Managerapplication.
* PFX Certificate: Click Browse to navigate to the folder where the certificate is saved. This certificate must be the same uploaded for the registered Directory Manager app in Microsoft Entra ID.
...
![ Upgrade wizard Microsoft Entra ID Messaging System page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/EntraIDMessagingSystem.png " Upgrade wizard Microsoft Entra ID Messaging System page")

Provide the following information:

* Client Secret: provide the client secret value generated against the certificate uploaded to Microsoft Entra Admin Center while registering the Directory Manager application.
* PFX Certificate: Click Browse to navigate to the folder where the certificate is saved. This certificate must be the same uploaded for the registered Directory Manager app in Microsoft Entra ID.
* PFX Certificate Password: Provide password of the PFX certificate.

Step 9 – If any Synchronize jobs exist in Directory Manager 10 or the version you are upgrading from, then the wizard checks the destination domain set for them. Let's assume you have 5 jobs that connect to different child domains in a forest with different service accounts and different messaging providers.
...
* When there is no identity store in Directory Manager 10 for the destination domain that the jobs connect to, the Upgrade wizard reads the FQDN of the destination domains used in the jobs and tries to create a forest structure. On identifying one, it proceeds to create an identity store for the forest, requiring you to provide a service account. (The user name and password fields will be auto populated but you can change them.) All jobs with destination provider for that forest domain or any of its child domains will be moved to the new identity store.
  ...
  ![](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/Synchronize Upgrade.png)

  NOTE:  The service account you provide here should have at least *read* permission in the entire forest, so that all objects from the forest can be replicated to Elasticsearch.
...
![Upgrade wizard Synchronize Messaging System page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/EntraIDSynMessagingSystem.png "Upgrade wizard Synchronize Messaging System page")

Provide the following information:

* Certificate: Click Browse to navigate to the folder where the certificate is saved. This certificate must be the same uploaded for the registered Directory Manager app in Microsoft Entra ID.
* Password: Provide password of the PFX certificate.
* Client Secret: provide the client secret value generated against the certificate uploaded to Microsoft Entra Admin Center while registering the Directory Manager application.

Step 11 – Click **Next**.

Step 12 – In Directory Manager 10 and earlier versions, reports were generated for the domain that the Directory Manager server was joined to. During upgrade, the wizard checks if an identity store for that domain exists or not.
...
* If an identity store for that domain does not exist, then you have to create an identity store for it. It must essentially be an Active Directory identity store. The wizard will bind the reports generated in Directory Manager 10 to the identity store, so you will be able to view them in Directory Manager 11.1.  
  ![](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/Reports Upgrade.png)
...
* For schedules with Synchronize jobs that use file-based providers as destination, the Upgrade wizard will display the following page that will list all such schedules. Select an identity store for each schedule, so that the schedule moves to that identity store.
  ...
  ![](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/Store Selection for schedules.png)

  The rules stated above also apply to schedules with job collections added to them.

Step 14 – Click **Next**.

![Summary page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/5-Summary.jpg "Summary page")
...
NOTE: If there are any disabled identity store(s) in the source Directory Manager version, Directory Manager will not upgrade those identity store(s). However, data of those identity store(s) will remain intact in the source Directory Manager version.

Step 15 – Review the summary and click **Next**.

![Upgrade Progress page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/6-Upgrade Process Complete.jpg "Upgrade Progress page")
...
![Upgrade Completed page](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/Upgrade/7-Upgrade Complete.jpg "Upgrade Completed page")
