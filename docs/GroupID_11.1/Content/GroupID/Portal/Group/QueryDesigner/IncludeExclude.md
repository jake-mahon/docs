...
* Immediately – When you close the Query Designer dialog box, the Directory Manager portal adds the objects in the Include list to the group membership and removes the objects in the Exclude list from the group
  ...
  job.
* At group membership update – When the group membership is updated using a scheduled job or the Update command, Directory Manager obtains the query results, adds the objects to include, and then removes the objects
  to exclude.
...
Displays the list of objects to include in group membership. The list also displays the objects that are imported to the membership of this group using the portal's import feature. Use the **Add** and **Remove** buttons
to modify this list.

**Exclude**

Displays the list of objects to exclude from group membership. Use the **Add** and **Remove** buttons to modify this list.
