...
permissions granted to a user over files and folder residing on a server.
...
* A server is added for permission analysis on the **Entitlement** page in an Active Directory identity store.

  Or
* A SharePoint site is added for permission analysis on the **Entitlement** page in a Microsoft Entra ID identity store.
...
to Elasticsearch, enabling users to view, manage and update these permissions in the Directory Managerportal.
...
Entitlement-related permissions for a security role in an identity store are discussed in the [Entitlement](../../../../AdminCenter/SecurityRole/Permissions#Entitlement "Entitlement") section.