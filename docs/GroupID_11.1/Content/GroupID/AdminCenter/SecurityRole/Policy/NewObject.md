...
* [Limit Object Creation to the OU the User Resides in](#limit "Limit Object Creation to the OU the User Resides in")
* [Restrict Role Members to Create Objects in Specific OUs](#restrict "Restrict Role Members to Create Objects in Specific OUs")
* [Remove an OU](#remove "Remove an OU")
...
7. Select the **Users can create objects only in their own container** check box to limit role members to create directory objects in the organizational unit they reside in respectively.  
...
8. On the **Add Container** dialog box, select one or more containers where role members can create the particular objects. Then click **Add**. The container(s) are displayed
   with the name of the respective object type.
9. You can choose to set a container for all object types. Click the **Copy** button for a container and then click **Yes** on the **Copy Container** message box. The container is displayed with all object types.
10. Click **OK**.
11. On the **Edit Security Role** page, click **Update Security Role**.
12. On the **Security Roles** page, click **Save**.

NOTE: When role members create a new object, the *Container* option shows the containers specified for the respective object type. The user can select the desired container to create the object there. When one container
is specified, the *Container* option displays its distinguished name as read-only.
...
7. For each object type, the allowed containers are listed with it.
   ...
   * Click **Remove All** for an object type
