...
* [Replace an Attribute on the Organizational Hierarchy Tree](#tree "Replace an Attribute on the Organizational Hierarchy Tree")
...
8. In the **Display Type** drop-down list, select the display type to use for rendering the attribute on the organizational hierarchy chart. The display type must match the attribute. For example, the ‘TreePicture’ display type matches the ‘thumbnailPhoto’ attribute.  
   This list contains basic display types and custom display types defined on the **Custom Display Types** page. See the [Display Type Categories](../DisplayType/Categories "Display Type Categories") topic.
9. Click **OK**.
