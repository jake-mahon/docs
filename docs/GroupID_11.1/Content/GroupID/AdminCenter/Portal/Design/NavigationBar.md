...
![](../../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/AdminCenter/Application/Navigation bar.png)
...
* [Add a Node (Category)](#Category "Add a Node (Category)")
* [Update the Details of a Node](#UpdateNode "Update the Details of a Node")
* [Rearrange the Nodes on the Navigation Bar](#RearrangeNode "Rearrange the Nodes on the Navigation Bar")
* [Remove a User-defined Node](#RemoveNode "Remove a User-defined Node")

**At the sub-node level:**

* [Add a Sub-node to a Node](#Subnode "Add a Sub-node to a Node")
* [Update the Details of a Sub-node](#UpdateSub "Update the Details of a Sub-node")
* [Rearrange the Sub-nodes for a Node](#RearrangeSub "Rearrange the Sub-nodes for a Node")
* [Remove a Sub-node from a Node](#Remove "Remove a Sub-node from a Node")

**At the tab level:**

* [Update Tab Properties](#properties "Update Tab Properties")
* [Rearrange the Tabs for a Sub-node](#Tabs "Rearrange the Tabs for a Sub-node")
* [Remove a Tab from a Sub-node](#RemoveTabs "Remove a Tab from a Sub-node")
...
* The *Create New*, *High Priority*, *Others*, and *External Links* nodes cannot be edited or removed.
* You can add and remove sub-nodes for nodes; however, you cannot add sub-nodes for the **Create New** node.
...
4. Click **Navigation Bar** in the left pane.  
   The **Navigation Bar** page has two tabs: the **Navigation Bar Categories** tab is used to manage nodes while the **Navigation Bars** tab is used to manage sub-nodes and their respective tabs.
5. Click **Navigation Bar Categories** to define a new node.
6. On the **Navigation Bar Categories** tab, click **Add**. The **Add Navigation Category** pane is displayed.
7. Enter a name for the node in the **Title** box. The node will be displayed with this name on the navigation bar.
8. In the **Access Level** drop-down list, select a security role. The node would be visible to users of this role and roles with a priority value higher than this role. For all other users, the node would be hidden. See [Priority](../../SecurityRole/Manage#Priority "Priority").  
   Select *Never* to hide the node for all users.
...
The *Create New*, *High Priority*, *Others*, and *External Links* nodes cannot be modified.
...
5. On the **Navigation Bar Categories** tab, click **Edit** for a node to update it.  
   The **Edit Navigation Category** pane is displayed. Follow step 7 and onwards in the [Add a Node (Category)](#Category "Add a Node (Category)") topic to update node details.
...
3. Select an identity store under **Design Settings** to customize the portal for it.  
   All identity stores linked with the portal are listed under **Design Settings**. You can design a different portal for each of these.
4. Click **Navigation Bar** in the left pane. Then click the **Navigation Bar Categories** tab. The **Navigation Bar Categories** tab is displayed.
5. To rearrange the nodes on the navigation bar, click the equal sign for a node and drag to change its position.  
   You can move all except the *Create New* node.
6. Click **Save**.
...
* You cannot add sub-nodes to the *Create New* node.
...
3. Select an identity store under **Design Settings** to customize the portal for it.  
   All identity stores linked with the portal are listed under **Design Settings**. You can design a different portal for each of these.
4. Click **Navigation Bar** in the left pane. The **Navigation Bar** page is displayed with the **Navigation Bars** tab selected by default.
5. In the **Select Navigation Bar Type** drop-down list, select a node on the portal’s navigation bar; its sub-nodes get listed under **Name**.
...
8. In the **Display Text** box, enter the text to display as the sub-node name on the navigation bar.
9. In the **Tooltip Text** box, enter the text to appear when a user hovers the mouse over the sub-node.
10. Use the **Icon Class** box to upload the image (icon) to be displayed with the sub-node name. See step 10 in the [Add a Node (Category)](#Category "Add a Node (Category)") topic for details.  
    The facility to upload an image is not available for sub-nodes under *Groups*. *Users*, *Requests*, and *External Links*.
11. In the **URL** box, provide the URL of a webpage to link to this sub-node. The respective webpage will be displayed when users click the sub-node in the portal.  
...
13. In the **Access Level** drop-down list, select a security role. The sub-node would be visible for users of this role and roles with a priority value higher than this role. For all other users, the sub-node would be hidden. See [Priority](../../SecurityRole/Manage#Priority "Priority").  
    Select *Never* to hide the sub-node for all users.
14. Click **OK**.
...
7. Follow steps 7-14 in the [Add a Sub-node to a Node](#Subnode "Add a Sub-node to a Node") topic to update the required information and save it.  
   The **Links** section is displayed for system-defined nodes only. Use it to manage the tabs for a sub-node.
...
4. Click **Navigation Bar** in the left pane. The **Navigation Bar** page is displayed with the **Navigation Bars** tab selected by default.
5. In the **Select Navigation Bar Type** drop-down list, select a node on the portal’s navigation bar; its sub-nodes get listed under **Name**.
6. Click **Remove** for a sub-node to remove it. This also removes all tabs defined for the sub-node.
7. Click **Save**.
...
4. Click **Navigation Bar** in the left pane. The **Navigation Bar** page is displayed with the **Navigation Bars** tab selected by default.
5. In the **Select Navigation Bar Type** drop-down list, select a node on the portal’s navigation bar; its sub-nodes get listed under **Name**.
6. Click **Edit** for a system-defined sub-node. On the **Edit Link** pane, the **Links** area lists the tabs for the sub-node (in case the sub-node has tabs).
...
8. On the **Edit Link** pane (for the tab), update the required information and click **OK**:
   ...
   5. **Access Level** – Select a security role. The tab would be visible to users of this role and roles with a priority value higher than this role. For all other users, the tab would be hidden. See [Priority](../../SecurityRole/Manage#Priority "Priority").  
      Select *Never* to hide the tab for all users.
...
2. Click the ellipsis button for a portal and select **Settings**.
3. Select an identity store under **Design Settings** to customize the portal for it.  
   All identity stores linked with the portal are listed under **Design Setting**s. You can design a different portal for each of these.
4. Click **Navigation Bar** in the left pane. The **Navigation Bar** page is displayed with the **Navigation Bars** tab selected by default.
5. In the **Select Navigation Bar Type** drop-down list, select a node on the portal’s navigation bar; its sub-nodes get listed under **Name**.
6. Click **Edit** for a system-defined sub-node. On the **Edit Link** pane, the **Links** area lists the tabs for the sub-node (in case the sub-node has tabs).
