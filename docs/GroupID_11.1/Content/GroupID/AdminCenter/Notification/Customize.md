...
Directory Manager generates a variety of notifications for different events, such as when changes are made to a group, when workflows are triggered, and when profile validation is due for users. Supported languages for notifications are listed in the [Localization](../../GettingStarted#Localization "Localization") topic.
...
By default, notifications are sent to users in the English language. However, a user can opt to receive notifications in a different language by personalizing the language settings from the **Settings** panel in
the Directory Manager portal.

## Customize a Notification Template

1. In Admin Center, click **Notifications** in the left pane. Then click **Notification Editor** on the **Notification Queue** page.  
   Or  
...
3. You can search for a notification template using the following filters:
   ...
   * Specify a search string in the search box at the top and press **Enter**; notification templates that have the string as part of their name or description are displayed. You can also search for a specific
...
5. Select a mode to view the template. Options are:
   ...
   * Interactive - Click the **Interactive** tab to view the template in a user–friendly format. This is how the notification will be displayed to the recipients. You cannot format or edit the template in
     this view.
6. You can customize the notification template in the following ways:

   * [Change the Subject Line](#line "Change the Subject Line")
   * [Edit and Format the Content](#content "Edit and Format the Content")
   * [Replace the Tags](#tags "Replace the Tags")
...
2. In the **Edit Title** dialog box, change the subject line of the notification and click **Update**.
...
* Use the options on the **Edit** menu to search for target text (whether it be a word, phrase, or tag), to cut the selected text, to copy the selected text, and to redo and undo an operation. You can also insert
...
1. In the Source Code view, click the **Dictionary** tile to view the recommended tags to replace the default tags with.

   ![](../../../../../../static/images/GroupID_11.1/Content/Resources/Images/GroupID/AdminCenter/Notifications/TagDictionary.png)

   Each notification template has its own set of recommended tags.
2. Use the *Go To* option on the **Edit** menu to search for a tag you want to replace. Refer to the tag dictionary to know what tags are used in the notification.
