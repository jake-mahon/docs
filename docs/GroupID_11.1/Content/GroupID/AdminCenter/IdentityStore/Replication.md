...
Step 6 – The Select Replication Attributes dialog box displays a list of the schema attributes in the provider. Select the check boxes for the attributes you want to replicate for the identity store and click **Save**. The attributes are added to the Attribute Name column on the Replication page.

NOTE: If in a Microsoft Entra ID based identity store extension attributes are added, Directory Manager Schema Replication schedule fetches the latest schema at its next run and add the newly added extension attributes to the Select Replication Attributes list. See the [Schema Replication Schedule](../Schedule/SchemaReplication "Schema Replication Schedule") for additional information.   
Select the required extension attributes from the Select Replication Attributes list and add them to the attribute to replicate list. See the [Specify Object Attributes to Replicate](#Specify "Specify Object Attributes to Replicate") section of the [Manage Local Replication Settings](# "Manage Local Replication Settings") topic.
...
Step 4 – On the Replication page, click **Add Replication Attributes** to search, select, and add the required attribute (for example, *SerialNumber*)
to the **Attribute Name** list.
...
Step 7 – Click **Restore** on the **Restore Data** card to restore object data to Elasticsearch for the identity store.
...
Force-starting the service has no impact on the interval set for triggering the service on the Replication page. See the [Specify a Replication Interval for Objects](../Replication/Settings#interval "Specify a Replication Interval for Objects")topic.
...
Step 2 – On the Identity Stores page, click the ellipsis button for an identity store and select **Replicate Objects**.   
Any changes
...
Step 4 – On the Replication page, click **Run** on the Replication Service card to run the Replication service instantly. Any changes
made to the values of attributes during the last and current run of the Replication service are replicated to Elasticsearch.  
...
Force-starting the service has no impact on the interval set for triggering the service on the **Replication** page. See the [Specify Interval for Deleting Tombstone Objects](../Replication/Settings#tombstone "Specify Interval for Deleting Tombstone Objects") topic.
...
Step 2 – On the Identity Stores page, click the ellipsis button for an identity store and select **Replicate Deleted Objects**. Any objects deleted in the directory between the last and current run of the Replication service will be removed from Elasticsearch.
...
Step 4 – On the Replication page, click **Run** on the Deleted Objects Replication card to run the Replication service instantly.
...
Step 4 – On the Replication page, click **Restore** on the Restore Data card to restore object data for the identity store to Elasticsearch.