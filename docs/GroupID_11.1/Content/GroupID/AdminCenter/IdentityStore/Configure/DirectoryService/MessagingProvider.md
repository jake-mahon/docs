...
When a messaging provider is not available in your environment or you want to restrict the creation of mail-enabled objects in the identity store, select the *None* option.
...
* [Set Microsoft Exchange as Messaging Provider](#exchange "Set Microsoft Exchange as Messaging Provider")
* [Set Office 365 as Messaging Provider](#0ffice "Set Office 365 as Messaging Provider")
* [Set Google Workspace as Messaging Provider](#google "Set Google Workspace as Messaging Provider")
* [Set an External Provider to Create Email Addresses](#external "Set an External Provider to Create Email Addresses")
* [Do Not Set a Messaging Provider](#None "Do Not Set a Messaging Provider")
...
Step 3 – Click **Configurations** under Settingsin the left pane. Then click **Messaging System**. The Messaging System page is
displayed.

Step 4 – In the Messaging Provider drop-down list, select the **Exchange version** that you want to use.
...
* In the Server Status & Priority section, select the check box for the server you want to specify or change the priority for. Directory Manager checks the availability of the server and displays its status
  as *Online* (available) or *Offline* (unavailable) in the **Status** column.
...
Step 4 – On theMessaging System page, select *Office 365* in the Messaging Provider drop-down list.
...
Step 9 – Specify the path to the PFX certificate in the PFX Certificate box. For that, click **Choose File** and browse for the file. Select it and click **Open**.  
...
Step 11 – In the Server Status & Priority area, the **Server Name** box displays the name of the mail server; the priority option does not apply to it. Directory Manager checks the availability
of the server and displays its status as *Online* (available) or *Offline* (unavailable) in theStatus column.  
...
Step 2 – On the Identify Storespage, click the ellipsis button for an identity store and select **Edit**.

Step 3 – Click **Configurations** under Settings in the left pane. Then click **Messaging System**.

Step 4 – On the Messaging System page, select *Google Workspace* in the **Messaging Provider** drop-down
list.
...
Step 4 – On the Messaging System page, select *Other* in the **Messaging Provider** drop-down list.
...
* [Introduction to Regular Expressions](https://docs.microsoft.com/en-us/previous-versions/visualstudio/visual-studio-2010/28hw3sce(v%3dvs.100) "Introduction to Regular Expressions")
* [Regular Expression Syntax Reference](https://msdn.microsoft.com/en-us/library/ms840427.aspx "Regular Expression Syntax Reference")

Step 8 – In the Enter Example to Validate Regular Expression box, enter an email alias as an example, and then click **Validate** to verify that the example satisfies the regular expression.
...
Step 4 – When no messaging provider is available in your environment or you want to prevent users from creating mail-enabled objects in the identity store, select *None* in the Messaging Provider drop-down list.

Step 5 – Click **Save**.

**See Also**

* [Configure an SMTP Server](../SMTPServer "Configure an SMTP Server")