...
* **When objects are created, modified, and deleted** - When you enable history tracking for an identity store, Directory Manager tracks all actions that are committed to the directory using:
  ...
  * APIs
  ...
  You can choose to track specific actions, such as ownership change and object deletion. The table in the [Track Specific Actions for an Identity Store](#track "Track Specific Actions for an Identity Store") topic lists some individual actions available for tracking.
...
* **When changes are made to an identity store’s configurations** - Directory Manager enables you to track the changes made to an identity store’s configurations, which include the following:
  ...
  * security roles
  ...
  See the [Identity Store History](../../History/View "Identity Store History") topic to view the tracked history data.
...
See the [History in Directory Manager](../../../History "History in Directory Manager") and [Event Logging](../../History/EventLogging "Event Logging") topics for additional information.

What do you want to do?

* [Enable History Tracking for All Actions](#enable "Enable History Tracking for All Actions")
* [Track Specific Actions for an Identity Store](#track "Track Specific Actions for an Identity Store")
* [Retain Complete History Data](#retain "Retain Complete History Data")
* [Retain History for a Specific Period](#retainspecific "Retain History for a Specific Period")
* [Disable History Tracking](#disable "Disable History Tracking")
...
4. Use the toggle button in the top right corner to enable or disable history tracking for the identity store.
   ...
   * On enabling, *All Actions* is auto selected in the **Track** drop-down list. This indicates that (a) all actions that users perform on directory objects and (b) changes
     made to identity store configurations, workflows, and security roles are tracked for the identity store.  
     To track specific actions, see the [Track Specific Actions for an Identity Store](#track "Track Specific Actions for an Identity Store") topic.
...
2. On the **Identity Stores** page, click the ellipsis button for an identity store and select **Edit**.
3. Click **Configurations** under **Settings** in the left pane. Then click **History**.
4. On the **History** page, make sure history tracking is enabled. Them select *Selected Actions* in the **Track** drop-down list. This enables
   the **Actions** area, where the following actions are listed:
...
this data by moving it from the database to CSV files. See the [History Retention Schedule](../../../Schedule/HistoryRetention "History Retention Schedule") topic.
