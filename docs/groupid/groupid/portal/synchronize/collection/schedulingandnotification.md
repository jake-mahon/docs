...
2. In the __Schedule__ section, the __Task name__ drop down lists the Smart Group Update jobs existing in the identity store for the Active Directory domain that is provided as destination in this Synchronize job. Select a job that you want to run along with this Synchronize job. The selected job will run each time the Synchronize job is run either manually or from the Synchronize job scheduler.
   ...
   If you are modifying an existing job collection, you can also a new schedule for the job collection. Visit [Synchronize Schedule](/docs/product_docs/groupid/groupid/admincenter/schedule/synchronize.md)
...
Step 9 – Generated workflow request will be displayed in the [Requests](/docs/product_docs/groupid/groupid/portal/request/overview.md) section for the workflow approver(s). If the approver approves the workflow request, the job will execute the results.
