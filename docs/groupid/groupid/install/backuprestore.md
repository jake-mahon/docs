...
Before you upgrade from GroupID 10 to Directory Manager 11.1, take a backup of the Elasticsearch data folder. The default folder location is:
...
Step 3 – Create a new folder (ideally on a different drive) and paste the copied data into that folder.

### Restore

Follow these steps to restore GroupID Self-Service portals.

Step 1 – Copy the folders containing the portals from the backup folder you created in the previous steps.
...
Step 1 – Copy the folders containing the Password Center portals from the backup folder you created in the previous steps.
...
Step 3 – Create a new folder (ideally on a different drive) and paste the __Jobs__ folder into it.
...
Step 1 – Copy the __Jobs__ folder from the backup folder you created in the preceding steps.
...
Step 3 – Paste the __Jobs__ folder into the location, replacing any existing files.
...
Step 1 – Browse to the location where the reports generated by these products are saved. The default locations are:
...
Step 2 – On the Windows Run dialog box, type the path of the location where the reports are saved and run the command:
