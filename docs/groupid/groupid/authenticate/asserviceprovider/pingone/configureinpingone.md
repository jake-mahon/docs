...
3. On the __My Applications__ tab of the __Applications__ page, click __Add Application__ and select the __New SAML Application__ option to configure the Directory Manager client _Wizard_ in PingOne.  
...
5. In the __Application Description__ box, provide a description. For example, you can specify the Directory Manager client with which you want to set up PingOne for single sign-on.
...
10. Use the metadata file you generated for the _Wizard_ portal in Directory Manager to configure certain settings on this page. See the [Generate the Metadata File](/docs/product_docs/groupid/groupid/authenticate/asserviceprovider/pingone/generatemetadata.md#Generate-the-Metadata-File) topic.  
...
11. The configurations we made in PingOne will be displayed. Click __Finish__.  
    The new Directory Manager application you created in PingOne is displayed under __My Applications__.

### Configure Users in PingOne

You must define users in PingOne. These users are authenticated in Directory Manager on the basis of an attribute, as discussed in the [Attribute Mapping in PingOne](#Attribute-Mapping-in-PingOne) topic.

Only the users you define here can authenticate on the Directory Manager portal _Wizard_ using PingOne. See the [Sign In Using PingOne](/docs/product_docs/groupid/groupid/authenticate/asserviceprovider/pingone/signin.md) topic.
