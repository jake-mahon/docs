...
- __When objects are created, modified, and deleted__ - When you enable history tracking for an identity store, Directory Manager tracks all actions that are committed to the directory using:
...
- __When changes are made to an identity store’s configurations__ - Directory Manager enables you to track the changes made to an identity store’s configurations, which include the following:
  ...
  See the [Identity Store History](/docs/product_docs/groupid/groupid/admincenter/identitystore/history/view.md) topic to view the tracked history data.
...
See the [History in Directory Manager](/docs/product_docs/groupid/groupid/admincenter/history.md) and [Event Logging](/docs/product_docs/groupid/groupid/admincenter/identitystore/history/eventlogging.md) topics for additional information.
...
You can choose to enable or disable history tracking for an identity store. When enabled, all actions that users perform on directory objects using Directory Manager are tracked by default. For example, when a user creates a group, adds members to it, changes
its expiry date, and assigns owners, history logs are generated in Directory Manager. Moreover, changes made to identity store configurations, workflows, and security roles are also tracked.
...
You can set Directory Manager to retain an identity store's history data for a specified length of time in the database. When the retention period is over, the History Retention schedule archives
this data by moving it from the database to CSV files. See the [History Retention Schedule](/docs/product_docs/groupid/groupid/admincenter/schedule/historyretention.md) topic.
