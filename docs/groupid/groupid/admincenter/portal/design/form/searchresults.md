...
10. In the __Display Type__ drop-down list, select the display type to use to render this field in the portal. Available options are limited to textbox, DN, DNs, and Link, which are basic display Types. See the [Basic Display Types](../../displaytype/categories.md#basic-display-types) topic.
...
6. On the __Edit Search Results__ pane, click __Edit__ for a field to update it.  
   The __Edit Field__ pane is displayed. Follow step 7 and onwards in the Add a Field to a Search Results Page topic to update the field.
