...
![Managing databases](../../../../../../../static/images/PasswordSecure_9.2/Content/Resources/Images/Installation_with_parameters_234-en.png "Managing databases")
...
Further information on this subject can be found in the section[Multifactor Authentication](Database settings/Multifactor authentication AC "Multifactor Authentication").
...
**CAUTION:** It is important to note that the logbook is also used for the filter functions. If the logbook is regularly cleaned up, it is possible that the full functions of the filter will no longer be available.
...
**CAUTION:** When the migration is started, the database is set to migration mode. For the duration of the migration, it is not possible to log in to the database – users who are already logged in will be sent a corresponding message. The sessions will, however, remain open so that users can continue working as soon as the migration is complete.
