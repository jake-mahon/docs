...
__NOTE:__  Prior to adding an Active Directory Sync policy, you must first configure a Credential Profile with credentials properly provisioned for running the sync operation for the domain. See the [Application Server Requirements](../../../requirements/server.md) topic for the permissions. See the [Credential Profile Page](credentialprofile.md) topic for additional information on creating a profile.
