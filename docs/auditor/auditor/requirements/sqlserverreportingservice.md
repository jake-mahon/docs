...
__Step 6 –__ Navigate to __Start >____All Apps > SQL Server____Reporting Services Configuration Manager__.

__Step 7 –__ In the Reporting Services Configuration Connection dialog, make sure that your local report server instance (for example, _SQLExpress_) is selected, and click __Connect__.
...
__Step 9 –__ In the Reporting Services Configuration Manager left pane, select __Database__. Make sure that the SQL Server Name and Database Name fields contain correct values. If necessary, click __Change Database__ and complete the Report Server Database Configuration wizard.

__Step 10 –__ In the Reporting Services Configuration Manager left pane, select __Report Manager URL__. Make sure __Virtual Directory__ is set correctly, and that the URL is valid.
