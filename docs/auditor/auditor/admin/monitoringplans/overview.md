...
- [Supported Data Sources](../../requirements/supporteddatasources.md) – Configure data source as required to be monitored
...
__Step 1 –__ Create a monitoring plan with the wizard. Select the data source when you start the monitoring plan wizard, and its initial settings are configured at the wizard steps. See the [Create a New Monitoring Plan](create.md) topic for additional information.
...
| Modify plan settings, add or delete data sources, add or delete items | Select a plan and click Edit. On the page that opens, review your plan settings. Then follow the instructions described in these sections:   - [Add Items for Monitoring](datasources.md) - [Fine-Tune Your Plan and Edit Settings](finetune.md) |
...
For many data sources, you can instruct Netwrix Auditor to collect state-in-time data along with event data. For that, Netwrix Auditor uses state-in-time snapshots of the relevant system (for example, see [VMware](vmware/overview.md)).
...
See the [Role-Based Access and Delegation](delegation.md) topic for additional information.
