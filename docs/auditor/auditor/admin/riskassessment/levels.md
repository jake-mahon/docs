...
| --- | --- | --- |
| Users and computers |  |  |
| User accounts with "Password never expires" | Number of enabled user accounts with __Password never expires__ property set | - 0 — Low - [1 – 5] — Medium - > 5 — High |
...
| Servers that have local user accounts with __Password never expires__\* | Servers that have local user accounts with Password never expires / Overall number of servers (%) | - 0% — Low - >0% — High |
| Permissions |  |  |
...
| Site collections with the __Get a link__ feature enabled | Number of site collections with the Get a link feature enabled / Total number of site collections (%) | - ≤30% — Low - (30% - 60%) — Medium - ≥60% — High |
| Sites with the __Anonymous access__ feature enabled | Number of sites with the Anonymous access feature enabled / Total number of sites (%) | - ≤30% — Low - (30% - 60%) — Medium - ≥60% — High |
...
\* -here the _Overall number of servers_ means the number of Windows servers for which data collection was a success. That said, this count may vary across the risks. In such a case, it is recommended to examine Netwrix Auditor health log and omit lists.
