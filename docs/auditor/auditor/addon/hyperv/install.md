...
__Step 1 –__ Prepare Netwrix Auditor for Data Processing.

__Step 2 –__ Download the Add-On.

__Step 3 –__ Configure Parameters for Data Collection.

__Step 4 –__ Register Windows Scheduled Task.
...
In such scenario, you will need to specify this monitoring plan in the _NetwrixAuditorPlan_ and _NetwrixAuditorPlanItem_ parameters in the __settings.xml__ file. See the [Add-On Parameters](/docs/product_docs/auditor/auditor/addon/hyperv/parameters.md) topic for additional information.
...
In the add-on folder, open the __settings.xml__ file and configure the add-on parameters for data collection, as listed below.

See the [Add-On Parameters](/docs/product_docs/auditor/auditor/addon/hyperv/parameters.md)topic for the full list of configuration parameters.
...
| DataCollectionUserName | (empty) | Specify user account that will be used for data collection from SCVMM server. To use the account currently logged in, leave this parameter empty.s  Make sure the account has administrative rights on that server (see the [Accounts and Rights](/docs/product_docs/auditor/auditor/addon/hyperv/overview.md#Accounts-and-Rights) topic for additional information). |
...
Save the __settings.xml__ file. New configuration settings will be applied automatically at the next data collection.

For the full list of parameters, see the [Add-On Parameters](/docs/product_docs/auditor/auditor/addon/hyperv/parameters.md) topic for additional information.

## Register Windows Scheduled Task

Run the __install.ps1__ PowerShell script from the add-on folder. It will configure and register a Windows scheduled task that will run periodically every 15 min to retrieve audit data from SCVMM.
