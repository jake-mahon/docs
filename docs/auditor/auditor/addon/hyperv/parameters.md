...
| DataCollectionUserName | (empty) | Specify user account that will be used for data collection from SCVMM server. To use the account currently logged in, leave this parameter empty.  Make sure the account has administrative rights on that server (see the [Accounts and Rights](/docs/product_docs/auditor/auditor/addon/hyperv/overview.md#Accounts-and-Rights) topic for additional information). |
