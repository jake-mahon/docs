...
For more information on gMSA, refer to [Use Group Managed Service Account (gMSA)](../../requirements/gmsa.md)[Microsoft documentation](https://docs.microsoft.com/en-us/windows-server/security/group-managed-service-accounts/group-managed-service-accounts-overview).
...
1. On the SharePoint server where the Netwrix Auditor Core Service will be deployed: the account must be a member of the local Administrators group.   
   To learn more about Netwrix Auditor Core Services, refer to [Installation](../../install/overview.md) topic.
