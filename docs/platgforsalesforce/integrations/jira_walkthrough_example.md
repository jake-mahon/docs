...
[install and configure](/docs/platgforsalesforce/integrations/jira_integration.md) the Platform Governance for Salesforce Salesforce Jira
integration, including setting up the **[Jira Statuses](/docs/platgforsalesforce/integrations/jira_integration.md)** prior to using this
walkthrough.
...
3. Click **Create** (**+**).
    ...
    ![Create a Jira ticket](/img/product_docs/platgfornetsuite/integrations/jira_example_create_issue.webp)
...
1. Open the **Comments** tab and select **Strongpoint Salesforce**.
    ...
    ![Accessing the Strongpoint Salesforce app](/img/product_docs/platgforsalesforce/integrations/jira_comments_open_app.webp)

2. There are two ways to connect: **Login User** or **Connected App** tabs.

    ![Connection options](/img/product_docs/platgforsalesforce/integrations/connection_options.webp)
...
1. Open the **Connected App** tab.
    ...
    ![Using the connected app](/img/product_docs/platgforsalesforce/integrations/connection_options_app.webp)
...
> - If the error persists, check the [Callback URL](/docs/platgforsalesforce/integrations/jira_integration.md).
>
...
![Enter your Jira credentials](/img/product_docs/platgforsalesforce/integrations/jira_credentials.webp)

If you do not have your security token, you can use these steps to reset your token:

1. Log in to your Salesforce account.
2. Open **View Profile** > **Settings**.  
   ![Open your Salesforce Profile settings](/img/product_docs/platgforsalesforce/integrations/jira_example_settings.webp)
...
![Connection details for the ticket](/img/product_docs/platgforsalesforce/integrations/jira_connection.webp)
...
- **Select Customizations** is where you specify the Customizations you are changing or adding. If
  ...
  ![delete](/img/product_docs/platgfornetsuite/integrations/delete.webp) icon.
...
- **Push** creates the Change Request in Salesforce. **Push** is also used to manually update your
  Change Request if you are not using the [Automatic Synchronization](/docs/platgforsalesforce/integrations/jira_integration.md) feature.
...
2. Click **+** to search for matching Customizations. Hover over **View** to display the **Type**
   and **API Name** for a Customization.
    ...
    ![Add an existing customization](/img/product_docs/platgforsalesforce/integrations/jira_customization_add.webp)
...
4. Click **Add Selected Customizations**. The selected Customization is added to the **Existing
   Customizations** list.
    ...
    ![Add an existing customization](/img/product_docs/platgforsalesforce/integrations/jira_example_add_existing.webp)
...
6. The **API Name** is displayed. Select it, and click **Add Selected Customizations**. The selected
   ...
   **UpsellOpportunities (ApexClass)**, an API in the _Strongpoint Demo Org_.
    ...
    ![Add an existing customization by API Name](/img/product_docs/platgforsalesforce/integrations/jira_example_add_existing_api.webp)
...
2. Click (**+**) to add it. If the API Name is valid, and does not match an existing API Name, the
   new Customization is added to the **Proposed Customizations** list.
    ...
    ![Add a proposed customization](/img/product_docs/platgforsalesforce/integrations/jira_example_proposed.webp)
...
up the **[Jira Statuses](/docs/platgforsalesforce/integrations/jira_integration.md)** procedure.

1. Change the Jira status of your ticket to match the status set up for **CR In Progress**. For
   example, **In Progress**. Refer to setting up the **[Jira Statuses](/docs/platgforsalesforce/integrations/jira_integration.md)**
   procedure.
2. Click **Push** to create the Change Request if you are not using the
   [Automatic Synchronization](/docs/platgforsalesforce/integrations/jira_integration.md) feature. The change request is created in
...
3. Expand the **Change Request** field on the right. The status is now **None/In Progress**. There
   is a link to open the Change Request in Salesforce.
    ...
    ![Change Request is In Progress](/img/product_docs/platgforsalesforce/integrations/jira_example_in_progress_status.webp)
...
![Policy and Change Level Req have been updated](/img/product_docs/platgforsalesforce/integrations/jira_example_policy.webp)
...
![Impact analysis report](/img/product_docs/platgfornetsuite/integrations/jira_example_impact_analysis.webp)
...
The [Dependency Relationship Diagram](/docs/platgforsalesforce/tools/viewing_drd.md) (DRD) tool graphically displays your
...
![Use the DRD to explore dependencies](/img/product_docs/platgforsalesforce/integrations/jira_example_drd.webp)
...
2. Click **Push** if you are not using [Automatic Synchronization](/docs/platgforsalesforce/integrations/jira_integration.md) to push
   status changes.
3. Expand the **Change Request** field on the right. The status is now **Pending Approval / In
   Progress**. There is a link to open the Change Request in Salesforce.
    ...
    ![Change Request in Pending Approval](/img/product_docs/platgforsalesforce/integrations/jira_example_pending_approval_status.webp)

4. Click the **Go To Record** link to view the Change Request.

    ![Change Request is In Progress / Pending Approval](/img/product_docs/platgforsalesforce/integrations/jira_example_pending_approval_change_request.webp)
...
1. Expand the **Change Request** field on the right. The status is **Approved / In Progress**.
    ...
    ![Change Request is approved](/img/product_docs/platgforsalesforce/integrations/jira_example_approved_status.webp)

2. Change the Jira status of your ticket to match the status set up for **CR Approved**. For
   example, **Ready for Deployment**.
3. Click **Push** if you are not using [Automatic Synchronization](/docs/platgforsalesforce/integrations/jira_integration.md) to push
   status changes.
...
- **External last Modified by**: is the last user who modified the Jira ticket.
    ...
    ![user_guide_example](/img/product_docs/platgforsalesforce/integrations/user_guide_example.webp)
