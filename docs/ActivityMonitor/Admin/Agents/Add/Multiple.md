...
Before deploying the activity agent, ensure all Prerequisites are met, including those for NAS devices when applicable. Follow the steps to deploy the activity agent to a multiple Windows servers. See the [Activity Agent Server Requirements](/Requirements/ActivityAgent.md "Activity Agent Server Requirements") topic for additional information.
...
![Specify Agent Port page - specify port that should be used by new agent](/img/activitymonitor/admin/PortDefault.png "Specify Agent Port page - specify port that should be used by new agent")
...
![Install Agents on Multiple Hosts page](/img/activitymonitor/admin/InstallAgentsOnMultipleHosts.png "Install Agents on Multiple Hosts page")

**Step 4 –** Windows or Linux hosts can be entered as either a name or an IP Address. The options are:

- Add server — Opens the Host name or IP address window. See the [Manual Entry](#manual-entry "Manual Entry") topic for additional information.
- Remove — Removes an entered host name or IP address from the table
- Import — Opens the Import from file window. See the [Import a List](#import-a-list "Import a List") topic for additional information.
...
![Enter Host Name or IP Address window](/img/activitymonitor/admin/HostNameOrIPAddressWindow.png "Enter Host Name or IP Address window")
...
![Import Hosts from a CSV File window](/img/activitymonitor/admin/ImportHostsFromACSVFileWindow.png "Import Hosts from a CSV File window")
...
The Activity Monitor will monitor the Host Names or IP addresses added to the **Install Agents on Multiple Hosts** table. Click **Next**.

![Credentials to Connect to the Server(s) window](/img/activitymonitor/admin/Credentials.png "Credentials to Connect to the Server(s) window")
...
![Credentials to Connect to the Server(s) ](/img/activitymonitor/admin/PublicKey.png "Credentials to Connect to the Server(s)")
...
![](/img/activitymonitor/admin/ClientCertificate.png)
...
![Agent Installation Path page](/img/activitymonitor/admin/AgentInstallLocation.png "Agent Installation Path page")
...
![Multiple Agents Installed](/img/activitymonitor/admin/ADAgentInstalled.png "Multiple Agents Installed")

When the activity agent installation completes, the status changes to **Installed** and the activity agent version populates. The next step is to add hosts to be monitored. See the [Monitored Hosts Tab](/Admin/MonitoredHosts/Overview.md "Monitored Hosts Tab") topic for additional information.