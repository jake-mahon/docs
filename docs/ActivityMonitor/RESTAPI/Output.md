...
| userName | string | | User name for the AMQP connection, if needed. For Threat Manager integration, use an empty string. |
| password | string | | Password / App Token for the AMQP connection. Password / App Token is not exposed. |
| queue | string | | Message queue name to post events to. For Threat Manager integration, use an empty string. |
...
| vhost | string | | Virtual Host name, if needed. For Threat Manager integration, use an empty string. |
