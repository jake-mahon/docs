...
- Agents – There are three types of agents that are deployed in the target environment used to monitor activity:
  ...
  - Activity Agent – The Activity Agent is installed on Windows servers to monitor Microsoft Entra ID, Network Attached Storage (NAS) devices, SharePoint farms, SharePoint Online, SQL Server, and Windows file servers. See the [Activity Agent Server Requirements](ActivityAgent.md "Activity Agent Server Requirements") topic for additional information.
  - AD Agent – The AD Agent is deployed to every domain controllers to monitor Active Directory domains. See the [AD Agent Server Requirements](ADAgent.md "AD Agent Server Requirements") topic for additional information.
  - Linux Agent – The Linux Agent is deployed to Linux servers to be monitored. See the [Linux Agent Server Requirements](LinuxAgent.md "Linux Agent Server Requirements") topic for additional information.
