...
Note that Netwrix Data Classification supports only Windows OS versions listed in the [Software Requirements](SoftwareRequirements) section.
...
See [Deployment Planning](../Install/Deployment/Overview) and [Configuring NDC Servers Cluster and Load Balancing with DQS Mode](DQSMode) for more information.
...
| Hard disk | Estimate required disk space assuming *10 - 12 KB* per indexed object. For example, for *5, 000, 000* objects, the database size will be approximately *50 GB*. See also [Deployment Planning](../Install/Deployment/Overview). | | |
...
Administration Dashboards for most Network Attached Storages include storage usage such as the amount of data and files count. In case you need a tool to calculate file count and size, you can use NDC script-based [file scanning tool](https://www.netwrix.com/download/products/DDC/ScanDirs.zip "file scanning tool").
...
**CAUTION:** Netwrix Data Classification servers and SQL servers require high-performance SSD drives for optimal performance.
