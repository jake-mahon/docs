...
* Windows Server can be physical or virtual. See the [Virtual Environment Recommendations](Application#Virtual "Virtual Environment Recommendations")topic for additional information.
...
**NOTE:** For SQL Server 2012+, it is necessary to restrict the maximum server memory value to 60-70% of the total physical RAM to avoid a situation where SQL Server will starve other applications of memory. See the [Restrict SQL Server Maximum Server Memory](../Troubleshooting/SQLServer "Restrict SQL Server Maximum Server Memory") topic for additional information.
...
Additional permissions are required for the optional Database Maintenance feature in Threat Prevention. See the [Database Maintenance Feature Requirements](DBMaintenance "Database Maintenance Feature Requirements") topic for additional information.