...
* Ensure that Threat Prevention has been configured to send events to Splunk. See the [SIEM Tab](../../../Admin/Configuration/SystemAlerting/SIEM "SIEM Tab")topic for additional information.
...
**NOTE:** In order to use the User Behavior Analytics dashboard in the app, install [Splunk User Behavior Analytics](https://www.splunk.com/en_us/products/premium-solutions/user-behavior-analytics) (any version) and the [Machine Learning Toolkit](https://splunkbase.splunk.com/app/2890/) app for Splunk (version 2.0.0+).
...
*Remember,* prior to using the Active Directory App for Splunk, the relevant Netwrix product must be configured to send data to Splunk.

**Step 1 –** Determine the IP address of the Splunk console. If Splunk is hosted on a UNIX machine, run . If Splunk is hosted on a Windows machine, run . This IP address is required on the [SIEM Tab](../../../Admin/Configuration/SystemAlerting/SIEM "SIEM Tab"), where you configure Threat Prevention to send data to Splunk.
...
**Step 8 –** To test that the configuration is working correctly, check the Search and Reporting app inside of the web console for Splunk (search for Threat Prevention). There should be sample logs with KEY=%VALUE% pairs in the Event column. If there are no events, use a packet sniffer to ensure that packets are being sent correctly between the two hosts, and diagnose any possible network issues.
