...
* Ensure that Threat Prevention has been configured to send events to QRadar. See the [SIEM Tab](../../Admin/Configuration/SystemAlerting/SIEM "SIEM Tab") topic for additional information.
...
**Step 5 –** Close the Extensions Management window, and then select the Active Directory tab in QRadar.

The Active Directory tab will appear in QRadar. It is necessary for the QRadar SEC token to be saved to the Settings interface of the Active Directory App. See the [Settings](Settings) topic for additional information.
...
*Remember,*  prior to using the Active Directory app for QRadar, Threat Prevention must be configured to send data to QRadar.

**Step 1 –** Determine the IP address of the QRadar console. If QRadar is hosted on a UNIX machine, run . If QRadar is hosted on a Windows machine, run . This IP address is required on the [SIEM Tab](../../Admin/Configuration/SystemAlerting/SIEM "SIEM Tab"), where you configure Threat Prevention to send data to QRadar.

**Step 2 –** Navigate to the Admin tab in the QRadar web interface and click Data Sources.

**Step 3 –** Select **Log Sources**.

**Step 4 –** View the Log Sources list. If the data source was not automatically created, click Add, enter the following information, and click Save.
