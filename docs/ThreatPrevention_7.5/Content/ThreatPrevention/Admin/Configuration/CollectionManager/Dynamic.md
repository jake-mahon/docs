...
Certain options in the upper-left corner of the [Add New Collection Window](ListCollections#_Add_New_Collection "Add New Collection Window") are only enabled for the following collection categories:

* Domains & Servers – [Dynamic Domains & Servers Collection Table Requirements](#_Dynamic_Domains_& "Navigates to the Dynamic Domains & Servers Collection Table Requirements section in the same topic. ")
* Objects – [Dynamic Objects Collection Table Requirements](#_Dynamic_Objects_Collection "Navigates to the Dynamic Objects Collection Table Requirements section of the same topic. ")
* Perpetrators and Lockdown Perpetrators – [Dynamic Perpetrators Collections Table Requirements](#_Dynamic_Perpetrators_Collections "Navigates to the Dynamic Perpetrators Collections Table Requirements section of this topic. ")
* IP Addresses – [Dynamic IP Addresses Collection Table Requirements](#_Dynamic_IP_Addresses "Navigates to the Dynamic IP Addresses Collection Table Requirements in the same topic.")
* Hosts – [Dynamic Hosts Collection Table Requirements](#_Dynamic_Hosts_Collection "Navigates to the Dynamic Hosts Collection Table Requirements section of the same topic. ")
* File Paths – [Dynamic File Paths Collection Table Requirements](#_Dynamic_File_Paths "Navigates to the Dynamic File Paths Collection Table Requirements section of the same topic. ")

![Options on the Add New Collection window](../../../../../../../static/images/ThreatPrevention_7.5/Content/Resources/Images/ThreatPrevention/Collections/AddCollectionOptions.png "Options on the Add New Collection window")

* The *I will provide a list* option button enables the default setting for a static collection. See the [Add New Collection Window](ListCollections#_Add_New_Collection "Add New Collection Window") topic to manually add a new collection.
* The *I want a list to come from the database table* option button enables a dynamic collection. Enter the table name in the textbox that appears or select it from the drop-down menu.
...
Any policy that has been assigned a dynamic collection uses the current table’s data for the policy filter, resulting in a dynamic policy.

Any changes to the selected table’s data are recognized by an active policy when the Agent communicates with the Threat Prevention Enterprise Manager. This may result in a maximum delay of five minutes between a change in the table and the Agent refresh.
...
**NOTE:** Threat Prevention creates an empty table with the required prefix and schema if the [Table name] entered does not exist in the NVMonitorConfig database.
