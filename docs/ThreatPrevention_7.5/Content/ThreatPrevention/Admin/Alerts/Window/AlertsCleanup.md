...
You can clear alert data displayed on the [Alerts Interface](../Overview "Alerts Interface") as well as schedule cleanups for this data.

***RECOMMENDED:*** Export alert data before using the Clear option. See the [Alerts Export Window](AlertsExport "Alerts Export Window") topic for additional information.
...
![Alerts Cleanup window](../../../../../../../static/images/ThreatPrevention_7.5/Content/Resources/Images/ThreatPrevention/Alerts/AlertsCleanup.png "Alerts Cleanup window")

**Step 3 –** The Alerts Cleanup window has these options:

* Delete – Removes alert data from the database. Select **All** to delete all alert data or select **Older than** and specify a certain number of days to delete alerts older than the specified number of days.
* Log Level – Deletes alerts that have the log levels that are checked. The log levels are equivalent to the alert severity levels in the data grid on the [Alerts Interface](../Overview "Alerts Interface").
...
**Step 5 –** Click **Save** to save any changes or **Close** to discard the changes and close the window.
