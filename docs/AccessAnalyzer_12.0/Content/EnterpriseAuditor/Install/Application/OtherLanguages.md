...
* Cannot resolve the collation conflict between **SQL\_Latin1\_General\_CP1\_CI\_AS** and **French\_CI\_AS** in the equal to operation.
...
![SQL Server Management Studio Database Properties window](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Install/Database/DatabasePropertiesCollation.png "SQL Server Management Studio Database Properties window")
...
![SQL Server Configuration Manager](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Install/Database/SQLServerConfigurationManager.png "SQL Server Configuration Manager")
...
```
See the Microsoft [Collation and Unicode support](https://learn.microsoft.com/en-us/sql/relational-databases/collations/collation-and-unicode-support "Collation and Unicode support") article for collation matches.
...
Access Analyzer does not support case sensitive collation settings. Case insensitive collations are notated by having **CI** in the collation, for example **Latin1\_General\_CI\_AS**.
...
```
* Error Returned – Cannot resolve the collation conflict between **Greek\_CI\_AS** and **French\_CI\_AS** in the equal to operation.
...
SELECT * FROM TestTab WHERE CharCol = CharCol2 COLLATE Albanian_CI_AI
  ```
**NOTE:** Explicit collation (Albanian\_CI\_AI) is not one of any column, but after that it will complete successfully. The collation of two columns have not been matched, instead the third rule of collation precedence was implemented. See the Microsoft [Collation Precedence](https://learn.microsoft.com/en-us/sql/t-sql/statements/collation-precedence-transact-sql "Collation Precedence") article for additional information.

### Resources

The following articles may be of assistance:

* Microsoft [Collation and Unicode support](https://learn.microsoft.com/en-us/sql/relational-databases/collations/collation-and-unicode-support "Collation and Unicode support") article
* Microsoft [Collation Precedence](https://learn.microsoft.com/en-us/sql/t-sql/statements/collation-precedence-transact-sql "Collation Precedence") article
* Microsoft [Set or change the server collation](https://learn.microsoft.com/en-us/sql/relational-databases/collations/set-or-change-the-server-collation "Set or change the server collation") article