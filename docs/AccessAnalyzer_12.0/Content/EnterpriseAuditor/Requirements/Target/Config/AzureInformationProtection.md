...
1. [Rights Management Service Client Installation](#Rights_Management_Service_Client_Installation "Navigates to the Rights Management Service Client Installation section")
2. [Create a Service Principal Account using PowerShell](#Create_a_Service_Principal_Account "Navigates to the Create a Service Principal Account section")
3. [Enable the Account as an AIP Super User using PowerShell](#Enable_the_Account_as_an_AIP_Super_User "Navigates to the Enable the Account as an AIP Super User section")
4. [Add User to the AIP Role in Microsoft® Azure](#Add_User_to_the_AIP_Roln_in_Microsoft%C2%AE_Azure "Navigates to the Add User to the AIP Role in Microsoft® Azure section")
5. [Azure Information Protection Target Requirements](#Access "Access Analyzer Configurations")
6. Enable settings in FSAA Data Collector in Access Analyzer.

   * See the FileSystemAccess Data Collector section in the [File System Solution](../../../Solutions/FileSystem/Overview "File System Solution") topic for additional information
...
***RECOMMENDED:*** Read the System Requirements and Install Instructions provided by Microsoft to complete the installation.
...
```
**Step 3 –** Connect to Azure with the `Connect-MsolService` command. Enter the Azure credentials in the **Sign in to your account** window that displays from Microsoft.

**Step 4 –** Once successfully connected to Azure, create a service principal with the following command:

```
New-MsolServicePrincipal
```
> Enter the **DisplayName** of the new service principal name. (For example, AIP\_EnterpriseAuditor)

**Step 5 –** Take note and save the **Symmetric Key** and **AppPrincipalID** to be used in later steps.

**CAUTION:** Do not lose the symmetric key. It is not retrievable again once the PowerShell window is closed.
...
**Step 3 –** Add the service principal to the Azure AD Rights Management service super users, using the AppPrincipalID saved from the steps in the [Create a Service Principal Account using PowerShell](#Create_a_Service_Principal_Account "Navigates to the Create a Service Principal Account section") section:

```
Add-AipServiceSuperUser-ServicePrincipalID 
```
...
**Step 1 –** Log into **http://portal.azure.com** with Azure credentials and select **Microsoft Entra ID** from the list of resources on the left-hand pane.

**Step 2 –** Navigate to **Roles and Administrators**. On the Administrative Roles page, select the **Azure Information Protection Administrator** role.

**Step 3 –** Use the **Add Assignment** button to display the Add assignments pane. Search for the name of the new service principal account (the **DisplayName** entered in PowerShell) and add it to the list of assignments.
...
* [Azure Connection Profile](#_Azure_Connection_Profile "Azure Connection Profile")
* [Configure FSAA Data Collector](#_Configure_FSAA_Data "Configure FSAA Data Collector")

### Azure Connection Profile

To collect tags for files protected with Azure Information Protection, an Azure connection profile must be configured in Access Analyzer before an FSAA scan runs. See the [Global Settings](../../../Admin/Settings/Overview "Global Settings") topic for additional information on how to set up a connection profile at the global level.

**Step 1 –** In Access Analyzer, add a credential for an Azure Active Directory account type to the existing Connection Profile used for File System scanning. Supply the Client ID field with the **AppPrincipalID** and the Key field with the **Symmetric key** created upon creation of the new service principal.
...
**Step 3 –** Ensure that the job is configured correctly before running a scan. See the [Configure FSAA Data Collector](#_Configure_FSAA_Data "Configure FSAA Data Collector") topic for additional information.
...
* Azure Tenant Mapping page – Add the **AppPrincipalID** (App ID) and the **Domain Name** or **Tenant ID**
...
See the FileSystemAccess Data Collector section in the [File System Solution](../../../Solutions/FileSystem/Overview "File System Solution") topic for additional information on these scoping options.