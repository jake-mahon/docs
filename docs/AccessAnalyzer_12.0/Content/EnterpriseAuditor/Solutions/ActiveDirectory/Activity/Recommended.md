...
Connection Profiles must be set directly on the [0.Collection > AD\_ActivityCollection Job](AD_ActivityCollection "0.Collection > AD_ActivityCollection Job") in order to connect to either the SAM API Server or the host with the network share housing the archived log files.

Access Token

Required for SAM API Server integration for the [0.Collection > AD\_ActivityCollection Job](AD_ActivityCollection "0.Collection > AD_ActivityCollection Job").
...
**Step 5 –** Configure the **AD\_ActivityCollection** Job Query.
...
See the [Active Directory Solution](../../../Requirements/Solutions/ActiveDirectory "Active Directory Solution") topic for additional information.