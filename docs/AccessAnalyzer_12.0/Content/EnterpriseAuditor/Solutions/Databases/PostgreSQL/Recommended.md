...
title: "Recommended Configuration for the PostgreSQL\_Solution"
---
...
* For AWS RDS and Aurora instances, right-click a job in the **PostgreSQL** > **0.Collection** folder and open the properties window. Select the **Performance** tab and ensure that the **Skip Hosts that do not respond to PING**checkbox is not selected.
...
This solution is designed to be run with the default query configurations. However, the PostgreSQL\_SensitiveDataScan Job query can be customized as needed. See the [Configure the SensitiveDataScan Query](Collection/PgSQL_SensitiveDataScan#Configure_the_SensitiveDataScan_Query "Configure the SensitiveDataScan Query") topic for additional information.
