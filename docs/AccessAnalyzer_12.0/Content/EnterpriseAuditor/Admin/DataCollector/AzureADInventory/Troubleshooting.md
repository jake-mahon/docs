...
Update the `` parameter to update the number of retries to run the query. The default is 3.

Error: An existing connection was forcible closed by the remote host

Update the `` parameter to update the max delta token age. The default is 6.

See the [View Job XML File](../../Jobs/Job/Properties/ViewXML "View Job XML File") topic for additional information.