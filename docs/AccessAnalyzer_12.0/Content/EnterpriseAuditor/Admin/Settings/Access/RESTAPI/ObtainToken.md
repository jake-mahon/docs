...
```
***RECOMMENDED:*** Tokens contain sensitive information and should be stored securely. See the Microsoft [ConvertTo-SecureString](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.security/convertto-securestring?view=powershell-7.4 "ConvertTo-SecureString") article for additional information.
...
The Client Secret expires after 72 hours. The access token expires after 1 hour after which time you can request a refresh token. See the [Use the Client Credentials to Grant a Refesh Token](RefreshToken "Use the Client Credentials to Grant a Refesh Token") topic for additional information.