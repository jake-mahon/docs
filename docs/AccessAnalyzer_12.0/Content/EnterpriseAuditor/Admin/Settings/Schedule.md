...
![Schedule node](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/Schedule.png "Schedule node")

Jobs can be executed manually as desired or scheduled to execute at designated times. For example, you could schedule a job to run during hours when the office is closed and network traffic is low. Windows uses the Schedule Service Account to access the task folders when launching scheduled tasks. Schedule Service Accounts are configured at the global level, and this account can be used to schedule jobs in the Schedule Wizard. See the [Schedules](../Schedule/Overview "Schedules") topic for additional information.

**CAUTION:** On Windows 2016 servers, the Schedule Service Account cannot be signed into an active session when the time comes for a scheduled task to start. Windows blocks the starting or running of scheduled tasks using an account that is logged into the server.
...
Choosing between the Access Analyzer application and Access Analyzer Vault is a global setting configured in the **Settings** > **Application** node. See the [Application](Application/Overview "Application") topic for additional information.
...
* To configure Least Privilege Model Schedule Service Accounts when Role Based Access is enabled, see the [Role Based Access](Access/RoleBased/Overview "Role Based Access") topic for additional information
* If using Windows authentication for the Storage Profile, the Schedule Service Account must have a sufficient level of rights to connect to and interact with the Access Analyzer database. See the [Storage](Storage/Overview "Storage") topic for additional information.
...
![](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/ServiceAccountTypes.png)
...
*Remember,*  the Schedule Service Account cannot be signed into an active session on the Access Analyzer Console server when the time comes for a scheduled task to start when it has a Windows 2016 operating system.
...
![Add User credential option in the Schedule view](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/AddUserCredential.png "Add User credential option in the Schedule view")

**Step 1 –** Click **Add User credential** at the top of the Schedule view. The User Credentials window opens.

![User Credentials window](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/UserCredentialsWindow.png "User Credentials window")
...
* Active Directory Account – Use this option to specify a domain account
  ...
  * Password Storage – Choose the option for credential password storage:
    ...
    * Managed Service Account – Use previously configured MSA and gMSAs for authentication. The password fields are not applicable when this option is selected. See the [Group Managed Service Accounts (gMSA) Configuration](Connection/gMSA) Configuration") topic for additional information.
...
**Step 3 –** Click **OK** and the credentials are verified. If there are no problems with the provided credentials, the User Credentials window closes. Otherwise, one of the following error messages might appear:

* Passwords Do Not Match Error

  ![Passwords Do Not Match Error](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/PasswordsDontMatch.png "Passwords Do Not Match Error")

  * This error indicates the two password entries do not match. Click **OK** and reenter the passwords.
* Bad User Name or Password Error

  ![Bad User Name or Password Error](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/IncorrectLogonDetails.png "Bad User Name or Password Error")

  * This error indicates either the user account does not exist or the username and password do not match. Click **OK** and reenter the information.
* Insufficient Rights Error

  ![Insufficient Rights Error](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/InsufficientRights.png "Insufficient Rights Error")

  * This error indicates the account supplied does not have sufficient rights to create and run scheduled tasks. Click **OK** and provide credentials with sufficient rights.
* GPO Network Security Error

  ![GPO Network Security Error](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/GPONetworkSecurity.png "GPO Network Security Error")
...
![Edit option in the Schedule view](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/Edit.png "Edit option in the Schedule view")

**Step 1 –** Select a credential from the User Credentials list and click on **Edit**. The User Credentials window opens.

**Step 2 –** Modify the credential information as needed. See Step 2 of the [Create a Schedule Service Account](#_Create_a_Schedule_1 "Create a Schedule Service Account") topic for additional information.

**Step 3 –** Click **OK** and the credentials will be verified. If there are no problems with the provided credentials, the User Credentials window closes.
...
![Delete option in the Schedule view](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/Delete.png "Delete option in the Schedule view")
...
![Delete Credentials confirmation window](../../../../../../static/images/AccessAnalyzer_12.0/Content/Resources/Images/EnterpriseAuditor/Admin/Settings/Schedule/DeleteCredentials.png "Delete Credentials confirmation window")
