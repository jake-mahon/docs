...
A workflow is made of successive activities, each of which is assigned an [Activity Templates](/docs/usercube_saas/usercube/integration-guide/workflows/activity-templates/index.md)that defines how transitions occur from a workflow step to another.

Activities never run in parallel in a workflow. Each activity can start once the previous one reached its final state.

## Forms

Workflows use [Form](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/user-interface/form/index.md) to collect input data through the UI.
...
For each workflow, some permissions must be assigned to specific [Profile](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/access-control/profile/index.md) so that said profiles are entitled to realize the workflow's actions.
...
[Menu Item](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/user-interface/menuitem/index.md) make workflows accessible from the UI.
...
An [Aspects](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/workflows/aspects/index.md) definition allows an action to be performed at a specific point in a workflow. Identity Manager provides a few [Aspects](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/workflows/aspects/index.md) templates that give the opportunity to delegate administration, to notify people of a request's progress and to compute special values like unique logins or email addresses.
...
A homonym search checks if a resource already exists in the system before creating/modifying it, preventing duplicates. It is configured through a [Homonym Entity Link](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/workflows/homonymentitylink/index.md).

See the [Configure a Homonym Detection](/docs/usercube_saas/usercube/integration-guide/workflows/how-to/configure-homonym-test/index.md)topic for additional information.
...
See the [Display Table](/docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/user-interface/displaytable/index.md) topic for additional informatrion.
