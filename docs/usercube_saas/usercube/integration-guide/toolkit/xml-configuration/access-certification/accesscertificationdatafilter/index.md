...
| IncludeManualAssignmentNotAllowed   default value: true | __Type__    Boolean   __Description__   ```true``` to include in the certification the resources that cannot be requested manually, i.e. those from resource types with ```ApprovalWorkflowType``` set to ```ManualAssignmentNotAllowed```. See the [Resource Type](/docs/product_docs/usercube_saas/usercube/integration-guide/toolkit/xml-configuration/provisioning/resourcetype/index.md) topic for additional information. |
