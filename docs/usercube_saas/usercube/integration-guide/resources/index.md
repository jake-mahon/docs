...
1. Input data directly from the applicative configuration. This is useful for a very limited amount of data. This is very often used for debugging or testing, less often in production. See the [Toolkit for XML Configuration](/docs/product_docs/usercube_saas/usercube/integration-guide/toolkit/index.md) topic for additional information.
...
When using methods 1. and 5., make sure to choose, for new resources, an ```Id``` that is not yet used for another resource in the database. Only use positive integer ```Id```s for resource-identity (that is, the resource to which you plan on assigning roles). See the [Entitlement Assignment](/docs/product_docs/usercube_saas/usercube/integration-guide/role-assignment/assignments-of-entitlements/index.md) topic for additional information.
