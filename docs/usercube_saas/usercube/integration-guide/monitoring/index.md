...
See the [
References: Logs
](/docs/product_docs/usercube_saas/usercube/integration-guide/monitoring/references/index.md) topic for additional information on the list of existing logs.
...
When using ```Serilog.Sinks.File```, the setting ```shared``` should be set to ```true``` in the ```Args``` section to enable Identity Manager's __Monitoring__ screen functionality.
  
As this ```shared``` setting allows several systems to interact with the log file simultaneously, so we can have both Serilog writing to the log file and Identity Manager reading it to display its content on the __Monitoring__ screen.
...
](/docs/product_docs/usercube_saas/usercube/integration-guide/monitoring/how-tos/qradar-setting/index.md) topic to learn how to send Identity Manager's logs to your QRadar system.
...
Identity Manager offers the ability to download the application logs directly through the User Interface (UI) via the __Monitoring__ screen in the __Administration__ section on the Dashboard.
...
This example shows how to reduce the overhead of the logging process for Identity Manager's main thread by delegating work to a background thread, using the _Async__Sink_.
