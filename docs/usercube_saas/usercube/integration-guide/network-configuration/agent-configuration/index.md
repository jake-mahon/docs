...
1. The [Application Settings](appsettings/index.md) set is written either to the Agent's working
   directory appsettings.json file or as environment variables. See the
   [ Architecture ](../../architecture/index.md) topic for additional information.
2. The [appsettings.agent](appsettings-agent/index.md) set is written as environment variables or to
...
3. There are two additional files involved in the _Agent_'s configuration to protect sensitive data:
   ...
   [ RSA Encryption ](rsa-encryption/index.md) and
   [CyberArk's AAM Credential Providers ](cyberark-application-access-manager-credential-providers/index.md)topics
