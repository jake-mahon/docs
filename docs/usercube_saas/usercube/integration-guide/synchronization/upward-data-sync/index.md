...
Upward Data Synchronization (Sync Up) is the process that copies relevant managed systems data into Identity Manager's resource repository and translates them into resources that match the configured Entity Model. See the [Entity Model](../../entity-model/index.md) topic for additional information.
...
- ascertains differences between the real managed system state and the [
  Assignment Policy
  ](../../role-model/role-model-rules/index.md) theoretical state.
...
A _Sync Up_ is associated with a [Connectors](../../connectors/index.md).
...
During this step, data is extracted from the managed system and generates _CSV files_ containing the managed system's raw data. The __output__ of this process is called the ___CSV source files___. They are written to the [Application Settings](../../network-configuration/agent-configuration/appsettings/index.md) export directory waiting to be used by the next-in-line _prepare-synchronization task_.
...
Identity Manager's [Connectors](../../connectors/index.md) provide native _Export_ tasks for the most common managed systems. _Active Directory_, _SAP_, or _SharePoint_ are examples of natively supported managed systems. The output _CSV source files_ format is described in the [Connectors](../../connectors/index.md) section together with an exhaustive list of supported source managed systems.

[Connectors](../../connectors/index.md)are Identity Manager's link to the managed system. They provide configurable export and fulfill capabilities that can be used by Identity Manager _as-is_ without any further development.
...
](../../toolkit/xml-configuration/jobs/tasks/agent/invokeexpressiontask/index.md), can then be used to retrieve the generated exports, adapt them to the _CSV source files_ format expected by Identity Manager and copy them to the [Application Settings](../../network-configuration/agent-configuration/appsettings/index.md) export directory. The whole can be scheduled and orchestrated by a [
Jobs
...
Invoke Expression Task
](../../toolkit/xml-configuration/jobs/tasks/agent/invokeexpressiontask/index.md) or an [
Invoke Sql Command Task
](../../toolkit/xml-configuration/jobs/tasks/server/invokesqlcommandtask/index.md). See the [
Invoke Expression Task
](../../toolkit/xml-configuration/jobs/tasks/agent/invokeexpressiontask/index.md) topic for additional information. Any Windows process that can be called from a PowerShell script and generate a CSV file can serve as an export process.
...
Inside those constraints, every natively supported _export task_ generates its own _CSV source file format_, described in the [Connectors](../../connectors/index.md) section. Usually, two kinds of files are generated: _entries_, describing plain entries, and _associations_, describing associations between entries.

All _CSV source files_ are written to the [Application Settings](../../network-configuration/agent-configuration/appsettings/index.md) export directory.
...
The [Application Settings](../../network-configuration/agent-configuration/appsettings/index.md) export directory can also contain opaque [cookie files](https://ldapwiki.com/wiki/DirSync) used for incremental export of a few systems such as Active Directory, Microsoft Entra ID, ServiceNow, and SCIM.
...
Export Task
](../../toolkit/xml-configuration/jobs/tasks/agent/exporttask/index.md) task within a Job.
...
![Active Directory Export Example](../../../../../../static/img/product_docs/usercube/usercube/integration-guide/synchronization/upward-data-sync/ad_export_example.webp)
...
The aim of the _Sync Up_ is to load managed systems' data into the resource repository. As such, it requires Identity Manager to translate data from the managed system format (or, more accurately, the _export task_'s output format) into the resource repository format, that is, the [Entity Model](../../entity-model/index.md).

The translation rules are described in the applicative configuration by [
Entity Type Mapping
](../../toolkit/xml-configuration/connectors/entitytypemapping/index.md) and [
Entity Association Mapping
](../../toolkit/xml-configuration/connectors/entityassociationmapping/index.md) elements.

Entity Type Mapping elements map the resources _CSV source files_ columns to [Entity Model](../../entity-model/index.md) properties. Each mapping also identifies one column as the _primary key_ for this Entity Type. The _primary key_ is used to uniquely identify a resource in the _Sync Up_ process. It's mandatory to be able to perform _incremental__Sync Up_, as it allows to identify a resource on which an _update_ or a _delete_ has to be performed.

[
Entity Association Mapping
](../../toolkit/xml-configuration/connectors/entityassociationmapping/index.md) elements translate the _CSV source files_ into [Entity Model](../../entity-model/index.md). They describe rules identifying associations between resources loaded thanks to the [](../../toolkit/xml-configuration/connectors/entitytypemapping/index.md)[
Entity Type Mapping
...
1. Removing columns that are not used in [
   Entity Type Mapping
   ](../../toolkit/xml-configuration/connectors/entitytypemapping/index.md) or [
   Entity Association Mapping
...
The result of the _Prepare-Synchronization_ is stored in the [Application Settings](../../network-configuration/agent-configuration/appsettings/index.md) export directory as three files:

For every entity type of the relevant _Connector_ involved in an[
Entity Type Mapping
](../../toolkit/xml-configuration/connectors/entitytypemapping/index.md) or an[
Entity Association Mapping
](../../toolkit/xml-configuration/connectors/entityassociationmapping/index.md) ```<EntityAssociationMapping>``` , a ```.sorted.csv``` file is generated, containing the final, cleaned, sorted result.
...
If the export step has provided a full extract of the managed systems, the _prepare-synchronization_ step computes changes. This computation is based on the result of the last data cleansing, generated by the previous _prepare-synchronization_, and stored in the ```previous``` folder in the [Application Settings](../../network-configuration/agent-configuration/appsettings/index.md) export directory.
...
- [
  ...
  ](../../toolkit/xml-configuration/jobs/tasks/agent/preparesynchronizationtask/index.md) is the standard _prepare-synchronization_ task.
...
![Active Directory Prepare-Synchronization Example](../../../../../../static/img/product_docs/usercube/usercube/integration-guide/synchronization/upward-data-sync/ad_preparesynchro_example.webp)
...
Connector
](../../toolkit/xml-configuration/connectors/connector/index.md), that are in the database but not in the _CSV source files_ will be deleted. That means no change made to the database from outside of the connectors or the UI are persistent.
...
- [
  Synchronize Task
  ](../../toolkit/xml-configuration/jobs/tasks/server/synchronizetask/index.md) is the standard _synchronization_ task.
...
![Active Directory Synchronization Example](../../../../../../static/img/product_docs/usercube/usercube/integration-guide/synchronization/upward-data-sync/ad_synchro_example.webp)
...
_Incremental_ mode also offers another optimization that will be described in the [Evaluate Policy](../../role-assignment/evaluate-policy/index.md) section. Trade-offs of that optimization can also be counterbalanced by running a daily _complete_ synchronization.
...
Connector
](../../toolkit/xml-configuration/connectors/connector/index.md), an [
Entity Type Mapping
](../../toolkit/xml-configuration/connectors/entitytypemapping/index.md) and/or an[
Entity Association Mapping
...
Assignment Policy
](../../role-model/role-model-rules/index.md).
