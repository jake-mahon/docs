...
![](../../../../static/images/enus/Content/Resources/Images/StrongpointJiraFlow.png "Jira Integration Process Flow")

## Set Up the Integration

To set up the Strongpoint Jira integration:

1. [Review Your Jira Statuses](#Configur2)
2. [Install the Netsuite Bundle](#Install)
3. [Configure NetSuite and Jira](#Configur)
4. [Install the App](#Install2)
5. [Create an Integration User Account (optional)](#IntegrationUser)
6. [Set Up Token Based Authentication](#Set)
7. [Jira Integration Deployment](#Jira)
...
4. Enter your **Jira User name** and the **Jira Token**.  Refer to the [Atlassian documentation](https://confluence.atlassian.com/cloud/api-tokens-938839638.html "Open the Atlassian article for information on obtaining a token") to obtain a Token
5. Enter the mappings between your Jira statuses and the Change Request statuses. For best practice, define your [Jira statuses](#Configur2 "Define your Jira statuses before mapping to Change Request statuses") prior to this step. You can enter multiple Jira statuses for each Change Request status, separated by commas. For example, **Ready for Deployment,Done**. Assign your mapping for each of the Change Request statuses:
...
6. Check **Automatic Synchronization** to enable Platform Governance for NetSuite to handle the synchronization between Jira and NetSuite when a Ticket status is updated to a mapped status. without having to click **Push**. If you are using an account where you do not want to create a Change Request, leave this unchecked so you can do your research or testing without generating Change Requests.
   ...
   ![](../../../../static/images/enus/Content/Resources/Images/jira_example_integration.png "Jira Integration settings with mapped statues")
...
13. Open the **Parameters** tab and enter the domain you use to access Jira.
    ...
    ![](../../../../static/images/enus/Content/Resources/Images/jira_parameters.png "Enter your domain on the Parameters tab")
14. Click **Save**.
...
1. From Jira, open **Settings** > **Apps**.
   ...
   ![](../../../../static/images/enus/Content/Resources/Images/jira_settings_cloud.png "Open Jira Apps settings")
2. Search for *Strongpoint for NetSuite*.

   ![](../../../../static/images/enus/Content/Resources/Images/jira_search.png "Jira Search.")
3. Click on the **Strongpoint for NetSuite** tile to open the details and follow the installation prompts.

### Create an Integration User Account (optional)

Consider creating an *Integration User* for your users to enter for the Jira Credentials. For accounts that use Two Factor Authentication (2FA) or Single Sign On (SSO), the *Integration User* can be configured with a limited role not requiring 2FA or SSO. The only requirement is the user must have access granted for the **Strongpoint Developer Role**.

### Set Up Token Based Authentication

Token Based Authentication is set up through NetSuite. Here is the basic process:

1. NetSuite Administrator creates an Integration Record for the Platform Governance for NetSuite application. This only needs to be performed once per account. Open **Setup** > **Integration** > **Integration Management** > **Manage Integrations** > **New**  
   Refer to the NetSuite help [Create Integration Records for Applications to Use TBA](https://netsuite.custhelp.com/app/answers/detail/a_id/82077/kw/tba "Open the NetSuite Help topic.").   
   To view a list of integration records in this account, open **Setup** > **Integration** > **Integration Management** > **Manage Integrations**.
2. Each user creates their token in NetSuite. Refer to the [NetSuite help Manage TBA Tokens in the NetSuite UI](https://netsuite.custhelp.com/app/answers/detail/a_id/41902 "Use the NetSuite help to create user tokens") for details. Once you have created your tokens, add them to Jira.
3. Open **Jira**.
...
7. Click **New Token Based Authentication** to add your credentials. This needs to be done once for each of your accounts.
...
3. Add customization/s(existing or proposed) to the Jira ticket:
   ...
   * Two ways to add: (a.) Click on **Lookup Customization** or **Proposed Customization** to narrow down customizations to attach. (b.) Click on **Import Customization** , add customizations in bulk from a file. File format accepted are XML and ZIP files. File contents for XML should have the same format as the MANIFEST.xml file in the SDF Project. SDF zip format uses the SDF structure as extracted from an existing SDF project.
...
4. Click **Push** when complete. Platform Governance for NetSuite creates a Change Request using the changes in the Jira ticket.
5. Start a Respider to create the Change Log and documents.

**Next Step:**  [Jira Walkthrough Example](jira_walkthrough_example "Click to open the Jira Walkthrough Example.")