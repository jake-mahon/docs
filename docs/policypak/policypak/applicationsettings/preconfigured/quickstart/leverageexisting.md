...
__Step 1 –__ In order to locate the ```pp-WinZip.DLL```, pull up the directory named Production-PAKS. Inside, you will find the WinZip International LLC WinZip 14 to 17 folder. You should see a file named ```Winzip 14 to 17.XML``` and``` pp-WinZip 14 to 17.dll```.
...
__NOTE:__ If you don't see the ```PolicyPak\Extensions``` folder, you may not have installed the Endpoint Policy Manager Admin Console.MSI (32-bit or 64-bit). This must be done on the machine where you manage Group Policy, and you must have the GPMC installed.
