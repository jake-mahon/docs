...
![policypak_application_settings_1_19](../../../../../static/img/product_docs/policypak/policypak/applicationsettings/modes/policypak_application_settings_1_19.webp)

Figure 40. A GPO that affects Computer accounts.

_Remember,_  that in order for Switched policies to apply, the GPO must be linked to an organizational unit (OU) containing computers, and the chosen computer must be in that OU to retrieve the GPO. Switched mode logs are generated automatically when users log in and when Group Policy reapplies in the background.
