...
On the computer side, under "PolicyPak/Applications/New/Application" we'll pick [https://www.policypak.com/products/manage-mozilla-firefox-with-group-policy.html](https://www.policypak.com/products/manage-mozilla-firefox-with-group-policy.html) Now remember, PolicyPak isn't delivering the application. That's what Mirage is going to do. PolicyPak is going to deliver and enforce the application's settings.

If we pick "www.vmware.com" as the [https://dev.policypak.com/](https://dev.policypak.com/) we can also right click and "Lockdown this setting using the system-wide config file" so now the users can't work around it. Under "Security," we'll make sure that these checkboxes are always checked and also "Lockdown this setting using the system-wide config file." I'll go ahead and lockdown two out of the three of them.
