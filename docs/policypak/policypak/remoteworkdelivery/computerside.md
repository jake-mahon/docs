...
![getting_to_know_policypak_34](../../../../static/img/product_docs/policypak/policypak/remoteworkdelivery/getting_to_know_policypak_34.webp)
...
__NOTE:__ Scripts placed on the Computer side, but run in the User context can be run interactively. But scripts run in the System context cannot be run interactively.
...
- All policies with __Always run__ selected will reapply when policy changes are made.
- All policies with __Always run__ selected will reapply when the Endpoint Policy Manager service starts up.
