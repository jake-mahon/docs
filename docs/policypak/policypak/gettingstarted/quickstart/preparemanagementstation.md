...
__TIP__: Use the instructions in the [What are the two ways that can I install the GPMC on my Admin Station (Server or Windows 10) machine?](../../install/methods.md) topic if you do not yet have the GPMC on your management station.

__NOTE:__ The Endpoint Policy Manager MMC Group Policy Snap-In does NOT require installation on a Domain Controller, it is simply an option. See the [Does Endpoint Policy Manager admin console need to be installed on Domain Controller (DC)?](../../install/adminconsole.md) topic for details.
