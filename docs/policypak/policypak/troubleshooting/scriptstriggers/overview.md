...
__Step 1 –__ Start with troubleshooting to verify that you are set up with the following scenarios:
...
__Step 2 –__ If needed, logs are automatically wrapped up and can be sent to [support@policypak.com](mailto:support@policypak.com) using the ```PPLOGS.EXE``` command on any endpoint where the client-side extension is installed.
