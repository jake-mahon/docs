...
![Status Node](/img/product_docs/accessanalyzer/enterpriseauditor/admin/jobs/job/statusnode.webp)
...
- Task Stats table – Provides information for each task run during job execution, according to the global configuration set in the __Settings__ > __Application__ node. By default, this is set to filter to the most recent data.
  ...
  __NOTE:__ The Job Statistics Retention settings in the __Settings__ > __Application__ node control how long the job statistics history is kept in the database and displayed Job Stats and Task Stats tables. See the [Application](/docs/accessanalyzer/enterpriseauditor/admin/settings/application/overview.md) topic for additional information.
