...
Navigate to the __Jobs__ > __Windows__ > __Privileged Accounts__ > __Service Accounts__ > __SG_ServiceAccounts__ > __Configure__ node and select __Analysis__ to view the analysis tasks.
...
- Domain Service Accounts – Creates the SA_SG_ServiceAccounts_Details table accessible under the job’s Results node
- Domain Summary – Creates the SA_SG_ServiceAccounts_DomainSummary table accessible under the job’s Results node

In addition to the tables created by the analysis tasks, the SG_ServiceAccounts job produces the following pre-configured report.
