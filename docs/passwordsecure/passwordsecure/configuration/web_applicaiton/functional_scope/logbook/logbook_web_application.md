# Logbook

The __logbook module__ exists of the following features:

- Filter function
- Quick view

NOTE: The Web Application module logbook is based on the same called client module logbook. Both modules differ in range and design. However, the handling is almost the same.

Differences to the logbook on the Client:

The following options are not available yet in the __Web Application__. If needed, you can use them on the Client.

- Documents
- Multifactor authentication
- Report configuration
- Applications
- Password Reset
- Password rules
- Sytem Task
