...
This guide shows how to set up a [SCIM](../../references-connectors/scim/index "SCIM") connector to extract data from your CyberArk instance into CSV source files that will in turn be fed to the [Upward Data Synchronization](../../../synchronization/upward-data-sync/index "Synchronization") task and to your Identity Manager resource repository. It will focus on registering Identity Manager within the target CyberArk instance, configuring the connector, and building the job to perform regularly scheduled synchronization.
...
The settings must be entered in the appsettings.agent > Connections section. See the [SCIM](../../references-connectors/scim/index "SCIM") topic for additional information.
...
* The **Server** attribute with the CyberArk's address. It has the form: `https://host:port/CyberArk/scim`.
...
An exhaustive list of entities and attributes provided by CyberArk is available in their [technical documentation](https://docs.cyberark.com/Product-Doc/OnlineHelp/Idaptive/Latest/en/Content/Applications/AppsOvw/SCIM-Provisioning.htm "technical documentation") or the SCIM `Swagger UI`.

The `Filter` and `FilterGroup` setting syntax is detailed in the [SCIM](../../references-connectors/scim/index "SCIM") optional attributes.
...
* **ldapFullDN** is an attribute of the `urn:ietf:params:scim:schemas:cyberark:1.0:User` schema because it is separated by `�`.
...
The export generates CSV source files that will be fed to the [Upward Data Synchronization](../../../synchronization/upward-data-sync/index "Synchronization") task.
...
See the [SCIM](../../references-connectors/scim/index "SCIM")topic for additional information.

The target directory and file name are chosen freely. However, Netwrix Identity Manager (formerly Usercube) strongly recommends using the [Create a Working Directory](../../../../installation-guide/production-ready/working-directory/index "Create a Working Directory") `Temp/ExportOutput` folder and choosing file names that start with the `CyberArk_` prefix.
...
To be used for export tasks, a connector must be declared in the [Toolkit for XML Configuration](../../../toolkit/index "Toolkit for XML Configuration") and linked to an Agent.

It is strongly recommended that the applicative configuration be stored the [Create a Working Directory](../../../../installation-guide/production-ready/working-directory/index "Create a Working Directory") `Conf` folder as a set of `xml` files organized by connector.
...
* Use the [Connector](../../../toolkit/xml-configuration/connectors/connector/index "Connector") element to declare the connector with the following attributes:
  ...
  * **Identifier** identifies this connector in the [Toolkit for XML Configuration](../../../toolkit/index "Toolkit for XML Configuration"). We recommend using a meaningful name such as `CyberArk`. If several connections to several CyberArk targets are possible, only one CyberArk Connector per Agent is used.
  * **DisplayName\_Li, i ? [1..16]** are used in the UI.
  * **Agent** is the identifier of the Agent that will run this connector's export task. The Agent's identifier can be found in the agent's [appsettings.agent](../../../network-configuration/agent-configuration/appsettings-agent/index "appsettings.agent") > OpenId > AgentIdentifier.
...
The exported data to be written to the [Identity Management](../../../../introduction-guide/overview/identity-management/index "Identity Management") must be aligned with the [Entity Model](../../../entity-model/index "Entity Model").

The [Entity Model](../../../entity-model/index "Entity Model") should match as closely as possible the structure of the CyberArk data relevant for Identity Manager. It is designed by analyzing the CyberArk data structure, and describing said data with the [Entity Model](../../../entity-model/index "Entity Model") and [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "Entity Association").
Eventually, it is up to the integration team to design the [Entity Model](../../../entity-model/index "Entity Model") that best serves the [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") needs. It will most likely be refined iteratively throughout the project integration.

A good starting point for the Entity Model is to mirror the shape of the exported CyberArk SCIM objects. This guide provides a few examples that can serve this purpose. Thus, CyberArk SCIM objects such as **Users** and **Groups** can be described by [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy"), and group membership by [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation").

The [Entity Model](../../../entity-model/index "Entity Model") for the CyberArk connector is written in the [Toolkit for XML Configuration](../../../toolkit/index "Toolkit for XML Configuration"). It is strongly recommended to write the entity model to the newly created `Conf/SCIMCyberArk/CyberArk Connector.xml` file.

#### Write entity types

Declaring an [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") is achieved with the `` tag and the following attributes:
...
* **DisplayName\_Li, i ? [1..16]** are used in the UI to identify this [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") for the end-user. **DisplayName\_L1** is the name of the entity type in *language number one*. If this language is *English*, a good example value would be `CyberArk - User`.
...
```
The CyberArk SCIM objects attributes are modeled by [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy")properties, with the `` tags declared as children of the ``.

Remember that there are several kinds of by [Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") (determined by the `TargetColumnIndex`): scalar and navigation.
...
Finally, the main attributes of the `` tag are the following:
...
* **TargetColumnIndex** defines in which column of the resource table the property is stored. See more details at [Entity Type](../../../toolkit/xml-configuration/metadata/entitytype/index "EntityType").
...
[Assignment Policy](../../../role-model/role-model-rules/index "Assignment Policy") are associated through their navigation properties with [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation") elements.
...
The `Users` property of a `CyberArk_Group` is a collection of `CyberArk_User`IDs which are members of this **Group**.
...
Notice the format of the **Property1** and **Property2** xml attributes: the name of the entity type followed by `:` and the name of an entity property. It is a [Binding](../../../toolkit/xml-configuration/metadata/binding/index "Binding") that describes in one expression both the target entity type and property.
...
The [Entity Type Mapping](../../../toolkit/xml-configuration/connectors/entitytypemapping/index "EntityTypeMapping"), [Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping"), and Entity Property Mapping elements serve this purpose.

#### Write the entity type mapping

The [Entity Type Mapping](../../../toolkit/xml-configuration/connectors/entitytypemapping/index "EntityTypeMapping") element maps scalar properties from the CSV source file to an entity type.
...
```
To do so, the entity type mapping uses the [Entity Type Mapping](../../../toolkit/xml-configuration/connectors/entitytypemapping/index "EntityTypeMapping") element with the `` tag. This maps the CSV column from `ConnectionColumn` to the target EntityType property which is written to the **Identifier** attribute.
...
The [Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping") element maps navigation properties, used in [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation").

An [[Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping")](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index) element refers to an [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation") written to the **Identifier** xml attribute. Then, just as the [Entity Type Mapping](../../../toolkit/xml-configuration/connectors/entitytypemapping/index "EntityTypeMapping") element, it maps columns values from a CSV source file to an EntityType property.
...
The following [[Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping")](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index) describes the mapping for the `CyberArk_Group_Members` EntityAssociation:
...
###### Users/*CyberArk\_Group*

The `Users` property in the `CyberArk_Group` entity:

* is written to the **Property1** attribute of the `CyberArk_Group_Members` [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation") element.
* is filled in by values from the `MemberId` column (written to the **Column2** attribute of the `CyberArk_Group_Members` [[Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping")](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index) element) in the `C:/UsercubeDemo/Temp/ExportOutput/CyberArk_members_Groups.csv` file.

These values identify resources of type `CyberArk_User` by their `CyberArk_id` property (written to the **EntityPropertyMapping2** attribute of the [[Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping")](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index) element.

###### Groups/*CyberArk\_User*

The `Groups` property in the `CyberArk_User` entity:

* is written to the **Property2** attribute of the `CyberArk_Group_Members` [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "EntityAssociation") element).
* is filled in by values from the *value* column (written to the **Column1** attribute of the `CyberArk_Group_Members` [Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping") element) in the `C:/UsercubeDemo/Temp/ExportOutput/CyberArk_members_Groups.csv` file.

These values identify resources of type `CyberArk_Group` by their `CyberArk_id` property (written to the **EntityPropertyMapping1** attribute of the [Entity Association Mapping](../../../toolkit/xml-configuration/connectors/entityassociationmapping/index "EntityAssociationMapping") element).
...
A [Menu Item](../../../toolkit/xml-configuration/user-interface/menuitem/index "MenuItem") can be added to include a link to the resources list in the left menu in the UI home screen.
...
![SCIM CyberArk Menu Items](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/scim_cyberark_export_menu_item_5.1.6.png)
...
The [Display EntityType](../../../toolkit/xml-configuration/user-interface/displayentitytype/index) describes how a single resource should be displayed.
...
![SCIM CyberArk Display Entity Type](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/scim_cyberark_export_display_entity_type_5.1.6.png)

The scalar properties don't need to be configured: they are automatically displayed. The only information that the [DisplayEntityType](../../../toolkit/xml-configuration/user-interface/displayentitytype/index) adds here, is that the property `BasicCollection` is a navigation property. An eye icon will be displayed to take you directly to the matching page.

#### Display table

The [DisplayTable](../../../toolkit/xml-configuration/user-interface/displaytable/index) elements describe how a list of resources should be displayed.

The [DisplayTable](../../../toolkit/xml-configuration/user-interface/displaytable/index) contains a list of [DisplayTableColumn](../../../toolkit/xml-configuration/user-interface/displaytable/index#column) elements that identify which properties should be included in the list display.
...
![SCIM CyberArk Display Table](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/scim_cyberark_export_display_table_5.1.6.png)

#### Internal display name

An `InternalDisplayName` can also be declared as an [Entity Type](../../../toolkit/xml-configuration/metadata/entitytype/index "EntityType"). The `InternalDisplayName` is used in several UI screens to identify a resource for the user.
...
All the job steps can be found in the [Create Connector Synchro Complete](../../../toolkit/xml-configuration/configuration/scaffoldings/jobs/createconnectorsynchrocomplete/index "Create Connector Synchro Complete") scaffolding.
...
Every request from Agent to Server within the execution of a Job needs to be authenticated with an [OpenIdClient](../../../toolkit/xml-configuration/access-control/openidclient/index "OpenIdClient") Connect /Secret pair, linked to a Profile.
...
For an Agent to launch server-side Tasks from the Job via the [Usercube-Invoke-Job](../../../executables/references/invoke-job/index "Identity Manager-Invoke-Job"), the profile linked to these tasks and used by the tool should be authorized to execute said tasks.
...
Granting access can be done via the [Synchronization Access Control Rules](../../../toolkit/xml-configuration/configuration/scaffoldings/accesscontrolrules/jobs/synchronizationaccesscontrolrules/index "Synchronization Access Control Rules") scaffolding and the[Job View Access Control Rules](../../../toolkit/xml-configuration/configuration/scaffoldings/templates/jobviewaccesscontrolrules/index "JobViewAccessControlRules") scaffolding.
...
An Agent's [Profile](../../../toolkit/xml-configuration/access-control/profile/index "Profile") is associated with a `ClientId/Secret` pair used by the Agent to authenticate to the Server.
...
It is strongly recommended that you write the `` xml element to a new or existing `OpenIdClients.xml` file in the configuration root folder.
