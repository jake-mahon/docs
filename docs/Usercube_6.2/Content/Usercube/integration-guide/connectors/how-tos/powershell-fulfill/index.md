...
This guide shows how to set up a [PowerShellProv](../../references-connectors/powershellprov/index "PowerShellProv") connector to fulfill data in Microsoft Exchange Server. It will focus on registering Identity Manager within the target Microsoft Exchange instance, configuring the connector, and building the job to perform a regularly scheduled fulfillment. Of course, any other system compatible with PowerShell can be chosen.
...
The Fulfill-PowerShell needs a script whose path is defined by the attribute **PowerShellScriptPath**. Identity Manager provides a script in the SDK in `Usercube.Demo/Scripts/Fulfill-Exchange.ps1`.See the [Write a PowerShell Script for Provisioning](../write-fulfill-powershell-script/index "Write a PowerShell Script for Provisioning") topic for more details on how to write a customized script.

To define and apply additional settings when authenticating to an external system, we can set the attribute [PowerShellProv](../../references-connectors/powershellprov/index "PowerShellProv") and add required parameters for authentication.

In the example above, the `Basic` AuthType was chosen to show how to fill the credentials, but it isn't mandatory to use this [Microsoft Exchange](../../references-connectors/microsoftexchange/index "Microsoft Exchange").

For pedagogical reasons, this guide focuses on the simplest way to set up the fulfillment, but it's not the most secure. Hence, it is strongly recommended to use Kerberos AuthType or [PowerShellProv](../../references-connectors/powershellprov/index "PowerShellProv") via Azure Key Vault or CyberArk in a production environment.  
...
In the `MicrosoftExchange` directory, create a `MicrosoftExchange Connector.xml` file. This file contains the declaration of the connector and the associated [Entity Model](../../../entity-model/index).

> This example declares the `MicrosoftExchange`[connector](../../../toolkit/xml-configuration/connectors/connector/index) on the `Local` agent, and the [connection](../../../toolkit/xml-configuration/connectors/connection/index) linked to the previously defined `MicrosoftExchangeExportFulfillment` JSON section (see the [example](#example) above):
>
> ```
...
The [Entity Model](../../../entity-model/index "Entity Model") should match as closely as possible the structure of the Microsoft Exchange data relevant for Identity Manager. It is designed by analyzing the Microsoft Exchange data structure, and describing said data with [Entity Model](../../../entity-model/index "Entity Model") and [Entity Association](../../../toolkit/xml-configuration/metadata/entityassociation/index "Entity Association").
...
```
:::note
The CSV file microsoftexchange\_mailboxes.csv must be exported to the Application Settings folder.
:::
...
* [Resource Type](../../../toolkit/xml-configuration/provisioning/resourcetype/index "Resource Type") which assigns a resource to a user
* [Resource Type](../../../toolkit/xml-configuration/provisioning/resourcetype/index "Resource Type") which specifies the value to be set to an assigned resource scalar property
* [Resource Type](../../../toolkit/xml-configuration/provisioning/resourcetype/index "Resource Type") which specifies a value to be set to an assigned resource multi-valued navigation property
...
![Microsoft Exchange Menu Items](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/microsoftexchange_fulfill_menu_item_5.1.7.png)
...
![Microsoft Exchange Display Entity Type](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/microsoftexchange_fulfill_display_entity_type_5.1.7.png)
...
![Microsoft Exchange Display Table](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/microsoftexchange_fulfill_display_table_5.1.7.png)
...
The [Access Control Rule](../../../toolkit/xml-configuration/access-control/accesscontrolrule/index "AccessControlRule") and [Access Control Rule](../../../toolkit/xml-configuration/access-control/accesscontrolrule/index "AccessControlRule") elements define [AccessControlPermission](../../../toolkit/xml-configuration/access-control/accesscontrolpermission/index "AccessControlPermission") permissions for end-user profiles to read and write the connector's data (such as resources of a given entity type). It is used by the UI when displaying data such as resources and available roles.
...
```
This job will be executed on Microsoft Exchange's connector[Fulfill Microsoft Exchange via PowerShell](# "Fulfill Microsoft Exchange via PowerShell").
...
There is also the tag  which means that the export will not be executed. Removing the tag will launch export-related tasks before fulfillment-related tasks. Export tasks need the same XML configuration and additional settings in[Fulfill Microsoft Exchange via PowerShell](#).
...
An Active Directory configuration is required for Microsoft Exchange to work. Fill the[Fulfill Microsoft Exchange via PowerShell](# "Fulfill Microsoft Exchange via PowerShell") settings in accordance with the configuration.
...
![Microsoft Exchange Jobs](../../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/microsoftexchange_jobs_5.1.7.png)
