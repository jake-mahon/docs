...
Upward Data Synchronization (Sync Up) is the process that copies relevant managed systems data into Identity Manager's [Entity Model](../../entity-model/index) and translates them into resources that match the configured Entity Model.
...
During this step, data is extracted from the managed system and generates *CSV files* containing the managed system's raw data. The **output** of this process is called the ***CSV source files***. They are written to the [Application Settings](../../network-configuration/agent-configuration/appsettings/index) waiting to be used by the next-in-line *prepare-synchronization task*.
...
If the managed system has built-in export capabilities, Identity Manager can simply rely on exports scheduled by the source managed system. Regularly, the managed system generates reports, in whatever format. A custom task, such as a [Invoke Expression Task](../../toolkit/xml-configuration/jobs/tasks/agent/invokeexpressiontask/index), can then be used to retrieve the generated exports, adapt them to the *CSV source files* format expected by Identity Manager and copy them to the [Application Settings](../../network-configuration/agent-configuration/appsettings/index). The whole can be scheduled and orchestrated by a [Jobs](../../tasks-jobs/jobs/index "Jobs").
...
All *CSV source files* are written to the [Upward Data Synchronization](# "Upward Data Synchronization").

At the end of the *export* step, the [Upward Data Synchronization](# "Upward Data Synchronization") contains several files per connectors, that will be translated into *resources* during *prepare-synchronization* and *synchronization* steps thanks to [Upward Data Synchronization](#) (see below).

:::note
The
Upward Data Synchronization
can also contain opaque cookie files used for incremental export of a few systems such as Active Directory, Microsoft Entra ID, ServiceNow, and SCIM.
:::
...
![Active Directory Export Example](../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/ad_export_example.png)
...
The result of the *Prepare-Synchronization* is stored in the [Upward Data Synchronization](# "Upward Data Synchronization") as three files:
...
* [Synchronize Task](../../toolkit/xml-configuration/jobs/tasks/server/synchronizetask/index "SynchronizeTask") i is the standard *synchronization* task.
...
![Active Directory Synchronization Example](../../../../../../../static/images/Usercube_6.2/Content/Resources/Images/ad_synchro_example.png)
