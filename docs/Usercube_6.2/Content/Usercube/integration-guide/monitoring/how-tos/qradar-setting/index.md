...
Typically, a Serilog configuration includes three parts: **MinimumLevel**, **Using** and **WriteTo**. See the [Monitoring](../../index "Monitoring") topic for additional information.
...
Logs can be sent into QRadar without using Identity Manager's DSM in QRadar, but the logs just won't be parsed. Not all Identity Manager's logs can be sent to QRadar. See the [References: Logs](../../references/index "References: Logs") topic for additional information.
...
1. In [Application Settings](../../../network-configuration/agent-configuration/appsettings/index) appsettings. json, make sure to have a **Serilog** section:
   ...
   ```
{
   ...
   ```
2. In the **Serilog** section, add a **Using** section to contain the used sink which depends on the logs' destination, output format, etc. See the list of supported [Monitoring](../../index).
...
In order to be sent to any system, Identity Manager's logs must be configured with MinimumLevel set to Information, or lower.
   ...
   > For example, we can define the logs' minimum level to `Information`. This way, all logs from the [References: Logs](../../references/index "References: Logs") with `Information` level or higher are sent.
   >
   > ```
>
   > appsettings.json
...
>
   ...
   >
   > ```
> For example, to produce an RFC5424 output for QRadar ([see more information about UdpSyslog attributes](https://github.com/IonxSolutions/serilog-sinks-syslog#serilogsinkssyslogmessages)):
   >
   > ```
>
   > appsettings.json
...
>
   ...
   >
   > ```
5. When needing to restrict the logs sent to the system, add a filter and wrap all **WriteTo** configuration into a sub-logger, in which case the **Name** at **WriteTo**'s root must be `Logger`. See the [Monitoring](../../index "Monitoring") topic for additional information.
   ...
   For example, among Identity Manager's logs, only the logs described in the e [References: Logs](../../references/index "References: Logs") can be parsed by QRadar's DSM and should be used by a SIEM system. Hence the importance of having a filter and a sub-logger.
