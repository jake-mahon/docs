...
**NOTE:** JSON files can contain any additional information that you might find useful. See the example below.
...
| --- | --- | --- |
| ApplicationUri (required) | Uri | Server's listening URI. Used by the agent to send requests to the server.  Example:  Code attributes enclosed with  need to be replaced with a custom value before entering the script in the command line.  ```
appsettings.json {  "ApplicationUri": " " } ```
|
...
| TaskTimeoutSupplement default value: 0 | Int32 | Additional time (in minutes) for the Invoke-Job tool's Timeout property.  Example:  ```
appsettings.json {     "TaskTimeoutSupplement": 10 } ```
|
...
| Serilog (optional) | Logger setting | Settings to configure the logging service, complying to the Logger properties and structure. See the [Monitoring](../../../monitoring/index "Monitoring") topic for additional information.  Example:  ```
...
**NOTE:** Storing a .pfx file password in plain text in a production environment is strongly discouraged. It should always be encrypted using the Usercube-Protect-CertificatePassword tool. See[Usercube-Protect-CertificatePassword](../../../executables/references/protect-certificatepassword/index "Identity Manager-Protect-CertificatePassword") topic for additional information.
...
If the certificate is saved in Azure Key Vault, we must define the certificate identifier and the Vault connection. See the [Azure Key Vault](../azure-key-vault/index "Azure Key Vault") topic for additional information.
...
**NOTE:** Storing a .pfx file password in plain text in a production environment is strongly discouraged. It should always be encrypted using the Usercube-Protect-CertificatePassword tool. See the [Usercube-Protect-CertificatePassword](../../../executables/references/protect-certificatepassword/index "Identity Manager-Protect-CertificatePassword") topic for additional information.
...
**NOTE:** If you are using the certificate provided in the SDK, the agent will fail when launching. You must create your own certificate.
