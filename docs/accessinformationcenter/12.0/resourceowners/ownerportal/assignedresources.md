...
| Change Access  | Opens the [Change Resource Access Wizard](/docs/accessinformationcenter/12.0/resourceowners/ownerportal/changeaccess.md) for the selected resource, which allows you to make ad hoc changes to access and group membership. This button is only enabled if the feature was enabled for the selected resource by the Ownership Administrators. |
