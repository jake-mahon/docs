...
After successful authorization, You are redirected to the dashboard. See the [1Secure Dashboard](../dashboard/overview.md) topic for additional information.

Once the initial login is completed, an MSP can configure Single Sign On (SSO) using supported authentication services, including Entra ID, 1Secure Authentication, or OpenID Connect. See the [SSO Configuration with Authentication Services](sso.md#sso-configuration-with-authentication-services) topic for additional information.
