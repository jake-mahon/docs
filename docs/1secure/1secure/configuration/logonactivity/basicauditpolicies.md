...
Basic local audit policies allow tracking changes to user accounts and groups and identifying originating workstations. You can configure advanced audit policies for the same purpose too. See [Configure Advanced Audit Policies](/docs/product_docs/1secure/1secure/configuration/logonactivity/advancedaudit.md)
...
__Step 2 –__ In the left pane, navigate to __Forest: <forest\_name>__ > __Domains > <domain\_name> > Domain Controllers__. Right-click the effective domain controllers policy (by default, it is the __Default Domain Controllers Policy__), and select __Edit__ from the pop-up menu.
...
![manualconfig_nla_auditpolicies2016](/static/img/product_docs/1secure/1secure/configuration/logonactivity/manualconfig_nla_auditpolicies2016.png)
