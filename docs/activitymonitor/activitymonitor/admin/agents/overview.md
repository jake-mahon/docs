...
- Add Agent – Opens the Add New Agent(s) window to deploy the activity/AD agent to a single server or to multiple servers at the same time. The following sections provide additional information:
  ...
  - [Single Activity Agent Deployment](/docs/activitymonitor/activitymonitor/admin/agents/add/single.md)
  - [Multiple Activity Agents Deployment](/docs/activitymonitor/activitymonitor/admin/agents/add/multiple.md)
  - [Active Directory Agent Deployment](/docs/activitymonitor/activitymonitor/admin/agents/add/activedirectory.md)
  - [Linux Agent Deployment](/docs/activitymonitor/activitymonitor/admin/agents/add/linux.md)
  - Remove – Opens the Remove Agents window where users can choose to remove the hosting server from the activity agents table or uninstalling the activity agent from the hosting server before removing the activity agent from the table. See the [Remove Agents](/docs/activitymonitor/activitymonitor/install/removeagent.md) topic for additional information.
- Edit – Opens the selected server’s Properties window to modify the server name or credentials. See the [Agent Properties Window](/docs/activitymonitor/activitymonitor/admin/agents/properties/overview.md) topic for additional information.
...
- Update Installer – Opens a browser window to select the newer AD agent installer. A confirmation window then opens and identifies the new installer version. See the [Update AD Agent Installer](/docs/activitymonitor/activitymonitor/install/updateadagentinstaller.md) topic for additional information.
...
For additional information on how to deploy agents manually, see the [Agent Information](/docs/activitymonitor/activitymonitor/install/agents.md) topic.
