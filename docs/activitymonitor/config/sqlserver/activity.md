...
In order for the Netwrix Activity Monitor to monitor SQL Server activity, a SQL login with certain server permissions, and must be mapped to user databases.
...
The logon trigger is required to obtain IP Addresses of client connections. Run the following script in order to allow the Activity Monitor to report client IP Addresses.
