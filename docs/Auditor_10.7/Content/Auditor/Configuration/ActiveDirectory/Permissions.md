...
* The combination of the following rights and permissions if you plan to disable network traffic compression for your monitoring plan or, for some reasons, do not want to add this account to the Domain Admins group:
  ...
  * The "Manage auditing and security log" policy must be defined for this account.   
    See the [Configure the Manage Auditing and Security Log Policy](#Configur "Configure Manage Auditing and Security Log Policy") topic for additional information.
  * If you plan to process the Active Directory Deleted Objects container, Read permission on
    this container are required. See the [Grant Permissions for the Deleted Objects Container](#Grant "Grant Permissions for Deleted Objects Container") topic for additional information.

If the account selected for data collection is not a member of the Domain Admins group, see the [Assign Permission To Read the Registry Key](PermissionsRegistryKeys "Assign Permission To Read the Registry Key") topic.
...
* The **Audit Logs** management role (see the [Assign Management Roles](../Exchange/Permissions#Assign2 "Assigning Management Roles") topic for additional information).

You will also need to configure Exchange Administrator Audit Logging (AAL) settings. See the [Exchange Administrator Audit Logging Settings](../Exchange/AuditLog "Exchange Administrator Audit Logging Settings") topic for additional information.
...
* Permissions to access the HKEY\_LOCAL\_MACHINE\System\CurrentControlSet\Services\EventLog\Security registry key on the domain controllers in the target domain. See the [Assign Permission To Read the Registry Key](PermissionsRegistryKeys "Assign Permission To Read the Registry Key") topic for additional information.
...
Thus, changes made to your Active Directory domain via that Exchange server will be reported with *domain\Exchange\_server\_name$* instead of the initiator (user) name in the "*Who*" field of reports, search results and activity summaries.
...
* If network traffic compression will be *disabled*, and the account you plan to use for data collection is not a member of the Domain Admins group, then the  **Manage auditing and security log** policy must be defined for this account.   
  See the [Configure the Manage Auditing and Security Log Policy](#Configur "Configure the Manage Auditing and Security Log Policy") topic for more information.

If you need to process Active Directory **Deleted Objects** container, consider the following:

* Read permission on
  this container is required. See the [Grant Permissions for the Deleted Objects Container](#Grant "Grant Permissions for the Deleted Objects Container") topic for additional information.
...
* Permissions to access the *HKEY\_LOCAL\_MACHINE\System\CurrentControlSet\Services\EventLog\Security* registry key on the domain controllers in the target domain. See the [Assign Permission To Read the Registry Key](../WindowsServer/PermissionsRegistryKeys "Assign Permission To Read the Registry Key") topic for additional information.
...
* Membership in the **Organization Management** or **Records Management** group.
* The **Audit Logs** management role (see the [Assigning Management Roles](../Exchange/AuditLogsRole "Assigning Management Roles") topic for additional information).
...
![](../../../../../../static/images/Auditor_10.7/Content/Resources/Images/Auditor/ManualConfig/ManualConfig_WS_Logonasbatch.png)
