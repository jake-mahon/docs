...
**Step 1 –** Create a user account to access the Nutanix REST API. See the [Create User Account to Access Nutanix REST API](UserAccount "Create User Account to Access Nutanix REST API") topic for additional information.

**Step 2 –** Open a port for inbound connections. See the [Nutanix Ports](Ports "Nutanix Ports") topic for additional information.
...
* Automatically when creating a monitoring plan. For that, you should select the **Adjust audit settings automatically** option in the monitoring plan wizard. See the [Settings for Data Collection](../../../Admin/MonitoringPlans/Create#Settings "Settings for Data Collection") topic for additional information.
* Manually, as described in the corresponding topics:
  * [Configure Partner Server](PartnerServer "Configure Partner Server")
  * [Create a Notification Policy](NotificationPolicy "Create a Notification Policy")
