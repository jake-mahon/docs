...
* Review the list of objects and attributes that can be monitored by Auditor: See the [File Servers](../Overview "File Servers")topic for additional information.
...
* To collect data from Windows Failover Cluster, network traffic compression must be enabled.
* Scale-Out File Server (SOFS) cluster is not supported.
* Several constraints apply to DFS auditing. See the [DFS-Related Constraints](#DFS-rela "DFS-related constraints") topic for additional information.
...
* Auditor may report on several unexpected changes with *who* (initiator's account) reported as *system* due to the native Windows File Servers audit peculiarities. If you do not want to see these changes, exclude them from the audit. See the [File Servers](../Overview "File Servers") topic for additional information. For example - mass file removals, when target Windows server generates too many events at a time and the product is unable to parse their sequences correctly.
...
* To report on *copy* actions on remote file shares, make sure that audit of successful read operations is enabled. See the [Configure Object-Level Access Auditing](ObjectLevel "Configure Object-Level Access Auditing") topic for additional information.
...
* **Automatically** when creating a monitoring plan.
  ...
  In this case, the audit settings will be applied automatically, then they will be periodically checked and adjusted if necessary. See the [Create a New Monitoring Plan](../../../Admin/MonitoringPlans/Create "Settings for Data Collection") topic for additional information.
* **Manually.** To configure your Windows File Servers for monitoring manually, perform the following procedures:

  * [Configure Object-Level Access Auditing](ObjectLevel "Configure Object-Level Access Auditing")
  * [Configure Local Audit Policies](LocalPolicy "Configure Local Audit Policies") or [Configure Advanced Audit Policies](AdvancedPolicy "Configure Advanced Audit Policies")
  * [Configure Event Log Size and Retention Settings](EventLog "Configure Event Log Size and Retention Settings")
  * [Enable Remote Registry Service](RemoteRegistryService "Enable Remote Registry Service")
  * [Windows File Server Ports](Ports "Configure Windows Firewall Inbound Connection Rules")
...
**Step 5 –** Configure Data Collecting Account. See the [Data Collecting Account](../../../Admin/MonitoringPlans/DataAccounts "Data Collecting Account") topic for additional information.

**Step 6 –** Configure required protocols and ports. Set up protocols and ports. See the [Dell Data Storage Ports](../DellDataStorage/Ports "File Servers") topic for additional information.
