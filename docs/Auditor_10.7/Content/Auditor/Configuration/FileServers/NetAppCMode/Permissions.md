...
See [Create Role on NetApp Clustered Data ONTAP 8 or ONTAP 9 and Enabling AD User Access](#Create "Create Role on NetApp Clustered Data ONTAP 8 or ONTAP 9 and Enabling AD User Access") section for additional information.
...
**Step 1 –** Create a new role (e.g., netwrix\_role for ONTAPI and netwrix\_rest\_role for RESTAPI) on your SVM (e.g., svm1). For example:
...
**NOTE:** In ONTAP 9.10 and higher, it is not possible to assign ONTAPI role (e.g. netwrix\_role) and RESTAPI role (e.g. netwrix\_rest\_role) to one AD user. To allow a user access to both the ONTAPI and RESTAPI, you can use different AD groups by assigning roles to them and including the user in these groups.
