...
For more information on gMSA, refer to [Use Group Managed Service Account (gMSA)](../../Requirements/gMSA)")[Microsoft documentation](https://docs.microsoft.com/en-us/windows-server/security/group-managed-service-accounts/group-managed-service-accounts-overview).
...
1. On the SharePoint server where the Netwrix Auditor Core Service will be deployed: the account must be a member of the local Administrators group.   
   To learn more about Netwrix Auditor Core Services, refer to [Installation](../../Install/Overview "Installation") topic.
2. On the SQL Server hosting SharePoint database: the SharePoint\_Shell\_Access role.   
   See the [Assigning 'SharePoint\_Shell\_Access' Role](#Assignin "Assigning 'SharePoint_Shell_Access' Role") topic for additional information.
...
1. In your SharePoint server, click Start → Microsoft SharePoint Products  SharePoint Management Shell.
