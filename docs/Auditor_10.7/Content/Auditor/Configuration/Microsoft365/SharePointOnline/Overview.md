...
* Manually – Native audit settings must be adjusted manually to ensure collecting comprehensive and reliable audit data. You can enable Auditor to continually enforce the relevant audit policies or configure them manually:
  ...
  * Prepare a Data Collecting Account as described in the [Permissions for SharePoint Online Auditing](Permissions "Permissions for SharePoint Online Auditing") topic.
  * Configure required protocols and ports, as described in the [SharePoint Online Ports](Ports "SharePoint Online Ports") topic.
