...
* Manually – Native audit settings must be adjusted manually to ensure collecting comprehensive and reliable audit data. You can enable Auditor to continually enforce the relevant audit policies or configure them manually:
  ...
  **NOTE:** Unified audit log must be enabled for a Tenant. See the Microsoft [Turn auditing on or off](https://docs.microsoft.com/en-us/microsoft-365/compliance/turn-audit-log-search-on-or-off?view=o365-worldwide) article for additional information.
...
* Prepare a Data Collecting Account as described in the [Permissions for Exchange Online Auditing](Permissions "Permissions for Exchange Online Auditing") topic
* Configure required protocols and ports, as described in the [Exchange Online Ports](Ports "Exchange Online Ports") topic
