...
* Manually – Native audit settings must be adjusted manually to ensure collecting comprehensive and reliable audit data. You can enable Auditor to continually enforce the relevant audit policies or configure them manually:
  ...
  * On the audited system(s):
    ...
    * The User Activity Core Service is installed on the monitored computers. See the [Install Netwrix Auditor Agent to Audit User Activity](#Install "Install Netwrix Auditor Agent to Audit User Activity") topic for additional information.
...
* [Configure Data Collection Settings](DataCollection#Configur "Configure Data Collection Settings")
* [Configure Video Recordings Playback Settings](VideoRecordings#Configur2 "Configure Video Recordings Playback Settings")
...
| Window | Application Run As:  | Standard user runs an application under credentials of another standard user. |
| Elevated Window | Application Run As:  | User runs program through Run As Administrator or Accepts UAC (User Account Control) elevation prompts. |
