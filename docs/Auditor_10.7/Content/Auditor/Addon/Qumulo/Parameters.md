...
Most parameters are optional, the service uses the default values unless parameters are explicitly defined (****value****). You can skip or define parameters depending on your execution scenario and security policies.
...
After you modify parameters in the **settings.xml** file, remember to save the changes and then restart (Undefined variable: Add-on.Addon\_Qumulo) service (**SyslogService.exe**) for them to take effect.
