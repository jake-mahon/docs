...
Supported object types and attributes are listed in the [Active Directory](../../../../Configuration/ActiveDirectory/Overview "Active Directory") topic.

For this report to function properly, you must enable the **Collect data for state-in-time reports** option for the data source in the monitoring plan settings. See the [Settings for Data Collection](../../../MonitoringPlans/Create#Settings "Settings for Data Collection") topic for additional information.
...
A security manager wants to find administrators of the *corp.local* domain with incorrect password settings (password not required). Service accounts (*svc\_%*) must be skipped in the report. This domain is included in the monitoring plan named *Active Directory Monitoring*. He or she needs to set report filters as follows:
