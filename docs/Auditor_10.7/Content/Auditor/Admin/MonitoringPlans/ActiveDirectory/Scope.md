...
You can fine-tune Netwrix Auditor by specifying data that you want to exclude from the Active Directory monitoring scope. You can apply restrictions to monitoring scope via the UI. See the [Objects](Overview#Objects "Objects") topic for additional information.

***RECOMMENDED:*** Configure monitoring scope restrictions on the Active Directory monitoring plan page. See the [Active Directory](Overview "Active Directory") topic for additional information.
...
| allowedpathlist.txt | Contains a list of AD paths to be included in Activity Summaries, reports, and search results. | `Path`  The path must be provided in the same format as it is displayed in the What column.  For example, if you only want to monitor specific OU(s)  in the AD domain, but not the entire domain. You can put a wildcard (\*) in the [omitpathlist.txt](#omitpathlist) file to exclude all paths, and then specify the OU(s) you want to monitor in the allowedpathlist.txt file.  Adding the widlcard (\*) to omitpathlist.txt will not allow Netwrix Auditor to run AD state-in-time data collection. |
| omitallowedpathlist.txt | Contains a list of AD paths to be excluded from Activity Summaries, reports, and search results.  This file can be used if you want to exclude certain paths inside those specified in the [allowedpathlist.txt](#allowedpathlist) file. | `Path`  The path must be provided in the same format as it is displayed in the What column.  For example, you can put a wildcard (\*) in the [omitpathlist.txt](#omitpathlist) file to exclude all paths, then specify the OU(s) you want to monitor in the [allowedpathlist.txt](#allowedpathlist) file, and then specify the paths you want to exclude from within them in the omitallowedpathlist.txt file.  Adding the widlcard (\*) to omitpathlist.txt will not allow Netwrix Auditor to run AD state-in-time data collection. |
