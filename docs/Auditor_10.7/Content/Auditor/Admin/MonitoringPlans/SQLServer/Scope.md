...
| omittracelist.txt | If you do not want the product to enable SQL tracing on some of your SQL Server instances, specify their names in this omitlist.  In this case the "Who", "Workstation" and "When" values will not be reported correctly (except for content changes).  This omit list does not affect triggerless data collection mode, SELECT statements auditing and logon activity auditing. Use the *omitlogonlist.txt* to exclude logon activity from monitoring. Use the *omitreadaccess.txt* to exclude SELECT statements from monitoring. | Enter the name of SQL Server instance, use \* for all servers:  `server\instance name`  Wildcard (\*) is supported and can replace any number of characters, e.g., MYSERVER\_SQL\*  Examples:  \*  \*\SQLExpress  MYSERVER\\* |
