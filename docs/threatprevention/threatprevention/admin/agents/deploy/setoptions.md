...
- Agent Service
  - Safe Mode – The Agent checks LSASS versions on start up. Any changes in LSASS since the previous start prevents the AD Events monitoring module from loading. See the [Agent Safe Mode](/docs/product_docs/threatprevention/threatprevention/admin/agents/safemode.md) topic for additional information.
...
![Deploy Agents wizard – Set Options page: Agent Install Path box](/static/img/product_docs/threatprevention/threatprevention/admin/agents/deploy/installpath.png)
...
The installation location applies to all computers where the Agent is being deployed in this session (as specified on the [Select Computers Window](/docs/product_docs/threatprevention/threatprevention/admin/agents/deploy/selectcomputers.md) of the Deploy Agents wizard). Once these settings are configured as desired, the Agent is ready for deployment on the selected machines. See the [Prerequisites Check Window](/docs/product_docs/threatprevention/threatprevention/admin/agents/deploy/prerequisitescheck.md) topic for the next step.
...
On the Agents interface, when you open the Set Options window through the Update Agent Settings option on the [Right-Click Menu](/docs/product_docs/threatprevention/threatprevention/admin/agents/overview.md#Right-Click-Menu), the window appears as follows:

![Update Agent Settings > Set Options page](/static/img/product_docs/threatprevention/threatprevention/admin/agents/deploy/updatesetoptions.png)
...
__NOTE:__ To view the current state and configured options for an Agent, hover over the Version String column on the [Agents Interface](/docs/product_docs/threatprevention/threatprevention/admin/agents/overview.md) data grid for the tool tip. The AD Agent column indicates the Agent’s mode.
