...
The following C# script sends an email notification to the specified administrator when an Active Directory account is set to have its password never expire.
...
In the Templates > Actions folder in the Navigation pane, the _ADChanges: Notify Admin that account now has Password Never Expires_ template is preconfigured to use this action script.
