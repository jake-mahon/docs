...
- Event data generated for a policy is sent to SIEM if the 'Send to SIEM' option is selected for that policy on the Actions tab. See the [Send to SIEM](/docs/product_docs/threatprevention/threatprevention/admin/policies/actions/overview.md#Send-to-SIEM) topic for additional information.
- Event data generated for an analytic is sent to SIEM if the 'Send Raw Data to SIEM' option is selected for that analytic on the Actions tab. See the [Send to SIEM](/docs/product_docs/threatprevention/threatprevention/admin/policies/actions/overview.md#Send-to-SIEM) topic for additional information.
- Event data generated for an event/incident/policy is sent to SIEM if a SIEM profile has been assigned to it on the [SIEM Tab](/docs/product_docs/threatprevention/threatprevention/admin/configuration/systemalerting/siem.md) of the System Alerting Window.
...
![SIEM Output Viewer window](/static/img/product_docs/threatprevention/threatprevention/admin/configuration/siemoutputviewer.png)
...
__NOTE:__ The host, port, protocol, and SSL columns display the settings configured for the SIEM server where the respective message is sent. See the [Configure a SIEM Server ](/docs/product_docs/threatprevention/threatprevention/admin/configuration/systemalerting/siem.md#Configure-a-SIEM-Server) topic for additional information.

This data grid employs features for sorting, filtering, searching, and more.

- Right-click anywhere in the column header to get a menu with several options to sort the data in the grid. See the [Data Grid Right-Click Menu](/docs/product_docs/threatprevention/threatprevention/admin/navigation/rightclickmenus.md#Data-Grid-Right-Click-Menu) topic for additional information.
- The Group by Box ribbon just above the header row impacts how much data is displayed. See the [Sort Data](/docs/product_docs/threatprevention/threatprevention/admin/navigation/datagrid.md#Sort-Data) topic for additional information.
- Columns can be reordered as desired as well as removed from the data grid. Removed columns can be added back through the [Customization Window](/docs/product_docs/threatprevention/threatprevention/admin/navigation/datagrid.md#Customization-Window).
- The Search icon opens the __Enter text to search…__ textbox where you can search for the required data in the data grid. See the [Search Data](/docs/product_docs/threatprevention/threatprevention/admin/navigation/datagrid.md#Search-Data) topic for additional information.
