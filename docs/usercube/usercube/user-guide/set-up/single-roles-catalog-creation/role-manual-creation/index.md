...
![createsinglerole](/img/product_docs/usercube/usercube/user-guide/set-up/single-roles-catalog-creation/role-manual-creation/createsinglerole.webp)
...
- Tags: Label(s) that can later be used to filter the target roles of access certification campaigns. See the [
  ...
  ](/docs/usercube/usercube/user-guide/administrate/access-certification/certification-campaign-scheduling/index.md) topic for additional information.
...
- Approve Role Implicitly: Needs at least the simple approval workflow. __Implicit__ mode bypasses the approval step(s) if the person who makes the role request is also the role officer. __Explicit__ refuses said bypass. __Inherited__ follows the policy decision to approve roles implicitly or not. See the [Create a Policy](/docs/usercube/usercube/user-guide/optimize/policy-creation/index.md) topic for additional information.
