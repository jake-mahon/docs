...
](/docs/product_docs/usercube/usercube/integration-guide/toolkit/xml-configuration/jobs/tasks/agent/preparesynchronizationtask/index.md)performs a preparatory data cleansing to spot errors and list them in a generated file in the ```/{InstallationFolder}/Work/Synchronization``` folder.
...
Connector
](/docs/product_docs/usercube/usercube/integration-guide/toolkit/xml-configuration/connectors/connector/index.md) attributes:
...
1. Access the list of connectors by clicking on __Connectors__ on the home page in the __Configuration__ section.

   ![Home - Connectors](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/home_connectors_v602.png)
2. On the relevant connector page, in the __Entity Types__ frame, click on __Jobs__.

   Here are all the tasks available for synchronization. They synchronize all connections and entity types for only this connector. It is possible to launch them individually in order to test them and debug a situation, or all together with __All Tasks__. According to the created connection(s) and package(s), all these tasks can be launched either in incremental or complete mode.

   ![Synchronize Job](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_executionjobs_v602.png)
   ...
   Notice that some connectors, depending on their connections and packages, can't be synchronized in incremental mode. As a consequence, when clicking on the __Jobs__ button, you wouldn't have a choice between ```Complete``` and ```Incremental```. See below this note.

   ![Synchronize Job (Only Complete)](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_executionjobs-complete_v602.png)

## Manage Synchronization Automation

Export and synchronization are executed manually from the connector screens. By default, they are also part of scheduled [
Jobs
](/docs/product_docs/usercube/usercube/integration-guide/tasks-jobs/jobs/index.md) provided by Identity Manager:
...
](/docs/product_docs/usercube/usercube/integration-guide/tasks-jobs/how-tos/jobfast/index.md) and [
Set up Complete Synchronization
](/docs/product_docs/usercube/usercube/integration-guide/tasks-jobs/how-tos/jobdaily/index.md) topics for additional information.

Scheduling the jobs avoids manually triggering them everyday.

However, you can choose to withdraw a given connector from both the complete and incremental jobs by clicking on __Deactivate__ on the connector's dashboard. This is particularly useful when modifying a connector. You can also re-insert it at any time with the same button which is now named __Activate__.

![Jobs Results Dashboard](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_dashboard_v522.png)

You can fine-tune the synchronization and/or provisioning of the connector by clicking on the __Edit__ button.

![Edit button](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_edit_v600.png)

Click on __Job Results__ to access the progress of this connector's jobs.

All jobs are accessible on the __Job Execution__ page in the __Administration__ section.

![Home - Job Execution](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/home_jobexecution_v602.png)

## Verify an Entity Type's Synchronization

In order to verify both the synchronization configuration and [Create an Entity Type](/docs/product_docs/usercube/usercube/user-guide/set-up/connect-system/entity-type-creation/index.md):
...
2. Access the connector's logs (from __Job Results__ on the connector's dashboard) to ensure that synchronization completed successfully.
   ...
   ![Jobs Results](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_results_v603.png)
3. Check that the entity types have been added to the left menu of the home page.

   ![Test Entity Type](/static/img/product_docs/usercube/usercube/user-guide/set-up/categorization/classification/entitytypecreation_test_v602.png)
4. Access the relevant entity types (from the menu items on the left of the home page) to check synchronized resources, by navigating in the UI from the accounts through a sample of associations, via the eye icon:
   ...
   >
   > ![SAB Example - Home Page](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_examplesab_v522.png)
   >
   > Clicking on ```SAB - Users``` displays the list of all synchronized resources.
   >
   > ![SAB Example - Data List](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_examplesab2_v602.png)
   >
   > Clicking on any resource displays its detailed attributes, for example ```Abbott Mark```:
   >
   > ![SAB Example - Resource Attributes](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_examplesab3_v602.png)
   >
...
Keep in mind that a problem observed in synchronized data might also come from a mistake made previously in the connector's configuration. Therefore, logs can give more details. Logs are accessible from the __Job Results__ button on the dashboard of a given connector.
...
![Test Entity Type](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/home_entitytypes_v602.png)

Access the relevant connector's page and click on the __Reload__ button to take into account the last changes in the entity type mappings.

If a newly added property doesn't appear in users' data, then:

Access the relevant connector's page to click on the __Reload__ button to take into account the most recent changes in the entity type mappings.

If a synchronization is blocked by an exceeded threshold, then:

![Threshold warning](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_threshold_v603.png)

Find out the reasons to decide whether or not to bypass the threshold. Proceed as follows:

1. On the logs page (accessible from the __Job Results__ button), click on the line of a task instance to see its logs.
2. Study synchronization counters and the list of all synchronization changes. These tools help you make a decision about whether to bypass synchronization thresholds.
   ...
   ![Job progress](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_thresholdlog_v603.png)
...
3. If, after verifying, all changes are legitimate, click on the __Resume__ button at the top of the job progress page. This will restart the job and allow the changes to be synchronized.

   Be cautious, check twice for mistakes before resuming.

   ![Resumed Job](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/synchro_thresholdresumed_v602.png)
...
- If you manually typed the source column of a property in the entity types, then make sure that the source column exists in the corresponding managed system.
  ...
  ![Source Column](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/entitytype_sourcecolumn_v602.png)
...
![Property Format](/static/img/product_docs/usercube/usercube/user-guide/set-up/synchronization/entitytype_format_v523.png)
