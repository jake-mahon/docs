...
References: Logs
](/docs/product_docs/usercube/usercube/integration-guide/monitoring/references/index.md) topic for additional information.
...
1. In [Application Settings](/docs/product_docs/usercube/usercube/integration-guide/network-configuration/agent-configuration/appsettings/index.md) appsettings. json, make sure to have a __Serilog__ section:
...
2. In the __Serilog__ section, add a __Using__ section to contain the used sink which depends on the logs' destination, output format, etc. See the list of supported [
   Monitoring
   ](/docs/product_docs/usercube/usercube/integration-guide/monitoring/index.md).
...
3. Add a __MinimumLevel__ section to define which logs are to be sent to the log management system.
   ...
   > References: Logs
   > ](/docs/product_docs/usercube/usercube/integration-guide/monitoring/references/index.md) with ```Information``` level or higher are sent.
   >
   > ```
   >
   > appsettings.json
...
5. When needing to restrict the logs sent to the system, add a filter and wrap all __WriteTo__ configuration into a sub-logger, in which case the __Name__ at __WriteTo__'s root must be ```Logger```. See the [
   ...
   References: Logs
   ](/docs/product_docs/usercube/usercube/integration-guide/monitoring/references/index.md) can be parsed by QRadar's DSM and should be used by a SIEM system. Hence the importance of having a filter and a sub-logger.
