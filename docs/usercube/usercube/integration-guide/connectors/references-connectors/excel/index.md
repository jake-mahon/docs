...
This connector is meant to generate to the [Application Settings](../../../network-configuration/agent-configuration/appsettings/index.md) folder a CSV file per spreadsheet included in the export, named ```<connectionIdentifier>_<index>.csv``` where ```<index>``` is the spreadsheet's index.
