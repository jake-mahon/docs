...
The following example uses the contract start/end dates as default boundaries in users' [validity period](../../../../identity-management/joiners-movers-leavers/on-offboarding/index.md#period-of-validity), instead of those from the default section. It may be because, for example, HR services do not enter an end date for the personal data of users on permanent contracts. So we prefer to use the start and end dates of their contracts.
