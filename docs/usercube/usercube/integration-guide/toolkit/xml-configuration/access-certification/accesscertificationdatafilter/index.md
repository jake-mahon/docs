...
| IncludeManualAssignmentNotAllowed   default value: true | __Type__    Boolean   __Description__   ```true``` to include in the certification the resources that cannot be requested manually, i.e. those from [resource types](/docs/product_docs/usercube/usercube/integration-guide/toolkit/xml-configuration/provisioning/resourcetype/index.md#resource-types) with ```ApprovalWorkflowType``` set to ```ManualAssignmentNotAllowed```. |
