...
| IncludeManualAssignmentNotAllowed   default value: true | __Type__    Boolean   __Description__   ```true``` to include in the certification the resources that cannot be requested manually, i.e. those from [resource types](../../provisioning/resourcetype/index.md) with ```ApprovalWorkflowType``` set to ```ManualAssignmentNotAllowed```. |
