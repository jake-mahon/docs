...
2. Write profile rules and profile rule contexts to make the previously created dimensions act as filters in rules meant to assign profiles to users.
   ...
   The profile rule context must use a [Profile Rule Context](/docs/usercube/usercube/integration-guide/toolkit/xml-configuration/access-control/profilerulecontext/index.md)to define the entity type that contains the dimension information.
