...
- Is missing one of the needed permissions:
    ...
    3. In the **Enter the object names to select** textbox, enter the service account name in the
       down-level logon format, such as `CONTOSO/UsercubeContosoServer`.
    4. Click **OK**.
...
{ ... "IdentityServer": { "X509KeyFilePath": "./UsercubeContoso.pfx", "X509KeyFilePassword":
"eff@�%fmel/" } ... }
...
`CONTOSO/UsercubeContosoServer` has been set as the Usercube Server IIS website identity.
